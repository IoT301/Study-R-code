...
# Main Logo - study1 - Circle - Transparent Bg.png

```

```{r}
#| label: fig-include-figure
#| fig-cap: study1 Main Logo
#| out-width: "350px"
#| echo: false
knitr::include_graphics(here("Pictures","study1_photo.png"))
```

::: {.callout-tip title="Study start date"}
Harmonized study1 study was launched on $23^{rd}$ November 2022.
:::

## Harmonized study1 - study2 Screening & Enrollment Flowchart

The figure below describes the current metrics with regard to `pre-screening`, `screening` and `enrollments`.

```{r}
#| label: fig-cascade
#| fig-cap: "Harmonized study1 - study2 Screening & Enrollment Flowchart"
...
Data <- read_rds(here("Data","study1-study2.rds")) 
Data1 <- Data %>% 
...
Locked_study1 <- read_rds(here("Data","Locked_study1.rds"))

study2 <-Data1 %>% filter(SCRN_OBSSTDAT>=ymd("2023-04-03")) |> 
  anti_join(Data1 |> drop_na(studyID) |> filter(vist=="Enrollment") |>
              filter(CON_DSSTDAT<ymd("2023-04-03")) |> select(studyID))

# study1 FLOWCHART -------------------------------------
enrollments_study2 <- study2 %>% drop_na(studyID) %>% distinct(studyID) %>% nrow()
pre_scrn_study1 <- Data1 %>% drop_na(SCRN_ID) %>% distinct(SCRN_ID) %>% nrow()
enrollmennts_study1 <- Data1 %>% drop_na(studyID) %>% distinct(studyID) %>% nrow()
scrnd_study1 <- Data1 %>%  filter(complete.cases(US_OHOSTDAT)) %>% distinct(SCRN_ID) %>% nrow()
not_enrol_study1 <- pre_scrn_study1 - enrollmennts_study1

Ga_less14 <- Data1 %>% drop_na(studyID) %>% filter(vist=="Enrollment") %>%
  select(studyID,US_GA_WKS_AGE_FTS1,US_GA_DAYS_AGE_FTS1) %>%
  mutate(Ga_weeks = US_GA_WKS_AGE_FTS1 +
           (US_GA_DAYS_AGE_FTS1/7)) %>% filter(Ga_weeks<14) %>% nrow()

Ga_less18 <- Data1 %>% drop_na(studyID) %>% filter(vist=="Enrollment") %>%
  select(studyID,US_GA_WKS_AGE_FTS1,US_GA_DAYS_AGE_FTS1) %>%
  mutate(Ga_weeks = US_GA_WKS_AGE_FTS1 +
           (US_GA_DAYS_AGE_FTS1/7)) %>% filter(Ga_weeks<18) %>% nrow()

Ga_18_les20 <- Data1 %>% drop_na(studyID) %>% filter(vist=="Enrollment") %>%
  select(studyID,US_GA_WKS_AGE_FTS1,US_GA_DAYS_AGE_FTS1) %>%
  mutate(Ga_weeks = US_GA_WKS_AGE_FTS1 +
           (US_GA_DAYS_AGE_FTS1/7)) %>% filter(Ga_weeks>=18.0 & Ga_weeks<20.0) %>% nrow()

trim1_study1 <- Data1 %>% drop_na(studyID) %>% filter(vist=="Enrollment") %>%
  select(studyID,US_GA_WKS_AGE_FTS1,US_GA_DAYS_AGE_FTS1) %>%
  mutate(Ga_weeks = US_GA_WKS_AGE_FTS1 +
...
trim2_study1 <- Data1 %>% drop_na(studyID) %>% filter(vist=="Enrollment") %>%
  select(studyID,US_GA_WKS_AGE_FTS1,US_GA_DAYS_AGE_FTS1) %>%
  mutate(Ga_weeks = US_GA_WKS_AGE_FTS1 +
           (US_GA_DAYS_AGE_FTS1/7)) %>% filter(Ga_weeks<18) %>%
  mutate(trimester=if_else(Ga_weeks<14,"trimester1","trimester2")) %>%
...
# trimester2 <- Ga_18_les20 + trim2_study1
...
[1]: paste0('Total Number Pre-Screened ','(',pre_scrn_study1,')')
[2]: paste0('Total Screened by US  ',scrnd_study1,' (',round((scrnd_study1/pre_scrn_study1)*100,1),'%)')
[3]: paste0('Total Enrollments  ',enrollmennts_study1, ' (',round((enrollmennts_study1/scrnd_study1)*100,1),'%)')
[4]: paste0('GA: less than 18 weeks  ','(',Ga_less18,') ',round((Ga_less18/enrollmennts_study1)*100,1),'%')
[5]: paste0('18 =< GA(wks) < 20:  ','(',Ga_18_les20,') ',round((Ga_18_les20/enrollmennts_study1)*100,1),'%')
[6]: paste0('Trimester 1:  ','(',trim1_study1,'/',Ga_less18,') ',round((trim1_study1/Ga_less18)*100,1),'%')
[7]: paste0('Trimester 2:  ','(',trim2_study1,'/',Ga_less18,') ',round((trim2_study1/Ga_less18)*100,1),'%')
[8]: paste0('Total exclusions  ',not_enrol_study1,'(',round((not_enrol_study1/pre_scrn_study1)*100,1),'%)')
[10]: paste0('GA<14 wks: ',Ga_less14, ' (',round(Ga_less14/enrollmennts_study1*100,1),'%)')

      ")
```

A total of **`r pre_scrn_study1`** potential participants have been pre-screened, with **`r scrnd_study1`** screened for enrollment based on the ultrasound (GA \< 20 weeks). Of all enrollments (**`r enrollmennts_study1`**), it's worth noting that (**`r Ga_less14`/`r enrollmennts_study1`**) -- **`r paste0(round(Ga_less14/enrollmennts_study1*100,1),'%')`** were enrolled with GA less than 14 weeks.

**`r enrollments_study2`** participants have been co-enrolled in Redefining Maternal Anemia in Pregnancy and Postpartum (**study2**) study, Which was launched on $3^{rd}$ April 2023.
...
enrol_dat <- Data1 %>% filter(complete.cases(studyID)) %>%  filter(vist=="Enrollment") %>%
  distinct(studyID, .keep_all = T)  %>%  tabyl(SCRN_FAC_SPFY_OBSLOC) %>% drop_na() %>%
 ...
 enrol_dat %>% ggplot(aes(x = reorder( SCRN_FAC_SPFY_OBSLOC,percent), y = n  ,fill=SCRN_FAC_SPFY_OBSLOC)) +
  ...
  scale_x_discrete("",
                   labels = c("facility_1" = "facility_1",
                              "facility_2" = "KUOYO",
                              "facility_3" = "Jamii Clinical \n Services Clinic",
                              "facility_4" = "SIAYA",
                              "facility_5" = "KCRH")) +
  labs(y = "Counts",
...
scrn_dta <- Data1 %>% filter(complete.cases(US_OHOSTDAT)) %>%  distinct(SCRN_ID,.keep_all = T)  %>% 
...
#| fig-width: 12
#| fig-height: 6
PRESCREENS_dta <- Data1 %>% distinct(SCRN_ID, .keep_all = T) %>%  tabyl(SCRN_FAC_SPFY_OBSLOC) %>% drop_na() %>%
  mutate(percent = round(percent*100,1)) %>%
...
@fig-exclusion displays the the reasons for failure to enroll most of our potential participants. A total of **`r not_enrol_study1`** have been excluded from the study.
...
Exclusion_data <- Data1 %>% filter(is.na(studyID)) %>% #filter(OTHR_IEORRES =="Yes") %>% 
...
#select(SCRN_ID,Reason,OTHR_IEORRES,OTHR_REASON_IEORRES) %>% 
# View()


outside_catch <- Exclusion_data %>% select(SCRN_ID,studyID,CATCHMENT_IEORRES,Reason) %>% filter(CATCHMENT_IEORRES == "No"|Reason =="outside the study area") %>% 
  filter(!SCRN_ID==11235)%>% nrow()
# Age requirement
age_require <- Exclusion_data %>% select(SCRN_ID,studyID,AGE_IEORRES,Reason) %>%  filter(AGE_IEORRES == "No") %>% nrow()
# Ga by LMP
GA_lmp <- Exclusion_data %>% select(SCRN_ID,studyID,EGA_LT25_IEORRES,Reason) %>% filter(EGA_LT25_IEORRES == "No"|Reason=="Above 25 weeks (LMP)") %>% nrow()
#Deferred
Deferred <- Exclusion_data %>% distinct(SCRN_ID, .keep_all = T) %>% filter(!complete.cases(studyID)) %>% select(SCRN_ID,studyID,OTHR_REASON_IEORRES,US_GA_WKS_AGE_FTS1,Reason) %>%
...
4# spousal consent
Spousal_consent <- Exclusion_data  %>% select(SCRN_ID,studyID,OTHR_REASON_IEORRES,Reason) %>% 
  filter(OTHR_REASON_IEORRES=="needs to seek spousal consent before she can accept"|Reason == "needs to seek spousal consent") %>% nrow()
# Not interested
Not_interested <- Exclusion_data %>% filter(is.na(studyID)) %>%  select(SCRN_ID,studyID,OTHR_REASON_IEORRES,Reason) %>% 
  filter(OTHR_REASON_IEORRES=="Not interested in research/not interested in study"|Reason == "Not interested in research") %>% nrow()
# Ga above 20 weeks
Ga_above20wks <- Exclusion_data %>% filter(is.na(studyID)) %>%
  select(SCRN_ID,studyID,US_GA_WKS_AGE_FTS1,Reason) %>% 
...
Others <- Exclusion_data %>% filter(is.na(studyID)) %>% filter(OTHR_IEORRES =="Yes"& Reason=="Others") %>% nrow()
...
data.frame(outside_catch,age_require,GA_lmp,Deferred,Spousal_consent,Not_interested,Ga_above20wks,busy,blood_draw,not_feasible,relocate,refusal,Others) %>% 
  ...
  mutate(percent = round((number/not_enrol_study1)*100,1)) %>% 
  ...
  labs(title = "Reasons for Exclusion from study1 Study",x="", subtitle = paste0("Total exclusions: ", not_enrol_study1))+
  theme_classic() +
 # scale_fill_brewer(palette = 2) +
  theme(
...
Among the outstanding reasons of exclusion, **Non residence** forms the majority, (**`r outside_catch`**) -- `r paste0(round((outside_catch/not_enrol_study1)*100,1),"%")`, followed by **Estimated GA above 25 weeks** by LMP & Deferred to come on a later date at **`r paste0(round((GA_lmp/not_enrol_study1)*100,1),"%")`** & **`r paste0(round((Deferred/not_enrol_study1)*100,1),"%")`** respectively. The table below stratify the reasons for exclusion by facility.
...
outside_catch_ <- Exclusion_data %>% select(SCRN_ID,studyID,SCRN_FAC_SPFY_OBSLOC,CATCHMENT_IEORRES,Reason) %>% 
  filter(CATCHMENT_IEORRES == "No"|Reason =="outside the study area") %>% 
  filter(!SCRN_ID==11235) %>% group_by(SCRN_FAC_SPFY_OBSLOC) %>% count() %>% rename("Outside catchment area" =n)


# Age requirement
age_require_ <- Exclusion_data %>% select(SCRN_ID,SCRN_FAC_SPFY_OBSLOC,studyID,AGE_IEORRES,Reason) %>%  filter(AGE_IEORRES == "No") %>% 
  group_by(SCRN_FAC_SPFY_OBSLOC) %>% count() %>% rename("Age requirement"=n)
# Ga by LMP
GA_lmp_ <- Exclusion_data %>% select(SCRN_ID,SCRN_FAC_SPFY_OBSLOC,studyID,EGA_LT25_IEORRES,Reason) %>% filter(EGA_LT25_IEORRES == "No"|Reason=="Above 25 weeks (LMP)") %>% 
  group_by(SCRN_FAC_SPFY_OBSLOC) %>% count() %>% rename("Est.GA>25wks (LMP)" =n)
#Deferred
Defered <- Exclusion_data %>% distinct(SCRN_ID, .keep_all = T) %>% filter(!complete.cases(studyID)) %>% 
  select(SCRN_ID,SCRN_FAC_SPFY_OBSLOC,studyID,OTHR_REASON_IEORRES,US_GA_WKS_AGE_FTS1,Reason) %>%
  filter(OTHR_REASON_IEORRES == "deferred—to come for enrollment on later date"|Reason =="deferred") %>% 
  mutate(US_GA_WKS_AGE_FTS_20 = if_else(US_GA_WKS_AGE_FTS1>=20,"Yes",NA),
         US_GA_WKS_AGE_FTS_20 = if_else(is.na(US_GA_WKS_AGE_FTS_20),"No",US_GA_WKS_AGE_FTS_20)) %>% 
...
# spousal consent
Spousal_consnt <- Exclusion_data  %>% select(SCRN_ID,SCRN_FAC_SPFY_OBSLOC,studyID,OTHR_REASON_IEORRES,Reason) %>% 
...
# Not interested
Not_interestd <- Exclusion_data %>% filter(is.na(studyID)) %>%  select(SCRN_ID,SCRN_FAC_SPFY_OBSLOC,studyID,OTHR_REASON_IEORRES,Reason) %>% 
...
# Ga above 20 weeks
Ga_above20wk <- Exclusion_data %>% filter(is.na(studyID)) %>%
  select(SCRN_ID,SCRN_FAC_SPFY_OBSLOC,studyID,US_GA_WKS_AGE_FTS1,Reason) %>% 
...
Other <- Exclusion_data %>% filter(is.na(studyID)) %>% filter(OTHR_IEORRES =="Yes"& Reason=="Others") %>% 
  group_by(SCRN_FAC_SPFY_OBSLOC) %>% count() %>% rename(Others = n)

Exclusion_data  %>% select(SCRN_ID,SCRN_OBSSTDAT,SCRN_FAC_SPFY_OBSLOC,Reason) %>% 
  ...
  mutate(Facility = str_replace(Facility,"Lumumba Sub County Hospital","LUMUMBA"),
         Facility = str_replace(Facility,"facility_4","SIAYA"),
         Facility = str_replace(Facility,"facility_5","KCRH"),
         Facility = str_replace(Facility,"facility_2","KUOYO"),
         Facility = str_replace(Facility,"facility_7","TINGWANGI"),
...
deferred_table <- Exclusion_data %>% distinct(SCRN_ID, .keep_all = T) %>% filter(!complete.cases(studyID)) %>% 
  select(SCRN_ID,SCRN_OBSSTDAT,studyID,OTHR_REASON_IEORRES,US_GA_WKS_AGE_FTS1,Reason) %>%
  filter(OTHR_REASON_IEORRES == "deferred—to come for enrollment on later date"|Reason =="deferred") %>% 
  mutate(US_GA_WKS_AGE_FTS_20 = if_else(US_GA_WKS_AGE_FTS1>=20,"Yes",NA),
         US_GA_WKS_AGE_FTS_20 = if_else(is.na(US_GA_WKS_AGE_FTS_20),"No",US_GA_WKS_AGE_FTS_20)) %>% 
...
Exclusion_data  %>% select(SCRN_ID,SCRN_OBSSTDAT,SCRN_FAC_SPFY_OBSLOC,studyID,OTHR_REASON_IEORRES,Reason) %>% 
  filter(OTHR_REASON_IEORRES=="needs to seek spousal consent before she can accept"|Reason == "needs to seek spousal consent") %>% 
...
Exclusion_data %>% select(SCRN_ID,studyID,SCRN_OBSSTDAT,SCRN_FAC_SPFY_OBSLOC,CATCHMENT_IEORRES,Reason) %>% 
  filter(CATCHMENT_IEORRES == "No"|Reason =="outside the study area") %>% 
  filter(!SCRN_ID==11235) %>% 
  mutate(year_month = zoo::as.yearmon(ymd(SCRN_OBSSTDAT))) %>% tabyl(year_month) %>% 
  mutate(year_month = as.factor(year_month)) %>% 
  ggplot(aes(x = year_month, y = n)) + 
...
Data1 %>% drop_na(REFERRER_COMM) %>% distinct(SCRN_ID,.keep_all = T) %>% 
  ...
  labs(x="",y="", title = "Top 10 study1 Referrals")+
  theme_light() +
  theme(
...
# Data1 %>% select(SCRN_ID,studyID,REFERRAL_DT) %>%
...
Data1 %>% drop_na(REFERRER_FAC) %>% #select(SCRN_ID,REFERRER_FAC) %>% View()
  distinct(SCRN_ID,.keep_all = T) %>% 
...
facility_1_scr <- (Data1 %>% distinct(SCRN_ID, .keep_all = T) %>%
                 filter(SCRN_FAC_SPFY_OBSLOC =="facility_1") %>% count())$n
KUOYO_scr <- (Data1 %>% distinct(SCRN_ID, .keep_all = T) %>%
                filter(SCRN_FAC_SPFY_OBSLOC =="facility_2") %>% count())$n
SIAYA_scr <- (Data1 %>% distinct(SCRN_ID, .keep_all = T) %>%
                filter(SCRN_FAC_SPFY_OBSLOC =="facility_4") %>% count())$n
KCRH_scr <- (Data1 %>% distinct(SCRN_ID, .keep_all = T) %>%
               filter(SCRN_FAC_SPFY_OBSLOC =="facility_5") %>% count())$n
LUMUMBA_scr <- (Data1 %>% distinct(SCRN_ID, .keep_all = T) %>%
                  filter(SCRN_FAC_SPFY_OBSLOC =="Lumumba Sub County Hospital") %>% count())$n

# Enrolments
facility_1_enrol <- (Data1 %>% filter(complete.cases(studyID)) %>%  distinct(studyID, .keep_all = T) %>%
                   filter(SCRN_FAC_SPFY_OBSLOC =="facility_1") %>% count())$n
KUOYO_enrol <- (Data1 %>% filter(complete.cases(studyID)) %>%  distinct(studyID, .keep_all = T) %>%
                  filter(SCRN_FAC_SPFY_OBSLOC =="facility_2") %>% count())$n
SIAYA_enrol <- (Data1 %>% filter(complete.cases(studyID)) %>%  distinct(studyID, .keep_all = T) %>%
                  filter(SCRN_FAC_SPFY_OBSLOC =="facility_4") %>% count())$n
KCRH_enrol <- (Data1 %>% filter(complete.cases(studyID)) %>%  distinct(studyID, .keep_all = T) %>%
                 filter(SCRN_FAC_SPFY_OBSLOC =="facility_5") %>% count())$n
LUMUMBA_enrol <- (Data1 %>% filter(complete.cases(studyID)) %>%  distinct(studyID, .keep_all = T) %>%
                    filter(SCRN_FAC_SPFY_OBSLOC =="Lumumba Sub County Hospital") %>% count())$n
# dataframe
scrn <- c(facility_1_scr,KUOYO_scr,SIAYA_scr,KCRH_scr,LUMUMBA_scr)
Enrol <- c(facility_1_enrol,KUOYO_enrol,SIAYA_enrol,KCRH_enrol,LUMUMBA_enrol)
facilities <- c("facility_1","KUOYO","SIAYA","KCRH","LUMUMBA")
...
#| fig-height: 5
target=98
trend_data <-  Data1 %>% filter(complete.cases(studyID)) %>% filter(vist == "Enrollment") %>%
  select(SCRN_ID,studyID,CON_DSSTDAT,visit_date,SCRN_OBSSTDAT) %>% distinct(studyID,.keep_all = T) %>% 
  mutate(Mon_year = zoo::as.yearmon(CON_DSSTDAT),
         Mon_year = as.factor(Mon_year)) %>% drop_na(studyID) %>%
...
Cumulatively, enrollments is at **`r enrollmennts_study1`**. Currently, our target is set to **`r target`**. In the month of **`r paste(month(today(),label=T),year(today()))`**, we have so far enrolled **`r (trend_data %>% filter(Mon_year==paste(month(today(),label=T),year(today()))))[1,2]`** participants
...
trend_dta <-  Data1 %>% filter(complete.cases(studyID)) %>% filter(vist == "Enrollment") %>%
  select(studyID,CON_DSSTDAT,US_GA_WKS_AGE_FTS1,US_GA_DAYS_AGE_FTS1) %>% distinct(studyID,.keep_all = T) %>%
  mutate(Ga_weeks = US_GA_WKS_AGE_FTS1 +
...
enrolltrn <- Data1 %>% drop_na(studyID) %>%  select(studyID,CON_DSSTDAT) %>% 
...
# prescreen
prescreen_trnd <- Data1 %>% select(SCRN_ID,SCRN_OBSSTDAT) %>% distinct(SCRN_ID,.keep_all = T) %>% 
  mutate(Mon_year = factor(zoo::as.yearmon(SCRN_OBSSTDAT))) %>% group_by(Mon_year) %>% count() %>% 
  rename(PreScreens = n) #%>% filter(Mon_year!="Feb 2022")
#screen
screentrn <- Data1 %>% filter(complete.cases(US_OHOSTDAT)) %>%  distinct(SCRN_ID,.keep_all = T)  %>% 
...
# #| fig-width: 11
# #| fig-height: 5
# Data1 %>% select(SCRN_ID,studyID,SCRN_OBSSTDAT,contains("lmp")) %>% distinct(SCRN_ID,.keep_all = T) %>% 
...
#| fig-height: 5
# Enrolmments
Enrols_data <- Data1 %>% filter(complete.cases(studyID)) %>%
  select(studyID,vist,CON_DSSTDAT,SCRN_OBSSTDAT,SCRN_FAC_SPFY_OBSLOC) %>%
  filter(vist == "Enrollment") %>%
  distinct(studyID,.keep_all = T) %>% drop_na(studyID) %>%
...
# screens
Scrns_data <- Data1 %>% distinct(SCRN_ID, .keep_all = T) %>% drop_na(SCRN_ID) %>%
...
::: panel-tabset
#### Proportion of study1 linked participants

```{r}
#| label: fig-hdsslinkage
#| fig-cap: Proportion of study1 participnats linked with HDSS
#| echo: false
#| fig-width: 8
#| fig-height: 5

hdss_tab <- Data1 %>% drop_na(studyID) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CON_DSSTDAT,SCRN_FAC_SPFY_OBSLOC,contains("hdss")) %>% 
  group_by(studyID) %>% fill(everything(.)) %>% distinct(studyID,.keep_all = T) %>% 
...
not_linked <- Data1 %>% drop_na(studyID) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CON_DSSTDAT,SCRN_FAC_SPFY_OBSLOC,contains("hdss")) %>% 
  group_by(studyID) %>% fill(everything(.)) %>% distinct(studyID,.keep_all = T) %>% 
  mutate(Hdss_Linkage= if_else(is.na(HDSS_SUBJID),"Not yet linked","Linkage done")) %>% filter(Hdss_Linkage=="Not yet linked") %>% nrow()

hdss_pending <- Data1 %>% drop_na(studyID) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CON_DSSTDAT,SCRN_FAC_SPFY_OBSLOC,contains("hdss")) %>% 
  group_by(studyID) %>% fill(everything(.)) %>% distinct(studyID,.keep_all = T) %>%  
  mutate(Hdss_Linkage= if_else(is.na(HDSS_SUBJID),"Not yet linked","Linkage done")) %>% 
...
Of all the study1 enrollments, **`r not_linked`** participants have not been linked to DSS. **`r hdss_pending`** are indicated as pending linkage.

#### Participants notyet linked with HDSS

```{r}
#| label: fig-missinglinkage
#| fig-cap: Counts of study1 participnats pending HDSS Linkage
#| echo: false
#| fig-width: 11
#| fig-height: 5
Hdss_missing_linkage <- Data1 %>% drop_na(studyID) %>% 
  select(studyID,SCRN_FAC_SPFY_OBSLOC,CON_DSSTDAT,SCRN_FAC_SPFY_OBSLOC,contains("hdss")) %>% 
  group_by(studyID) %>% fill(everything(.)) %>% distinct(studyID,.keep_all = T) %>%  
  mutate(Hdss_Linkage= if_else(is.na(HDSS_SUBJID),"Not yet linked","Linkage done")) %>% 
  filter(Hdss_Linkage =="Not yet linked") 

Hdss_missing_linkage %>% tabyl(SCRN_FAC_SPFY_OBSLOC) %>% 
  mutate(SCRN_FAC_SPFY_OBSLOC=str_replace(SCRN_FAC_SPFY_OBSLOC,"facility_5","KCRH"),
         SCRN_FAC_SPFY_OBSLOC=str_replace(SCRN_FAC_SPFY_OBSLOC,"facility_2","KUOYO"),
         SCRN_FAC_SPFY_OBSLOC=str_replace(SCRN_FAC_SPFY_OBSLOC,"Lumumba Sub County Hospital","LUMUMBA"),
         SCRN_FAC_SPFY_OBSLOC=str_replace(SCRN_FAC_SPFY_OBSLOC,"facility_4","SIAYA")) %>% 
...
Data1 %>% drop_na(studyID) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CON_DSSTDAT,SCRN_FAC_SPFY_OBSLOC,contains("hdss")) %>% 
  group_by(studyID) %>% fill(everything(.)) %>% distinct(studyID,.keep_all = T) %>%  
  mutate(Hdss_Linkage= if_else(is.na(HDSS_SUBJID),"Not yet linked","Linkage done")) %>% 
  filter(Hdss_Linkage =="Not yet linked") %>% filter(HDSSLinkage_Comments == "Pending") %>% #View()
  select(studyID,CON_DSSTDAT) %>% mutate(time_elapsed = as.numeric(today() - CON_DSSTDAT),
...
#| fig-cap: List of study1 participnats pending HDSS Linkage
#| echo: false
#| warning: false
#| column: body-outset-right
#| fig-width: 14
#| fig-height: 8
Data1 %>% drop_na(studyID) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CON_DSSTDAT,SCRN_FAC_SPFY_OBSLOC,contains("hdss")) %>% 
  group_by(studyID) %>% fill(everything(.)) %>% distinct(studyID,.keep_all = T) %>%  ungroup() %>% 
  mutate(Hdss_Linkage= if_else(is.na(HDSS_SUBJID),"Not yet linked","Linkage done")) %>% 
...
Data1 %>%  drop_na(studyID) %>% drop_na(vist) %>% 
filter(MAT_VISIT_MNH04%in%c("Yes, visit conducted in person",NA)) %>% filter(MAT_VISIT_MNH12%in%c("Yes, visit conducted in person",NA)) %>%  full_join(Data1 %>% filter(studyID=="KEARC00300")) %>% 
  select(studyID,vist) %>% group_by(studyID,vist) %>% distinct() %>% ungroup() %>%
  dplyr::group_by(vist) %>% count() %>% filter(!vist%in%c("Non-scheduled visit","14")) %>%
  mutate(vist = factor(vist,levels = c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","IPC","PNC-0",
                                                   "PNC-1","PNC-4","PNC-6","PNC-26","PNC-52"))) %>%
  mutate(vist=fct_relevel(str_wrap(vist,width=6), "Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","IPC","PNC-0",
                                "PNC-1","PNC-4","PNC-6","PNC-26","PNC-52")) %>% drop_na(vist) %>% 
  ggplot(aes(x=(vist),y=n))+
...
Data1 %>% filter(SCRN_OBSSTDAT>=ymd("2023-04-03")) |> 
  anti_join(Data1 |> drop_na(studyID) |> filter(vist=="Enrollment") |>
              filter(CON_DSSTDAT<ymd("2023-04-03")) |> select(studyID)) %>% 
  filter(complete.cases(studyID)) %>% drop_na(vist) %>% 
filter(MAT_VISIT_MNH04%in%c("Yes, visit conducted in person",NA)) %>% filter(MAT_VISIT_MNH12%in%c("Yes, visit conducted in person",NA)) %>% 
  full_join(Data1 %>% filter(studyID=="KEARC00300")) %>% 
  select(studyID,vist) %>% group_by(studyID,vist) %>% distinct() %>% ungroup() %>%
  dplyr::group_by(vist) %>% count() %>% filter(!vist%in%c("Non-scheduled visit","14")) %>%
  mutate(vist = factor(vist,levels = c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","IPC","PNC-0",
                                                   "PNC-1","PNC-4","PNC-6","PNC-26","PNC-52"))) %>%
...
**study2 Aim 3** started in `February 2024`. It is rolling over into the next five subsequent months, hence anticipated to end in the month of `June 2024`.

```{r}
April_trime2 <- readxl::read_xlsx("C:/Users/User/OneDrive - Kenya Medical Research Institute/study2 Cloud/study2 Sampled data/May trimester 2 sample_2024-05-02.xlsx") %>% 
  select(-last_seen)
April_trime3 <- readxl::read_xlsx("C:/Users/User/OneDrive - Kenya Medical Research Institute/study2 Cloud/study2 Sampled data/May trimester 3 sample_2024-05-02.xlsx") %>% 
select(-last_seen)
```

## Progress of study2 Aim 3
...
ALL_Random_sample <- readxl::read_xlsx("C:/Users/User/OneDrive - Kenya Medical Research Institute/study2 Cloud/study2 Sampled data/ALL Random sample.xlsx")

Aim3 <- Data1 %>% drop_na(studyID) %>% drop_na(LB_study23_TRI) %>% droplevels() %>% 
  ...
  mutate(Trimester = case_when(Ga_visit_weeks<14~"Trimester I (<14 weeks)",
                               vist=="ANC-20"~"Trimester II (ANC-20)",
                               vist=="ANC-32"~"Trimester III (ANC-32)")) %>% 
 select(studyID,US_GA_WKS_AGE_FTS1_Enrol,vist,SCRN_FAC_SPFY_OBSLOC,SCRN_OBSSTDAT,CON_DSSTDAT,ANC_OBSSTDAT,
        MAT_SPEC_COLLECT_DAT,visit_date,LB_study23_TRI,LB_study23,month_year,Trimester,FORMCOMPLID_MNH07,FORMCOMPLDAT_MNH07) #%>% 

#  filter(month_year=="May 2024") %>% View()

# April_trime3 %>% select(studyID) %>% left_join(Data1 %>% drop_na(studyID) %>% select(studyID,vist,visit_date,LB_study23,LB_study23_TRI,FORMCOMPLID_MNH07)) %>% View()
...
Data1 %>% drop_na(studyID) %>% select(studyID,vist,visit_date,Ga_visit_weeks,SCRN_FAC_SPFY_OBSLOC) %>% filter(studyID=="KEARC00952") %>% View()
...
Aim3 %>%  select(month_year,Trimester) %>% 
 ...
 # Data1 %>% drop_na(studyID) %>% drop_na(LB_study23_TRI) %>% droplevels() %>% mutate(month_year = zoo::as.yearmon(visit_date)) %>%
 #  mutate(Trimester = case_when(Ga_visit_weeks<14~"Trimester I (<14 weeks)",
 #                               vist=="ANC-20"~"Trimester II (ANC-20)",
 #                               vist=="ANC-32"~"Trimester III (ANC-32)")) %>%
 # select(studyID,US_GA_WKS_AGE_FTS1_Enrol,vist,SCRN_FAC_SPFY_OBSLOC,SCRN_OBSSTDAT,CON_DSSTDAT,ANC_OBSSTDAT,
 #        MAT_SPEC_COLLECT_DAT,visit_date,LB_study23,month_year,Trimester,FORMCOMPLID_MNH07,FORMCOMPLDAT_MNH07) %>% arrange(month_year,vist) %>%
 #   View()
 # 
 # Data1 %>% drop_na(studyID) %>% drop_na(LB_study23_TRI) %>% droplevels() %>% mutate(month_year = zoo::as.yearmon(visit_date)) %>%
 #  # select(studyID,US_GA_WKS_AGE_FTS1_Enrol,vist,SCRN_FAC_SPFY_OBSLOC,Ga_visit_weeks,LB_study23_TRI,LB_study23,visit_date) %>%
 #  mutate(Trimester = case_when(Ga_visit_weeks<14~"Trimester I (<14 weeks)",
 #                               vist=="ANC-20"~"Trimester II (ANC-20)",
 #                               vist=="ANC-32"~"Trimester III (ANC-32)")) %>%
 # select(studyID,US_GA_WKS_AGE_FTS1_Enrol,vist,SCRN_FAC_SPFY_OBSLOC,SCRN_OBSSTDAT,CON_DSSTDAT,
 #        MAT_SPEC_COLLECT_DAT,LB_study23,month_year,Trimester,FORMCOMPLID_MNH07,FORMCOMPLDAT_MNH07) %>% #View()
 #  filter(month_year=="April 2024") %>% #View()
 # filter(Trimester=="Trimester II (ANC-20)")  %>% View()
 # 
 # Data1 %>% filter(studyID=="KEARC00926") %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,LB_study23_TRI,LB_study23) %>% View()
...
June_trime2 %>% select(1,2,5,6,7) %>% 
  ...
  left_join(Data1 %>% filter(vist=="ANC-20") %>% select(studyID,vist,visit_date)) %>% 
  filter(!complete.cases(visit_date)) %>%  arrange(SCRN_FAC_SPFY_OBSLOC) %>%  select(1:5) %>% mutate(index = row_number()) %>% gt() %>% 
  tab_header(title = "study2 AIM 3 Trimester II (pending/yet to be seen) participants -- Month of June") %>% 
...
June_trime3 %>% select(1,4,6,7,8) %>% 
  ...
  left_join(Data1 %>% filter(vist=="ANC-32") %>% select(studyID,vist,visit_date)) %>% 
  filter(!complete.cases(visit_date)) %>% select(1:5) %>% arrange(SCRN_FAC_SPFY_OBSLOC) %>% mutate(index = row_number()) %>%  gt() %>% 
  tab_header(title = "study2 AIM 3 Trimester III (pending/yet to be seen) participants -- Month of June") %>% 
  opt_table_outline() %>% opt_table_lines() %>% 
  opt_row_striping() %>% opt_vertical_padding(scale = .5) %>%
  tab_options( table.font.size = 11,) %>% 
  tab_style(style = cell_text(weight = "bold"),
...
## Sample collection among study2 Aim 3 Participants

```{r}
#| label: fig-ReMpp_samp
#| fig-cap: "study2 sample collection"
#| echo: false
#| warning: false
#| column: body-outset-right
#| fig-width: 11
...
study2_spe_sample <- Data1 %>% filter(LB_study23=="Yes") %>%
  select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,visit_date,LB_study23,LB_study23_TRI,
         MAT_BLOOD_SPEC_1:MAT_HELM_SPEC_COLLECT_COUNT,HELM_LBORRES) 

study2_spe_sample %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist, MAT_BLOOD_SPEC_COUNT_1) %>% distinct(studyID,.keep_all = T) %>%
  ...
  rbind(study2_spe_sample %>% filter(complete.cases(studyID)) %>% select(studyID, SCRN_FAC_SPFY_OBSLOC,vist, MAT_BLOOD_SPEC_COUNT_3) %>% distinct(studyID,.keep_all = T) %>%
  ...
  rbind(study2_spe_sample %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist, MAT_BLOOD_SPEC_COUNT_4) %>% distinct(studyID,.keep_all = T) %>%
          ...
          tabyl(Result) %>% mutate(Sample = paste0("Trace-element-free\n polyethylene evacuated tubes"),
                                   Percent = round(percent*100,1)) %>%
          filter(Result == "Result Available") %>% select(Result,Sample, n , Percent)) %>%
  rbind(study2_spe_sample %>% filter(complete.cases(studyID)) %>% select(studyID, SCRN_FAC_SPFY_OBSLOC,vist, MAT_BLOOD_SPEC_COUNT_5) %>% distinct(studyID,.keep_all = T) %>%
  ...
  rbind(study2_spe_sample %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist, MAT_BLOOD_SPEC_COUNT_6) %>% distinct(studyID,.keep_all = T) %>%
  ...
  rbind(study2_spe_sample %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist, MAT_URINE_COLLECT_COUNT) %>% distinct(studyID,.keep_all = T) %>%
  ...
  rbind(study2_spe_sample %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist, MAT_TB_SYM_TEST, MAT_TB_SPEC_COLLECT_COUNT) %>% distinct(studyID,.keep_all = T) %>%
  ...
  rbind(study2_spe_sample %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,HELM_LBORRES, MAT_HELM_SPEC_COLLECT_COUNT) %>% distinct(studyID,.keep_all = T) %>%
  ...
  #ylim(0,125) +
  labs(x = "",
      # title = "Sample Collection For study2 Aim 3 Participants",
       y = "Completeness (%)") +
  theme_light() +
...
Reten_Data <- Data1 %>% filter(complete.cases(studyID)) %>%  select(studyID,vist,ANC_OBSSTDAT,DELIV_DSSTDAT_INF1,CLOSE_DSSTDAT) %>%
  group_by(studyID,vist) %>% fill(DELIV_DSSTDAT_INF1) %>% ungroup() %>%
  ...
  full_join(Data1 %>% filter(vist == "Enrollment") %>% distinct(studyID,.keep_all = T) %>%
              select(studyID,CON_DSSTDAT, vist,SCRN_FAC_SPFY_OBSLOC,US_OHOSTDAT, US_GA_WKS_AGE_FTS1,US_GA_DAYS_AGE_FTS1) %>%
              ...
              select(studyID, CON_DSSTDAT,SCRN_FAC_SPFY_OBSLOC, US_GA_WKS_AGE_FTS1, US_GA_DAYS_AGE_FTS1,
                     ...
                     ANC_32_Latest_Date, ANC_36_Earliest_Date, ANC_36_Actual_Date, ANC_36_Latest_Date)) %>% drop_na(studyID)   #%>% view()

# To be removed
RM_ANC20 <- Reten_Data %>% filter(vist=="IPC") %>% filter(composite_obsdate<ANC_20_Earliest_Date) %>% distinct(studyID) %>%
  full_join( Reten_Data %>% drop_na(CLOSE_DSSTDAT) %>% filter(composite_obsdate<ANC_20_Earliest_Date)%>% distinct(studyID))

RM_ANC28 <- Reten_Data %>% filter(vist=="IPC") %>% filter(composite_obsdate<ANC_28_Earliest_Date) %>% distinct(studyID) %>%
  full_join(Reten_Data %>% drop_na(CLOSE_DSSTDAT) %>% filter(composite_obsdate<ANC_28_Earliest_Date) %>% distinct(studyID))

RM_ANC32 <- Reten_Data %>% filter(vist=="IPC") %>% filter(composite_obsdate<ANC_32_Earliest_Date) %>% distinct(studyID) %>%
  full_join(Reten_Data %>% drop_na(CLOSE_DSSTDAT) %>% filter(composite_obsdate<ANC_32_Earliest_Date) %>% distinct(studyID))

RM_ANC36 <- Reten_Data %>% filter(vist=="IPC") %>% filter(composite_obsdate<ANC_36_Earliest_Date) %>% distinct(studyID) %>%
  full_join(Reten_Data %>% drop_na(CLOSE_DSSTDAT) %>% filter(composite_obsdate<ANC_28_Earliest_Date) %>% distinct(studyID))

#Enrollment
ENROL_NUM <- Reten_Data %>% filter(vist == "Enrollment") %>%
  distinct(studyID,.keep_all = T) %>% mutate(status = paste0("Enrollment")) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,status) #%>% view()
# ANC 20
ANC_20_retaind <- Reten_Data %>% filter(vist=="ANC-20") %>% distinct() %>%  mutate(status = paste0("Retained")) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,status) %>% distinct() %>% anti_join(RM_ANC20)

Anc20_WO <- Reten_Data %>% anti_join(ANC_20_retaind %>% select(studyID)) %>%
  filter(vist=="Enrollment") %>% rowwise() %>%
  mutate(status = case_when(between(today(),ANC_20_Earliest_Date,ANC_20_Latest_Date)~"Window open (Yet to be seen)",
         ...
         vist = replace(vist,vist=="Enrollment","ANC-20")) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,status) %>%
  ungroup() %>% distinct() %>%  anti_join(RM_ANC20)

# ANC 28
ANc28_data <- Reten_Data %>% filter(vist=="ANC-20") %>% distinct() %>%
  full_join(Reten_Data %>% filter(today()>ANC_20_Latest_Date) %>% distinct()) %>% arrange(studyID) %>% anti_join(RM_ANC28)
#ANc28_data %>% tabyl(vist)

ANC_28_retaind <- ANc28_data %>% filter(vist=="ANC-28") %>% distinct() %>%  mutate(status = paste0("Retained")) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,status) %>% distinct() #%>% view()

Anc28_WO <- ANc28_data %>% anti_join(ANC_28_retaind %>% select(studyID)) %>%
  filter(vist%in%c("Enrollment","ANC-20")) %>% distinct(studyID,vist,.keep_all = T) %>% rowwise() %>%
  mutate(status = case_when(between(today(),ANC_28_Earliest_Date,ANC_28_Latest_Date)~"Window open (Yet to be seen)",
         ...
         vist = replace(vist,vist%in%c("Enrollment","ANC-20"),"ANC-28")) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,status) %>%
  ungroup() %>% distinct() #%>% view()


# ANC 32
ANc32_data <- Reten_Data %>% filter(vist=="ANC-28") %>% distinct() %>%
  full_join(Reten_Data %>% filter(today()>ANC_28_Latest_Date) %>% distinct()) %>% arrange(studyID) %>% anti_join(RM_ANC32)

ANC_32_retaind <- ANc32_data %>% filter(vist=="ANC-32") %>% distinct() %>%  mutate(status = paste0("Retained")) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,status) %>% distinct() #%>% view()


Anc32_WO <- ANc32_data %>% anti_join(ANC_32_retaind %>% select(studyID)) %>%
  filter(vist%in%c("Enrollment","ANC-20","ANC-28")) %>% distinct(studyID,vist,.keep_all = T) %>% rowwise() %>%
  mutate(status = case_when(between(today(),ANC_32_Earliest_Date,ANC_32_Latest_Date)~"Window open (Yet to be seen)",
         ...
         vist = replace(vist,vist%in%c("Enrollment","ANC-20","ANC-28"),"ANC-32")) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,status) %>%
  ungroup() %>% distinct() #%>% view()


# ANC 36
ANc36_data <- Reten_Data %>% filter(vist=="ANC-32") %>% distinct() %>%
  full_join(Reten_Data %>% filter(today()>ANC_32_Latest_Date) %>% distinct()) %>% arrange(studyID) %>% anti_join(RM_ANC36)

ANC_36_retaind <- ANc36_data %>% filter(vist=="ANC-36") %>% distinct() %>%  mutate(status = paste0("Retained")) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,status) %>% distinct() #%>% view()


Anc36_WO <- ANc36_data %>% anti_join(ANC_36_retaind %>% select(studyID)) %>%
  filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32")) %>% distinct(studyID,.keep_all = T) %>% rowwise() %>%
  mutate(status = case_when(between(today(),ANC_36_Earliest_Date,ANC_36_Latest_Date)~"Window open (Yet to be seen)",
         ...
         vist = replace(vist,vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32"),"ANC-36")) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,status) %>%
  ungroup() %>% distinct() #%>% view()
...
reten_anc_overall <- Bind_data %>% group_by(vist,status) %>% count() %>% ungroup() %>% group_by(vist) %>%
...
# reten_anc_overall %>% mutate(vist = fct_relevel(vist, "Enrollment","ANC-20","ANC-28","ANC-32","ANC-36")) %>%
#   ggplot(aes(x =vist,y = prop,fill =status)) +
...
#=================================================================
library(tidylog)
OverallReten_Data <- Data1 %>% drop_na(studyID) %>%  
  ...
  select(studyID,CON_DSSTDAT,FETAL_LOSS_DSDECOD,FETAL_LOSS_DSSTDAT,MAT_VISIT_MNH04,vist,ANC_OBSSTDAT,DELIV_DSSTDAT_INF1,CLOSE_DSSTDAT,visit_date) %>% 
  group_by(studyID,vist) %>% fill(DELIV_DSSTDAT_INF1) %>% ungroup() %>%
  full_join(Data1 %>% filter(vist == "Enrollment") %>% distinct(studyID,.keep_all = T) %>%
              select(studyID,CON_DSSTDAT, vist,SCRN_FAC_SPFY_OBSLOC,US_OHOSTDAT, US_GA_WKS_AGE_FTS1,US_GA_DAYS_AGE_FTS1,visit_date) %>% 
...
# To be removed
RM_ANC20 <- OverallReten_Data %>% filter(vist=="IPC") %>% anti_join(OverallReten_Data %>% filter(vist=="ANC-20") %>% select(studyID)) %>% 
  filter(DELIV_DSSTDAT_INF1<ANC_20_Earliest_Date|between(DELIV_DSSTDAT_INF1,ANC_20_Earliest_Date,ANC_20_Latest_Date)) %>% distinct(studyID,.keep_all = T) %>%
  full_join( OverallReten_Data %>% drop_na(CLOSE_DSSTDAT) %>% anti_join(OverallReten_Data %>% filter(vist=="ANC-20") %>% select(studyID)) %>% 
               filter(CLOSE_DSSTDAT<ANC_20_Earliest_Date|between(CLOSE_DSSTDAT,ANC_20_Earliest_Date,ANC_20_Latest_Date))%>% distinct(studyID)) %>% select(studyID)

RM_ANC28 <- OverallReten_Data %>% filter(vist=="IPC") %>% anti_join(OverallReten_Data %>% filter(vist=="ANC-28") %>% select(studyID)) %>% 
  filter(DELIV_DSSTDAT_INF1<ANC_28_Earliest_Date|between(DELIV_DSSTDAT_INF1,ANC_28_Earliest_Date,ANC_28_Latest_Date)) %>% distinct(studyID,.keep_all = T) %>%
  full_join(OverallReten_Data %>% drop_na(CLOSE_DSSTDAT) %>% anti_join(OverallReten_Data %>% filter(vist=="ANC-28") %>% select(studyID)) %>% 
              filter(CLOSE_DSSTDAT<ANC_28_Earliest_Date|between(CLOSE_DSSTDAT,ANC_28_Earliest_Date,ANC_28_Latest_Date))%>% distinct(studyID)) %>% select(studyID)

RM_ANC32 <- OverallReten_Data %>% filter(vist=="IPC") %>% anti_join(OverallReten_Data %>% filter(vist=="ANC-32") %>% select(studyID)) %>% 
  filter(DELIV_DSSTDAT_INF1<ANC_32_Earliest_Date|between(DELIV_DSSTDAT_INF1,ANC_32_Earliest_Date,ANC_32_Latest_Date)) %>% distinct(studyID,.keep_all = T) %>%
  full_join(OverallReten_Data %>% drop_na(CLOSE_DSSTDAT) %>% anti_join(OverallReten_Data %>% filter(vist=="ANC-32") %>% select(studyID)) %>% 
              filter(CLOSE_DSSTDAT<ANC_32_Earliest_Date|between(CLOSE_DSSTDAT,ANC_32_Earliest_Date,ANC_32_Latest_Date))%>% distinct(studyID)) %>% select(studyID)

RM_ANC36 <- OverallReten_Data %>% filter(vist=="IPC") %>% anti_join(OverallReten_Data %>% filter(vist=="ANC-36") %>% select(studyID)) %>% 
  filter(DELIV_DSSTDAT_INF1<ANC_36_Earliest_Date|between(DELIV_DSSTDAT_INF1,ANC_36_Earliest_Date,ANC_36_Latest_Date)) %>% distinct(studyID,.keep_all = T) %>%
  full_join(OverallReten_Data %>% drop_na(CLOSE_DSSTDAT) %>% anti_join(OverallReten_Data %>% filter(vist=="ANC-36") %>% select(studyID)) %>% 
              filter(CLOSE_DSSTDAT<ANC_36_Earliest_Date|between(CLOSE_DSSTDAT,ANC_36_Earliest_Date,ANC_36_Latest_Date))%>% distinct(studyID)) %>% select(studyID)

#Enrollment
ENROLMNTS <- OverallReten_Data %>% filter(vist == "Enrollment") %>%
  distinct(studyID,.keep_all = T) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,GA_days,CON_DSSTDAT,
         ANC_20_Earliest_Date,ANC_20_Actual_Date,ANC_20_Latest_Date,ANC_28_Earliest_Date,
...
ANC_20Rete_expec <- ENROLMNTS  %>% filter(current_Ga>=126) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,status, ANC_20_Earliest_Date,ANC_20_Actual_Date,ANC_20_Latest_Date) %>%
...
ANC_20_retent <- OverallReten_Data %>% filter(vist=="ANC-20") %>% distinct() %>%
  select(studyID,visit_date) %>% mutate(status = paste0("Retained")) %>%  distinct() #%>% view()

ANC20_merged <- ANC_20Rete_expec %>% full_join(ANC_20_retent) %>% group_by(studyID) %>%
  fill(SCRN_FAC_SPFY_OBSLOC,visit_date,Earliest_Date,Latest_Date,Actual_Date,.direction = "updown") %>%
  mutate(vist =paste0("ANC-20")) #%>% view()

# ANC 28
ANC_28Rete_expec <- ENROLMNTS  %>% filter(current_Ga>=182) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,status, ANC_28_Earliest_Date,ANC_28_Actual_Date,ANC_28_Latest_Date) %>%
...
ANC_28_retent <- OverallReten_Data %>% filter(vist=="ANC-28") %>% distinct() %>%
  select(studyID,visit_date) %>% mutate(status = paste0("Retained")) %>%  distinct()# %>% view()


ANC28_merged <- ANC_28Rete_expec %>% full_join(ANC_28_retent) %>% group_by(studyID) %>%
  fill(SCRN_FAC_SPFY_OBSLOC,visit_date,Earliest_Date,Latest_Date,Actual_Date,.direction = "updown") %>%
  mutate(vist =paste0("ANC-28"))


# ANC 32
ANC_32Rete_expec <- ENROLMNTS  %>% filter(current_Ga>=217) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,status, ANC_32_Earliest_Date,ANC_32_Actual_Date,ANC_32_Latest_Date) %>%
...
ANC_32_retent <- OverallReten_Data %>% filter(vist=="ANC-32") %>% distinct() %>%
  select(studyID,visit_date) %>% mutate(status = paste0("Retained")) %>%  distinct() #%>% view()


ANC32_merged <- ANC_32Rete_expec %>% full_join(ANC_32_retent) %>% group_by(studyID) %>%
  fill(SCRN_FAC_SPFY_OBSLOC,visit_date,Earliest_Date,Latest_Date,Actual_Date,.direction = "updown") %>%
  mutate(vist =paste0("ANC-32"))


# ANC 36
ANC_36Rete_expec <- ENROLMNTS  %>% filter(current_Ga>=238) %>%
  select(studyID,SCRN_FAC_SPFY_OBSLOC,status, ANC_36_Earliest_Date,ANC_36_Actual_Date,ANC_36_Latest_Date) %>%
...
ANC_36_retent <- OverallReten_Data %>% filter(vist=="ANC-36") %>% distinct() %>%
  select(studyID,visit_date) %>% mutate(status = paste0("Retained")) %>%  distinct() #%>% view()


ANC36_merged <- ANC_36Rete_expec %>% full_join(ANC_36_retent) %>% group_by(studyID) %>%
  fill(SCRN_FAC_SPFY_OBSLOC,visit_date,Earliest_Date,Latest_Date,Actual_Date,.direction = "updown") %>%
  mutate(vist =paste0("ANC-36"))
...
anc20_32 <- bind_rows(ANC20_merged,ANC28_merged,ANC32_merged,ANC36_merged) %>% 
  ...
  mutate(SCRN_FAC_SPFY_OBSLOC = replace(SCRN_FAC_SPFY_OBSLOC,SCRN_FAC_SPFY_OBSLOC=="Lumumba Sub County Hospital","Lumumba Sub-County\n Hospital"),
         SCRN_FAC_SPFY_OBSLOC = replace(SCRN_FAC_SPFY_OBSLOC,SCRN_FAC_SPFY_OBSLOC=="facility_4","Siaya County\n Referral Hospital")) #%>% View()

anc20_36 <- anc20_32 %>% filter(Latest_Date<=ymd("2024-03-31")) #%>% anti_join(three_cons_vists)

missed_visits <- anc20_36 %>% filter(!complete.cases(visit_date)) %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,Earliest_Date,Actual_Date,Latest_Date) %>% 
  left_join(Data %>% drop_na(studyID) %>% select(studyID,vist,MAT_VISIT_MNH04)) %>% group_by(studyID) %>% 
  mutate(index = row_number()) %>% ungroup() # %>% 

# more than three Missed visits

three_cons_vists <- missed_visits %>% filter((vist=="ANC-32" & index==3)|(vist=="ANC-36" & index==4)) %>% distinct(studyID) %>% 
 mutate(vist="ANC-36") #%>% View()
...
overall_reten <- anc20_32 %>% 
  ...
  group_by(vist) %>% count(Retention) %>% ungroup() %>% filter(Retention!="Missed visit") #%>% View()

overall_props <- overall_reten %>%
  pivot_wider(id_cols = "vist",names_from = "Retention",values_from = n ) %>%
...
overall_reten <- anc20_32 %>% filter(between(Actual_Date,ymd("2023-01-01"),ymd("2024-03-31"))) %>%
  #filter(between(Actual_Date,ymd(input$overalStart),ymd(input$overalEnd))) %>%
  group_by(SCRN_FAC_SPFY_OBSLOC,vist) %>% count(Retention)%>% ungroup() %>% filter(Retention!="Missed visit")

overall_props <- overall_reten %>%
  pivot_wider(id_cols = c("SCRN_FAC_SPFY_OBSLOC","vist"),names_from = "Retention",values_from = "n" ) %>%
  mutate(prop = (round(Retained/Expected,3)*100)) %>% select(SCRN_FAC_SPFY_OBSLOC,vist,prop) %>% 
  pivot_wider(id_cols = "SCRN_FAC_SPFY_OBSLOC",names_from = "vist",values_from = "prop") %>% 
  mutate(SCRN_FAC_SPFY_OBSLOC = replace(SCRN_FAC_SPFY_OBSLOC,SCRN_FAC_SPFY_OBSLOC=="facility_5","KCRH"),
         SCRN_FAC_SPFY_OBSLOC = replace(SCRN_FAC_SPFY_OBSLOC,SCRN_FAC_SPFY_OBSLOC=="facility_2","KUOYO"),
...
overall_reten <- anc20_32 %>% 
  ...
  group_by(vist) %>% count(Retention)%>% ungroup() %>% filter(Retention!="Missed visit")
...
overall_props %>% ggplot(aes(vist,prop))+
...
#   mutate(Retention = "Expected")
# 
# overall_reten %>% ggplot(aes(vist,n,fill=Retention))+
...
#   geom_text(data = overall_props, aes(x=vist,y=Expected,label=prop),vjust=-.8,size = 4,color="black")
...
#                               "Lumumba Sub-County\n Hospital" = "LUMUMBA",
#                               "Migosi Sub County Hospital" = "Migosi Sub \n County Hospital",
#                               "facility_7" = "Tingwangi Health \n Centre",
#                               "Kisumu Police Lines Dispensary" = "Kisumu Police \n Lines Dispensary"))
# 
# 
# overall_reten <- anc20_32 %>% filter(between(Actual_Date,ymd("2023-01-01"),ymd("2024-03-31"))) %>%
#   
#   group_by(SCRN_FAC_SPFY_OBSLOC) %>% filter(SCRN_FAC_SPFY_OBSLOC=="facility_5") #%>% View()
...
cum <- missed_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC) %>%
  bind_rows(anc20_32 %>% filter(Retention=="Missed visit") %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC)) %>% 
  group_by(vist) %>% count() %>% mutate(SCRN_FAC_SPFY_OBSLOC = "Cumulatively") # based on the ontime windows: for any query, remove late window visits

missed_vsts  %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC) %>% 
    bind_rows(anc20_32 %>% filter(Retention=="Missed visit") %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC)) %>% 
  group_by(vist) %>% count(SCRN_FAC_SPFY_OBSLOC) %>% bind_rows(cum) %>% 

ggplot(aes(area = n,fill=vist,
                           label = paste0(vist,"\n","(",n,")")))+
...
```{r}
IPCdta <- Data1 %>% drop_na(studyID) %>% filter(!vist%in%c("Non-scheduled visit","14")) %>% 
  mutate(month_visit = zoo::as.yearmon(US_EDD_BRTHDAT_FTS1)) %>% 
  mutate(Cal_EDD = if_else(vist == "Enrollment",US_EDD_BRTHDAT_FTS1,NA),
         IPC_status = if_else(vist == "IPC","IPC: Available",NA)) %>% 
  group_by(studyID) %>% fill(Cal_EDD,month_visit,IPC_status,.direction = "updown") %>% 
  mutate(IPC_status = if_else(is.na(IPC_status),"IPC: Missing/pending",IPC_status)) %>% 
  arrange(studyID,vist) %>% 
  mutate(rank = row_number(),max_rank = max(rank),
         last_seen = if_else(max_rank == rank,vist,NA)) %>% 
fill(last_seen,.direction = "updown") %>% ungroup() %>% 

distinct(studyID,.keep_all = T) %>% 
  filter(Cal_EDD <=ymd("2024-05-31")) %>% 
 # filter(US_EDD_BRTHDAT_FTS1 <=ymd("2024-03-31")) %>%
  select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,US_EDD_BRTHDAT_FTS1,Cal_EDD,month_visit,GA_today_weeks,IPC_status,last_seen) %>% 
  anti_join(Data1 %>% filter(complete.cases(CLOSE_DSSTDAT)) %>% 
  anti_join(Data1 %>% filter(complete.cases(CLOSE_DSSTDAT)) %>% filter(vist=="IPC") %>% select(studyID)) %>% distinct(studyID)) #%>% View()
...
#   select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,US_EDD_BRTHDAT_FTS1,Cal_EDD,month_visit,GA_today_weeks,IPC_status,last_seen) %>%
#   anti_join(Data1 %>% filter(complete.cases(CLOSE_DSSTDAT)) %>%
#   anti_join(Data1 %>% filter(complete.cases(CLOSE_DSSTDAT)) %>% filter(vist=="IPC") %>% select(studyID)) %>% distinct(studyID)) %>%
...
#-------------------------------------------------------------------------------------
# Fetal losses
ftl_loses <- Data1 %>% drop_na(studyID) %>%
    select(studyID,Ga_Enrollment,CON_DSSTDAT,INFANTID,vist,visit_date,Ga_visit_weeks,FETAL_LOSS_DSDECOD,FETAL_LOSS_DSSTDAT,
  ...
  mutate(stillbirth = if_else(is_ga_wk_20wks>=20,"Yes",as.character(NA)),
         ...
         stillbirth2 = if_else(BIRTH_DSTERM_INF2=="Fetal death","Yes",NA)) %>% group_by(studyID) %>% 
...
fetal_loss_less_20wks <- ftl_loses %>%  filter(miscarriage == "Yes") %>%  distinct(studyID,.keep_all = T) %>%
  anti_join( ftl_loses %>%  filter(vist == "IPC",stillbirth1 == "Yes") %>%  distinct(studyID)) 

# Fetal loss above 20weeks -- followed upto PNC-52
fetal_loss_above_20wks <- ftl_loses %>%  filter(vist == "IPC",stillbirth1 == "Yes") %>%  distinct(studyID)

# Neeonatal deaths - Followed upto PNC-52

infant_closeout <- read_rds(here("Data","MNH24_Infant_close_out.rds"))
Neon_deaths <- Data1 %>% drop_na(studyID) %>% group_by(studyID) %>% 
  ...
  select(studyID,vist,BIRTH_DSTERM_INF1,INF_VITAL_MNH11,INF_VITAL_MNH13,INF_VITAL_MNH14,INF_VITAL_MNH15) %>% 
  mutate(Neonatal_death = if_else(INF_VITAL_MNH11 == "Died"|INF_VITAL_MNH13 == "Died"|INF_VITAL_MNH14== "Died"|INF_VITAL_MNH15== "Died","Yes",NA)) %>% 
  filter(Neonatal_death == "Yes") %>% distinct(studyID) %>% full_join(infant_closeout %>% filter(studyID%in%c("KEARC01006","KEARC00102")) %>% distinct(studyID))


# making the IPC data
IPC_data <- Data1 %>% drop_na(studyID) %>%  filter(vist=="IPC") %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,DELIV_DSSTDAT_INF1,CLOSE_DSSTDAT) %>% 
  group_by(studyID) %>% fill(DELIV_DSSTDAT_INF1,.direction = "updown") %>% distinct(studyID,.keep_all = T) %>%   
...
pnc_0_ret <- IPC_data %>% filter(PNC_0_Latest_Date<today()) %>% 
  select(studyID,SCRN_FAC_SPFY_OBSLOC,Actual_Date = PNC_0_Actual_Date,latest_date = PNC_0_Latest_Date) %>% left_join(
    Data1 %>% drop_na(studyID) %>% select(studyID,vist,VISIT_OBSSTDAT_PNC_var,visit_date) %>% 
      group_by(studyID,vist) %>% fill(visit_date,VISIT_OBSSTDAT_PNC_var,.direction = "updown") %>% filter(vist=="PNC-0") %>% 
      distinct(studyID,.keep_all = T) %>% select(studyID,VISIT_OBSSTDAT_PNC_var,visit_date)) %>% rename(visit = vist) %>% 
  mutate(Retention = if_else(between(visit_date,Actual_Date,latest_date),"Retained","Missed visit"),
         Retention = replace(Retention, is.na(Retention),"Missed visit")) %>% mutate(vist = "PNC-0") %>% 
  anti_join(PNC_0_clsd) #%>% View()


# pnc 1 retention
pnc_1_ret <- IPC_data %>% filter(PNC_1_Latest_Date<today()) %>% 
  select(studyID,SCRN_FAC_SPFY_OBSLOC,Actual_Date = PNC_1_Actual_Date,latest_date = PNC_1_Latest_Date) %>% left_join(
    Data1 %>% drop_na(studyID) %>% select(studyID,vist,VISIT_OBSSTDAT_PNC_var,visit_date) %>% 
      group_by(studyID,vist) %>% fill(visit_date,VISIT_OBSSTDAT_PNC_var,.direction = "updown") %>% filter(vist=="PNC-1") %>% 
      distinct(studyID,.keep_all = T) %>% select(studyID,VISIT_OBSSTDAT_PNC_var,visit_date)) %>% rename(visit = vist) %>% 
  mutate(Retention = if_else(between(visit_date,Actual_Date,latest_date),"Retained","Missed visit"),
         Retention = replace(Retention, is.na(Retention),"Missed visit")) %>% mutate(vist = "PNC-1") %>% 
  anti_join(PNC_1_clsd) #%>% View()

# pnc 4 retention
pnc_4_ret <- IPC_data %>% filter(PNC_4_Latest_Date<today()) %>% 
  select(studyID,SCRN_FAC_SPFY_OBSLOC,Actual_Date = PNC_4_Actual_Date,latest_date = PNC_4_Latest_Date) %>% left_join(
    Data1 %>% drop_na(studyID) %>% select(studyID,vist,VISIT_OBSSTDAT_PNC_var,visit_date) %>% 
      group_by(studyID,vist) %>% fill(visit_date,VISIT_OBSSTDAT_PNC_var,.direction = "updown") %>% filter(vist=="PNC-4") %>% 
      distinct(studyID,.keep_all = T) %>% select(studyID,VISIT_OBSSTDAT_PNC_var,visit_date)) %>% rename(visit = vist) %>% 
  mutate(Retention = if_else(between(visit_date,Actual_Date,latest_date),"Retained","Missed visit"),
         Retention = replace(Retention, is.na(Retention),"Missed visit")) %>% mutate(vist = "PNC-4") %>% 
...
pnc_6_ret <- IPC_data %>% 
  ...
  select(studyID,SCRN_FAC_SPFY_OBSLOC,Actual_Date = PNC_6_Actual_Date,latest_date = PNC_6_Latest_Date) %>% left_join(
    Data1 %>% drop_na(studyID) %>% select(studyID,vist,VISIT_OBSSTDAT_PNC_var,visit_date) %>% 
      group_by(studyID,vist) %>% fill(visit_date,VISIT_OBSSTDAT_PNC_var,.direction = "updown") %>% filter(vist=="PNC-6") %>% 
      distinct(studyID,.keep_all = T) %>% select(studyID,VISIT_OBSSTDAT_PNC_var,visit_date)) %>% rename(visit = vist) %>% 
  mutate(Retention = if_else(between(visit_date,Actual_Date,latest_date),"Retained","Missed visit"),
         Retention = replace(Retention, is.na(Retention),"Missed visit")) %>% mutate(vist = "PNC-6") %>% 
   anti_join(PNC_6_clsd) #%>% View()

# pnc 26 retention
pnc_26_ret <- IPC_data %>% filter(PNC_26_Latest_Date<today()) %>% 
  select(studyID,SCRN_FAC_SPFY_OBSLOC,Actual_Date = PNC_26_Actual_Date,latest_date = PNC_26_Latest_Date) %>% #anti_join(fetal_loss_less_20wks) %>% 
  left_join(Data1 %>% drop_na(studyID) %>% select(studyID,vist,VISIT_OBSSTDAT_PNC_var,visit_date) %>% 
              group_by(studyID,vist) %>% fill(visit_date,VISIT_OBSSTDAT_PNC_var,.direction = "updown") %>% filter(vist=="PNC-26") %>% 
              distinct(studyID,.keep_all = T) %>% select(studyID,VISIT_OBSSTDAT_PNC_var,visit_date)) %>% rename(visit = vist) %>% 
  mutate(Retention = if_else(between(visit_date,Actual_Date,latest_date),"Retained","Missed visit"),
         Retention = replace(Retention, is.na(Retention),"Missed visit")) %>% mutate(vist = "PNC-26") %>% 
 anti_join(PNC_26_clsd) %>% anti_join(fetal_loss_less_20wks) #%>% View()

# pnc 52 retention
pnc_52_ret <- IPC_data %>% filter(PNC_52_Latest_Date<today()) %>% 
  select(studyID,SCRN_FAC_SPFY_OBSLOC,Actual_Date = PNC_52_Actual_Date,latest_date = PNC_52_Latest_Date) %>% #anti_join(fetal_loss_less_20wks) %>% 
  left_join(Data1 %>% drop_na(studyID) %>% select(studyID,vist,VISIT_OBSSTDAT_PNC_var,visit_date) %>% 
              group_by(studyID,vist) %>% fill(visit_date,VISIT_OBSSTDAT_PNC_var,.direction = "updown") %>% filter(vist=="PNC-52") %>% 
              distinct(studyID,.keep_all = T) %>% select(studyID,VISIT_OBSSTDAT_PNC_var,visit_date)) %>% rename(visit = vist) %>% 
  mutate(Retention = if_else(between(visit_date,Actual_Date,latest_date),"Retained","Missed visit"),
         Retention = replace(Retention, is.na(Retention),"Missed visit")) %>% mutate(vist = "PNC-52") %>% 
...
pnc_re <-  all_pnc0_52 %>% filter(between(Actual_Date,ymd("2023-01-01"),ymd("2024-03-31"))) %>% 
  mutate(SCRN_FAC_SPFY_OBSLOC = replace(SCRN_FAC_SPFY_OBSLOC,SCRN_FAC_SPFY_OBSLOC=="Lumumba Sub County Hospital","LUMUMBA"),
         SCRN_FAC_SPFY_OBSLOC = replace(SCRN_FAC_SPFY_OBSLOC,SCRN_FAC_SPFY_OBSLOC=="facility_4","SIAYA"),
         SCRN_FAC_SPFY_OBSLOC = replace(SCRN_FAC_SPFY_OBSLOC,SCRN_FAC_SPFY_OBSLOC=="facility_5","KCRH"),
         SCRN_FAC_SPFY_OBSLOC = replace(SCRN_FAC_SPFY_OBSLOC,SCRN_FAC_SPFY_OBSLOC=="facility_2","KUOYO")) %>% 
  group_by(vist,SCRN_FAC_SPFY_OBSLOC) %>% count(Retention) %>% 
  mutate(percent =round(n/sum(n)*100,1)) %>% filter(Retention == "Retained") %>%
  select(vist,SCRN_FAC_SPFY_OBSLOC,percent) %>% 
  pivot_wider(id_cols = SCRN_FAC_SPFY_OBSLOC,names_from = vist,values_from = percent) %>% tibble::as_tibble()
```
...
pnc_ontime <- all_pnc0_52 %>% filter(between(Actual_Date,ymd("2023-01-01"),ymd("2024-03-31"))) %>% 
  group_by(vist) %>% count(Retention) %>% 
  mutate(percent =round(n/sum(n)*100,1)) %>% filter(Retention == "Retained") 

ANCs_Prop %>% select(vist,percent = prop) %>% bind_rows(pnc_ontime %>% select(vist,percent)) %>% 
  mutate(vist = fct_relevel(vist,"ANC-20","ANC-28","ANC-32","ANC-36","PNC-0","PNC-1","PNC-4","PNC-6","PNC-26","PNC-52")) %>% 
  ggplot(aes(x=vist,y = percent)) +
...
pnc_ontime <- all_pnc0_52 %>% filter(between(Actual_Date,ymd("2023-01-01"),ymd("2024-03-31"))) %>% 
 ...
 pnc_ontime %>%    mutate(vist = factor(vist,levels = c("PNC-0","PNC-1","PNC-4","PNC-6","PNC-26","PNC-52")),
         vist = fct_relevel(vist,"PNC-0","PNC-1","PNC-4","PNC-6","PNC-26")) %>% 
  ggplot(aes(x=vist,y = percent,fill = vist))+
  geom_col(width = 0.3,alpha=.5,color="black") +
  #geom_text(aes(label = paste0(n,"(",percent,"%)")),vjust =-.6,size = 3.7,alpha=15) +
...
pnc_6_ret_late <- IPC_data %>% 
  filter(PNC_6_Latest_Date<today()) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,Actual_Date = PNC_6_Actual_Date,latest_date = PNC_6_Late_WindowE) %>% left_join(
    Data1 %>% drop_na(studyID) %>% select(studyID,vist,VISIT_OBSSTDAT_PNC_var,visit_date) %>% 
      group_by(studyID,vist) %>% fill(visit_date,VISIT_OBSSTDAT_PNC_var,.direction = "updown") %>% filter(vist=="PNC-6") %>% 
      distinct(studyID,.keep_all = T) %>% select(studyID,VISIT_OBSSTDAT_PNC_var,visit_date)) %>% rename(visit = vist) %>% 
  mutate(Retention = if_else(between(visit_date,Actual_Date,latest_date),"Retained","Missed visit"),
...
# map out pnc6
# pnc_6_ret_late <- IPC_data %>% 
#   filter(PNC_6_Latest_Date<today()) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,PNC_6_Actual_Date,PNC_6_Latest_Date,PNC_6_Late_WindowS, PNC_6_Late_WindowE) %>% left_join(
#     Data1 %>% drop_na(studyID) %>% select(studyID,vist,VISIT_OBSSTDAT_PNC_var,visit_date) %>% 
#       group_by(studyID,vist) %>% fill(visit_date,VISIT_OBSSTDAT_PNC_var,.direction = "updown") %>% filter(vist=="PNC-6") %>% 
#       distinct(studyID,.keep_all = T) %>% select(studyID,VISIT_OBSSTDAT_PNC_var,visit_date)) %>% rename(visit = vist) %>% 
...
pnc_26_ret_late <- IPC_data %>% filter(PNC_26_Latest_Date<today()) %>% 
  select(studyID,SCRN_FAC_SPFY_OBSLOC,Actual_Date = PNC_26_Actual_Date,latest_date = PNC_26_Late_WindowE,) %>% #anti_join(fetal_loss_less_20wks) %>% 
  left_join(Data1 %>% drop_na(studyID) %>% select(studyID,vist,VISIT_OBSSTDAT_PNC_var,visit_date) %>% 
              group_by(studyID,vist) %>% fill(visit_date,VISIT_OBSSTDAT_PNC_var,.direction = "updown") %>% filter(vist=="PNC-26") %>% 
              distinct(studyID,.keep_all = T) %>% select(studyID,VISIT_OBSSTDAT_PNC_var,visit_date)) %>% rename(visit = vist) %>% 
  mutate(Retention = if_else(between(visit_date,Actual_Date,latest_date),"Retained","Missed visit"),
...
# pnc_26_ret_late <- IPC_data %>% filter(PNC_26_Latest_Date<today()) %>% 
#   select(studyID,SCRN_FAC_SPFY_OBSLOC,Actual_Date = PNC_26_Actual_Date,PNC_26_Latest_Date,PNC_26_Late_WindowS,latest_date = PNC_26_Late_WindowE,) %>% #anti_join(fetal_loss_less_20wks) %>% 
#   left_join(Data1 %>% drop_na(studyID) %>% select(studyID,vist,VISIT_OBSSTDAT_PNC_var,visit_date) %>% 
#               group_by(studyID,vist) %>% fill(visit_date,VISIT_OBSSTDAT_PNC_var,.direction = "updown") %>% filter(vist=="PNC-26") %>% 
#               distinct(studyID,.keep_all = T) %>% select(studyID,VISIT_OBSSTDAT_PNC_var,visit_date)) %>% rename(visit = vist) %>% 
#   mutate(Retention = if_else(between(visit_date,Actual_Date,latest_date),"Retained","Missed visit"),
...
pnc_52_ret_late <- IPC_data %>% filter(PNC_52_Latest_Date<today()) %>% 
  select(studyID,SCRN_FAC_SPFY_OBSLOC,Actual_Date = PNC_52_Actual_Date,latest_date = PNC_52_Late_WindowE) %>% anti_join(fetal_loss_less_20wks) %>% 
  left_join(Data1 %>% drop_na(studyID) %>% select(studyID,vist,VISIT_OBSSTDAT_PNC_var,visit_date) %>% 
              group_by(studyID,vist) %>% fill(visit_date,VISIT_OBSSTDAT_PNC_var,.direction = "updown") %>% filter(vist=="PNC-52") %>% 
              distinct(studyID,.keep_all = T) %>% select(studyID,VISIT_OBSSTDAT_PNC_var,visit_date)) %>% rename(visit = vist) %>% 
  mutate(Retention = if_else(between(visit_date,Actual_Date,latest_date),"Retained","Missed visit"),
...
vsit_late <- all_pnc0_52_late %>% filter(between(Actual_Date,ymd("2023-01-01"),ymd("2024-03-31"))) %>% 
  ...
  group_by(vist) %>% count(Retention) %>% 
  mutate(percent =round(n/sum(n)*100,1)) %>% filter(Retention == "Retained") %>% 
  mutate(vist = factor(vist,levels = c("PNC-0","PNC-1","PNC-4","PNC-6","PNC-26","PNC-52")),
         vist = fct_relevel(vist,"PNC-0","PNC-1","PNC-4","PNC-6","PNC-26"))

ANCs_Prop %>% select(vist,percent = prop) %>% bind_rows(vsit_late %>% select(vist,percent)) %>% 
  mutate(vist = fct_relevel(vist,"ANC-20","ANC-28","ANC-32","ANC-36","PNC-0","PNC-1","PNC-4","PNC-6","PNC-26","PNC-52")) %>% 
  ggplot(aes(x=vist,y = percent)) +
  geom_col(width = 0.3,alpha=.6,colour="black",fill = c("darksalmon")) +
 # geom_text(aes(label = paste0(n,"(",percent,"%)")),vjust =-.6,size = 3.7,alpha=15) +
...
vsit_late <- all_pnc0_52_late %>% filter(between(Actual_Date,ymd("2023-01-01"),ymd("2024-03-31"))) %>% 
  ...
  vsit_late %>% ggplot(aes(x=vist,y = percent,fill = vist))+
  geom_col(width = 0.3,alpha=.5,color="black") +
...
cum_pnc <- all_pnc0_52_late  %>% filter(Retention=="Missed visit")  %>% filter(latest_date<ymd("2024-03-31")) %>% 
  group_by(vist) %>% count() %>% mutate(SCRN_FAC_SPFY_OBSLOC = "Cumulatively")
  
  all_pnc0_52_late %>%filter(latest_date<ymd("2024-03-31")) %>%  filter(Retention=="Missed visit") %>% 
  group_by(vist) %>% count(SCRN_FAC_SPFY_OBSLOC) %>% bind_rows(cum_pnc) %>% 
filter(SCRN_FAC_SPFY_OBSLOC!="facility_7") %>% 
...
MNH_forms <- CleanEasy::load_multipl_df("C:\\Users\\User\\OneDrive - Kenya Medical Research Institute\\study1 Harmonized\\rds ouput files",format = ".rds")
}else if(here() == "C:/Users/Edwin/OneDrive - Kenya Medical Research Institute/Rcloud"){
 MNH_forms <- CleanEasy::load_multipl_df("C:\\Users\\Edwin\\OneDrive - Kenya Medical Research Institute\\study1 Harmonized\\rds ouput files",format = ".rds")
}
 
#                          forms at enrolmemt
#-------------------------====================----------------------------------------------------------
# screening form
Enrol_forms <- Data1 %>% drop_na(studyID) %>% distinct(studyID, .keep_all = T) %>% select(SCRN_ID,studyID) %>%  left_join(
  MNH_forms$MNH00_prescreening %>% drop_na(SCRN_ID) %>% distinct(SCRN_ID) %>% mutate(form_type = "MNH00")) %>% 
  mutate(`Form name` = "MNH00:Pre-screening",
         status = if_else(!is.na(form_type),"filled","Not filled")) %>% 
  select(`Form name`,status) %>% 
  #Ultrasound
  bind_rows(Data1 %>% drop_na(studyID) %>% distinct(studyID, .keep_all = T) %>% select(SCRN_ID,studyID) %>% left_join(
    MNH_forms$MNH01_US %>% drop_na(SCRN_ID) %>% filter(vist == "Enrollment") %>%  distinct(SCRN_ID) %>% 
  ...
  # Enrolments
  bind_rows(Data1 %>% drop_na(studyID) %>% distinct(studyID, .keep_all = T) %>% select(SCRN_ID,studyID) %>% left_join(
    MNH_forms$MNH02_Enrolment_status %>% drop_na(studyID) %>%  distinct(studyID) %>% 
  ...
  #Socio demographics
  bind_rows(Data1 %>% drop_na(studyID) %>% distinct(studyID, .keep_all = T) %>% select(SCRN_ID,studyID) %>% left_join(
    MNH_forms$MNH03_Socio_Demographics_Data %>% drop_na(studyID) %>%  distinct(studyID) %>% 
  ...
  # ANC clinical status
  bind_rows(Data1 %>% drop_na(studyID) %>%  distinct(studyID, .keep_all = T) %>% select(SCRN_ID,studyID) %>% left_join(
    MNH_forms$MNH04_Clinical_Status_Data %>% drop_na(studyID) %>% filter(vist == "Enrollment") %>%  distinct(studyID) %>% 
  ...
  bind_rows(Data1 %>% drop_na(studyID) %>% distinct(studyID, .keep_all = T) %>% select(studyID) %>% mutate(studyID = as.character(studyID)) %>% left_join(
    MNH_forms$MNH05_MaternalAnthro_Data %>% drop_na(studyID) %>% distinct(studyID,vist) %>%  filter(vist == "Enrollment") %>% 
      select(studyID)%>% mutate(form_type = "MNH05") %>% mutate(studyID = as.character(studyID))) %>% 
  ...
  bind_rows(Data1 %>% drop_na(studyID) %>% distinct(studyID, .keep_all = T) %>% select(SCRN_ID,studyID) %>% left_join(
    MNH_forms$MNH06_Maternal_POC %>% drop_na(studyID) %>% filter(vist == "Enrollment") %>%  distinct(studyID) %>% 
  ...
  bind_rows(Data1 %>% drop_na(studyID) %>% distinct(studyID, .keep_all = T) %>% select(SCRN_ID,studyID) %>% left_join(
    MNH_forms$MNH07_Maternal_Spec_Coll_Data %>% drop_na(studyID) %>% filter(MAT_SPEC_COLLECT_VISIT == "Enrollment") %>%  distinct(studyID) %>% 
  ...
  bind_rows(Data1 %>% drop_na(studyID) %>% distinct(studyID, .keep_all = T) %>% select(SCRN_ID,studyID) %>% left_join(
    MNH_forms$MNH08_Maternal_Lab_Results %>% drop_na(studyID) %>% filter(VISIT_LBSTDAT == "Enrollment") %>%  distinct(studyID) %>% 
...
# ANC 20 FORMS
#----------------------------------------------------------------------------------------------------------------------
Data1_No_anc20 <- Data1 %>% drop_na(studyID) %>%  mutate(us_ga_enrol = US_GA_WKS_AGE_FTS1, us_ga_enrolmnt = replace(us_ga_enrol,vist!="Enrollment", NA)) %>% 
  group_by(studyID) %>% fill(us_ga_enrolmnt,.direction = "updown") %>% ungroup() %>% 
  mutate(anc20_check = if_else(vist == "ANC-20"& us_ga_enrolmnt>=18,NA,us_ga_enrolmnt)) %>% filter(complete.cases(anc20_check))


# ANC clinical status
ANc20_forms <- Data1_No_anc20  %>% distinct(studyID,vist, .keep_all = T) %>% 
  filter(vist == "ANC-20") %>%  select(studyID) %>% left_join(
    MNH_forms$MNH04_Clinical_Status_Data %>% drop_na(studyID) %>% filter(vist == "ANC-20") %>%  distinct(studyID) %>% 
      mutate(form_type = "MNH04")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                             `Form name` = "MNH04: ANC Clinical Status") %>%  select(`Form name`,status) %>% 
  # Anthropometrics
  bind_rows(Data1_No_anc20  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-20") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH05_MaternalAnthro_Data %>% drop_na(studyID) %>% filter(vist == "ANC-20") %>%  distinct(studyID) %>% 
  ...
  bind_rows(Data1_No_anc20  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-20") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH06_Maternal_POC %>% drop_na(studyID) %>% filter(vist == "ANC-20") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH06")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH06: Maternal POC Diagnostics") %>%  select(`Form name`,status)) %>% 
  # Maternal Specimen Collection
  bind_rows(Data1_No_anc20  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-20") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH07_Maternal_Spec_Coll_Data %>% drop_na(studyID) %>% filter(MAT_SPEC_COLLECT_VISIT == "ANC-20") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH07")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH07: Maternal specimen collection") %>%  select(`Form name`,status)) %>% 
  # MATERNAL lab results
  bind_rows(Data1_No_anc20  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-20") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH08_Maternal_Lab_Results %>% drop_na(studyID) %>% filter(VISIT_LBSTDAT == "ANC-20") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH08")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH08: Maternal Laboratory Results") %>%  select(`Form name`,status)) %>% 
  # Maternal depression form
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(MAT_VISIT_MNH04%in%c("Yes, visit conducted in person",NA)) %>% 
              filter(vist == "ANC-20") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH25_depression %>% drop_na(studyID) %>% mutate(vist = case_when(ANC_VISIT_N == 1~"Enrollment",
                                                                                              ANC_VISIT_N == 2~"ANC-20",
                                                                                              ANC_VISIT_N == 4~"ANC-32",
                                                                                              ANC_VISIT_N == 5~"ANC-36",
                                                                                              ANC_VISIT_N == 10~"PNC-6")) %>% 
                  filter(vist%in%c("ANC-20","Enrollment")) %>%  distinct(studyID) %>% 
...
# ANC clinical status
ANc28_forms <- Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
  filter(vist == "ANC-28") %>%  select(studyID) %>% left_join(
    MNH_forms$MNH04_Clinical_Status_Data %>% drop_na(studyID) %>% filter(vist == "ANC-28") %>%  distinct(studyID) %>% 
      mutate(form_type = "MNH04")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                             `Form name` = "MNH04: ANC Clinical Status") %>%  select(`Form name`,status) %>% 
  # Anthropometrics
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-28") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH05_MaternalAnthro_Data %>% drop_na(studyID) %>% filter(vist == "ANC-28") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH05")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH05: Maternal Anthropometrics") %>%  select(`Form name`,status)) %>% 
  # Maternal POC diagnostics
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-28") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH06_Maternal_POC %>% drop_na(studyID) %>% filter(vist == "ANC-28") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH06")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH06: Maternal POC Diagnostics") %>%  select(`Form name`,status)) %>% 
  # Maternal Specimen Collection
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-28") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH07_Maternal_Spec_Coll_Data %>% drop_na(studyID) %>% filter(MAT_SPEC_COLLECT_VISIT == "ANC-28") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH07")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH07: Maternal specimen collection") %>%  select(`Form name`,status)) %>% 
  # MATERNAL lab results
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-28") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH08_Maternal_Lab_Results %>% drop_na(studyID) %>% filter(VISIT_LBSTDAT == "ANC-28") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH08")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH08: Maternal Laboratory Results") %>%  select(`Form name`,status)) %>% 
  mutate(status = fct_relevel(status,"filled","Not filled")) %>% 
  tbl_summary(status,percent = "row", digits = list(everything()~c(0,1))) %>% 
...
#Ultrasound
ANC32_forms <- Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
  filter(vist == "ANC-32") %>%  select(studyID) %>% left_join(
    MNH_forms$MNH01_US %>% drop_na(studyID) %>% filter(vist == "ANC-32") %>%  distinct(studyID) %>% 
      mutate(form_type = "MNH01")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                             `Form name` = "MNH01: Ultrasound Form") %>%  select(`Form name`,status) %>%
  # ANC clinical status
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-32") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH04_Clinical_Status_Data %>% drop_na(studyID) %>% filter(vist == "ANC-32") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH04")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH04: ANC Clinical Status") %>%  select(`Form name`,status)) %>% 
  # Anthropometrics
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-32") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH05_MaternalAnthro_Data %>% drop_na(studyID) %>% filter(vist == "ANC-32") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH05")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH05: Maternal Anthropometrics") %>%  select(`Form name`,status)) %>% 
  # Maternal POC diagnostics
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-32") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH06_Maternal_POC %>% drop_na(studyID) %>% filter(vist == "ANC-32") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH06")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH06: Maternal POC Diagnostics") %>%  select(`Form name`,status)) %>% 
  # Maternal specimen collection
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-32") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH07_Maternal_Spec_Coll_Data %>% drop_na(studyID) %>% filter(MAT_SPEC_COLLECT_VISIT == "ANC-32") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH07")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH07: Maternal specimen collection") %>%  select(`Form name`,status)) %>% 
  # Maternal Lab Results
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-32") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH08_Maternal_Lab_Results %>% drop_na(studyID) %>% filter(VISIT_LBSTDAT == "ANC-32") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH08")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH08: Maternal Laboratory Results") %>%  select(`Form name`,status)) %>% 
  # Maternal depression form
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% filter(MAT_VISIT_MNH04%in%c("Yes, visit conducted in person",NA)) %>% 
              filter(vist == "ANC-32") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH25_depression %>% drop_na(studyID) %>% mutate(vist = case_when(ANC_VISIT_N == 1~"Enrollment",
                                                                                              ANC_VISIT_N == 2~"ANC-20",
                                                                                              ANC_VISIT_N == 4~"ANC-32",
                                                                                              ANC_VISIT_N == 5~"ANC-36",
                                                                                              ANC_VISIT_N == 10~"PNC-6")) %>% 
                  filter(vist%in%c("ANC-32")) %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH25")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH25: Maternal Depression Form") %>%  select(`Form name`,status)) %>%
  mutate(status = fct_relevel(status,"filled","Not filled")) %>% 
  tbl_summary(status,percent = "row", digits = list(everything()~c(0,1))) %>% 
...
# ANC clinical status
ANc36_forms <- Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
  filter(vist == "ANC-36") %>%  select(studyID) %>% left_join(
    MNH_forms$MNH04_Clinical_Status_Data %>% drop_na(studyID) %>% filter(vist == "ANC-36") %>%  distinct(studyID) %>% 
      mutate(form_type = "MNH04")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                             `Form name` = "MNH04: ANC Clinical Status") %>%  select(`Form name`,status) %>% 
  # Anthropometrics
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-36") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH05_MaternalAnthro_Data %>% drop_na(studyID) %>% filter(vist == "ANC-36") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH05")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH05: Maternal Anthropometrics") %>%  select(`Form name`,status)) %>% 
  # Maternal POC diagnostics
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-36") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH06_Maternal_POC %>% drop_na(studyID) %>% filter(vist == "ANC-36") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH06")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH06: Maternal POC Diagnostics") %>%  select(`Form name`,status)) %>% 
  # MATERNAL lab results
  bind_rows(Data1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "ANC-36") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH08_Maternal_Lab_Results %>% drop_na(studyID) %>% filter(VISIT_LBSTDAT == "ANC-36") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH08")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                         `Form name` = "MNH08: Maternal Laboratory Results") %>%  select(`Form name`,status)) %>% 
  mutate(status = fct_relevel(status,"filled","Not filled")) %>% 
  tbl_summary(status,percent = "row", digits = list(everything()~c(0,1))) %>% 
...
# fetal_loss_less_20wks <- Data1 %>% drop_na(studyID) %>%  filter(PRG_DSDECOD=="Fetal loss") %>% 
#   select(studyID,PRG_DSDECOD,FETAL_LOSS_DSSTDAT,CON_DSSTDAT,US_GA_WKS_AGE_FTS1_Enrol,US_GA_DAYS_AGE_FTS1_Enrol) %>% 
...
#   filter(ga_outcome_weeks<20) %>% select(studyID) %>% distinct()

fetal_losses <- Locked_study1 %>% filter(pregnancy_outcomes%in%c("Miscarriages","Stillbirths")) %>% distinct(studyID,INFANTID)
#-------------------------------------------------------------------------------------

#  # Anthropometrics
IPC_forms <- Data1  %>%   filter(vist == "IPC") %>% distinct(studyID,INFANTID, .keep_all = T) %>%select(studyID,INFANTID) %>%  left_join(
  MNH_forms$MNH05_MaternalAnthro_Data %>% drop_na(studyID) %>% filter(vist == "IPC") %>%  distinct(studyID) %>% 
    mutate(form_type = "MNH05")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
  ...
  bind_rows( Data1  %>%   filter(vist == "IPC") %>% distinct(studyID,INFANTID, .keep_all = T) %>%select(studyID,INFANTID) %>%  left_join(
    MNH_forms$MNH09_LabourAndDelivery_Data %>% drop_na(studyID) %>% filter(vist == "IPC") %>%  distinct(studyID) %>% 
  ...
  bind_rows( Data1  %>%   filter(vist == "IPC") %>% distinct(studyID,INFANTID, .keep_all = T) %>%select(studyID,INFANTID) %>% left_join(
    MNH_forms$MNH10_Labour_Delivery %>% drop_na(studyID) %>% filter(vist == "IPC") %>%  distinct(studyID) %>% 
      mutate(form_type = "MNH05")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
  ...
  bind_rows( Data1  %>%   filter(vist == "IPC") %>% distinct(studyID,INFANTID, .keep_all = T) %>%select(studyID,INFANTID) %>% 
               left_join(
                 MNH_forms$MNH11_Newborn_birth_outcome %>% drop_na(studyID) %>% filter(vist == "IPC") %>%  distinct(studyID,INFANTID) %>% 
                   mutate(form_type = "MNH05")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
...
tbl_merge(tbls = list(Enrol_forms,ANc20_forms,ANc28_forms,ANC32_forms,ANc36_forms,IPC_forms),
          tab_spanner = c(paste0("Enrollment N: ",Data1 %>% drop_na(studyID) %>% distinct(studyID, .keep_all = T) %>% nrow()),
                          paste0("ANC-20 N: ",Data1 %>% drop_na(studyID) %>%
                                    filter(MAT_VISIT_MNH04=="Yes, visit conducted in person") %>%  
                                   filter(vist == "ANC-20") %>% 
                                   distinct(studyID, .keep_all = T) %>% nrow()),
                          paste0("ANC-28 N: ",Data1 %>% drop_na(studyID) %>% 
                                    filter(MAT_VISIT_MNH04=="Yes, visit conducted in person") %>%  
                                   filter(vist == "ANC-28") %>% 
                                   distinct(studyID, .keep_all = T) %>% nrow()),
                          paste0("ANC-32 N: ",Data1 %>% drop_na(studyID) %>% 
                                   filter(MAT_VISIT_MNH04=="Yes, visit conducted in person") %>%  
                                   filter(vist == "ANC-32") %>% 
                                   distinct(studyID, .keep_all = T) %>% nrow()),
                          paste0("ANC-36 N: ",Data1 %>% drop_na(studyID) %>% 
                                    filter(MAT_VISIT_MNH04=="Yes, visit conducted in person") %>%  
                                   filter(vist == "ANC-36") %>% 
                                   distinct(studyID, .keep_all = T) %>% nrow()),
                          paste0("IPC N: ",Data1  %>%   filter(vist == "IPC") %>% 
                                   distinct(studyID,INFANTID, .keep_all = T) %>% select(studyID,INFANTID)  %>% nrow()))) %>% bold_levels() %>% 
...
Stillbirth_data <- Locked_study1 %>% drop_na(studyID) %>%
  select(studyID,Ga_Enrollment,CON_DSSTDAT,INFANTID,vist,visit_date,Ga_visit_weeks,FETAL_LOSS_DSDECOD,BIRTH_DSTERM_INF1,BIRTH_DSTERM_INF2,DELIV_DSSTDAT_INF1) %>% 
  mutate(FETAL_LOSS_DSDECOD = replace(FETAL_LOSS_DSDECOD, FETAL_LOSS_DSDECOD==77,NA)) %>% 
  mutate(is_ga_wk_20wks = if_else(BIRTH_DSTERM_INF1 == "Fetal death",(as.numeric(DELIV_DSSTDAT_INF1 - CON_DSSTDAT) + Ga_Enrollment)/7,NA)) %>% 
  mutate(stillbirth = if_else(is_ga_wk_20wks>=20,"Yes",as.character(NA)),
         miscarriage = if_else(FETAL_LOSS_DSDECOD == "Miscarriage/spontaneous abortion","Yes",as.character(NA)),
...
# Fetal losses
#fetal_losses <- Stillbirth_data %>% filter(miscarriage=="Yes"|stillbirth1=="Yes") %>% distinct(studyID,INFANTID) #%>% drop_na(INFANTID)
#fetal_losses <- Locked_study1 %>% filter(Miscarriages == "Yes"|StillBirths =="Yes") %>% distinct(studyID,INFANTID) #%>% select(studyID,INFANTID,Miscarriages,StillBirths) %>% View()
fetal_losses <- Locked_study1 %>% filter(pregnancy_outcomes%in%c("Miscarriages","Stillbirths")) %>% distinct(studyID,INFANTID)

infant_closeout <- read_rds(here("Data","MNH24_Infant_close_out.rds"))

# neondeaths <- Locked_study1 %>% drop_na(studyID) %>% group_by(studyID) %>% 
#   fill(BIRTH_DSTERM_INF1,INF_VITAL_MNH11,INF_VITAL_MNH13,INF_VITAL_MNH14,INF_VITAL_MNH15,.direction = "updown") %>% 
#   ungroup() %>% filter(BIRTH_DSTERM_INF1 == "Live birth") %>% 
#   select(studyID,vist,BIRTH_DSTERM_INF1,INF_VITAL_MNH11,INF_VITAL_MNH13,INF_VITAL_MNH14,INF_VITAL_MNH15) %>% 
#   mutate(Neonatal_death = if_else(INF_VITAL_MNH11 == "Died"|INF_VITAL_MNH13 == "Died"|INF_VITAL_MNH14== "Died"|INF_VITAL_MNH15== "Died","Yes",NA)) %>% 
#   filter(Neonatal_death == "Yes") %>% distinct(studyID) %>% full_join(infant_closeout %>% filter(studyID%in%c("KEARC01006","KEARC00102")) %>% select(studyID)) 

neondeaths <- Locked_study1 %>% filter(Neonatal_inf_death=="Yes") %>% distinct(studyID,INFANTID)
#============================+++++++===++++++++++================+====+++++++++++++==================+++++++++++++++++++=

#                              PNC 0 FORMS

# Maternal POC diagnostics
PNC_0_table <- Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist, .keep_all = T) %>% 
  filter(MAT_VISIT_MNH12%in%c("Yes, visit conducted in person",NA)) %>% 
  select(SCRN_ID,studyID,vist) %>% filter(vist == "PNC-0") %>% left_join(
    MNH_forms$MNH06_Maternal_POC %>% drop_na(studyID) %>% filter(vist == "PNC-0") %>%  
      mutate(form_type = "MNH06")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                              `Form name` = "MNH06: Maternal POC Diagnostics") %>%  select(`Form name`,status) %>%
  
  
  # Maternal PNC Clinical forms
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(MAT_VISIT_MNH12%in%c("Yes, visit conducted in person",NA)) %>% 
              #filter(MAT_VISIT_MNH12%in%c("Yes, visit conducted in person",NA)) %>% 
              select(SCRN_ID,studyID,vist) %>% filter(vist == "PNC-0") %>% left_join(
                MNH_forms$MNH12_Maternal_PNC_clinical %>% drop_na(studyID) %>% filter(vist == "PNC-0") %>%  
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              filter(MAT_VISIT_MNH12%in%c("Yes, visit conducted in person",NA)) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-0") %>% anti_join(fetal_losses) %>% anti_join(as.data.frame(neondeaths)) %>%   left_join(
                MNH_forms$MNH13_Clinical_status %>% drop_na(studyID) %>% filter(vist == "PNC-0") %>%  
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-0") %>% anti_join(fetal_losses)%>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH14_POC_Diagnostics %>% drop_na(studyID) %>% filter(POC_VISIT == "PNC-0") %>%  
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-0") %>% anti_join(fetal_losses)%>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH15_infant_vaccination %>% drop_na(studyID) %>% filter(vist == "PNC-0") %>%  
...
# Maternal POC diagnostics
PNC_1_table <- Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist, .keep_all = T) %>% 
  select(SCRN_ID,studyID,vist) %>% filter(vist == "PNC-1") %>% left_join(
    MNH_forms$MNH06_Maternal_POC %>% drop_na(studyID) %>% filter(vist == "PNC-1") %>%  
      mutate(form_type = "MNH06")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                              `Form name` = "MNH06: Maternal POC Diagnostics") %>%  select(`Form name`,status) %>% 
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist) %>% filter(vist == "PNC-1") %>% left_join(
                MNH_forms$MNH12_Maternal_PNC_clinical %>% drop_na(studyID) %>% filter(vist == "PNC-1") %>%  
                  mutate(form_type = "MNH12")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH12: Maternal PNC Clinical Status") %>%  select(`Form name`,status)) %>% 
  ...
  # Infant PNC Clinicl status -- remove fetal losses
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-1") %>% anti_join(fetal_losses) %>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH13_Clinical_status %>% drop_na(studyID) %>% filter(vist == "PNC-1") %>%  
                  mutate(form_type = "MNH13")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH13: Infant PNC Clinical Status") %>%  select(`Form name`,status)) %>% 
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-1") %>% anti_join(fetal_losses)%>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH14_POC_Diagnostics %>% drop_na(studyID) %>% filter(POC_VISIT == "PNC-1") %>%  
                  mutate(form_type = "MNH14")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH14: Infant POc Diagnostics") %>%  select(`Form name`,status)) %>% 
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-1") %>% anti_join(fetal_losses)%>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH15_infant_vaccination %>% drop_na(studyID) %>% filter(vist == "PNC-1") %>%  
                  mutate(form_type = "MNH15")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH15: Infant Vaccination Form") %>%  select(`Form name`,status)) %>% 
  mutate(status = fct_relevel(status,"filled","Not filled")) %>% 
  tbl_summary(by = status,percent = "row", digits = list(everything()~c(0,1)),statistic = all_categorical()~"{n} ({p}%)") %>% 
...
# Maternal POC diagnostics
PNC_4_table <- Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist, .keep_all = T) %>% 
  select(SCRN_ID,studyID,vist) %>% filter(vist == "PNC-4") %>% left_join(
    MNH_forms$MNH06_Maternal_POC %>% drop_na(studyID) %>% filter(vist == "PNC-4") %>%  
      mutate(form_type = "MNH06")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                              `Form name` = "MNH06: Maternal POC Diagnostics") %>%  select(`Form name`,status) %>% 
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist) %>% filter(vist == "PNC-4") %>% left_join(
                MNH_forms$MNH12_Maternal_PNC_clinical %>% drop_na(studyID) %>% filter(vist == "PNC-4") %>%  
                  mutate(form_type = "MNH12")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH12: Maternal PNC Clinical Status") %>%  select(`Form name`,status)) %>% 
  ...
  # Infant PNC Clinicl status -- remove fetal losses
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-4") %>% anti_join(fetal_losses) %>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH13_Clinical_status %>% drop_na(studyID) %>% filter(vist == "PNC-4") %>%  
                  mutate(form_type = "MNH13")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH13: Infant PNC Clinical Status") %>%  select(`Form name`,status)) %>% 
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-4") %>% anti_join(fetal_losses)%>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH14_POC_Diagnostics %>% drop_na(studyID) %>% filter(POC_VISIT == "PNC-4") %>%  
                  mutate(form_type = "MNH14")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH14: Infant POc Diagnostics") %>%  select(`Form name`,status)) %>% 
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-4") %>% anti_join(fetal_losses)%>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH15_infant_vaccination %>% drop_na(studyID) %>% filter(vist == "PNC-4") %>%  
                  mutate(form_type = "MNH15")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH15: Infant Vaccination Form") %>%  select(`Form name`,status)) %>% 
  mutate(status = fct_relevel(status,"filled","Not filled")) %>% 
  tbl_summary(by = status,percent = "row", digits = list(everything()~c(0,1)),statistic = all_categorical()~"{n} ({p}%)") %>% 
...
# Maternal POC diagnostics
PNC_26_table <- Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist, .keep_all = T) %>% 
  select(SCRN_ID,studyID,vist) %>% filter(vist == "PNC-26") %>% left_join(
    MNH_forms$MNH06_Maternal_POC %>% drop_na(studyID) %>% filter(vist == "PNC-26") %>%  
      mutate(form_type = "MNH06")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                              `Form name` = "MNH06: Maternal POC Diagnostics") %>%  select(`Form name`,status) %>% 
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist) %>% filter(vist == "PNC-26") %>% left_join(
                MNH_forms$MNH12_Maternal_PNC_clinical %>% drop_na(studyID) %>% filter(vist == "PNC-26") %>%  
                  mutate(form_type = "MNH12")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH12: Maternal PNC Clinical Status") %>%  select(`Form name`,status)) %>% 
  ...
  # Infant PNC Clinicl status -- remove fetal losses
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-26") %>% anti_join(fetal_losses) %>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH13_Clinical_status %>% drop_na(studyID) %>% filter(vist == "PNC-26") %>%  
                  mutate(form_type = "MNH13")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH13: Infant PNC Clinical Status") %>%  select(`Form name`,status)) %>% 
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-26") %>% anti_join(fetal_losses)%>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH14_POC_Diagnostics %>% drop_na(studyID) %>% filter(POC_VISIT == "PNC-26") %>%  
                  mutate(form_type = "MNH14")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH14: Infant POc Diagnostics") %>%  select(`Form name`,status)) %>% 
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-26") %>% anti_join(fetal_losses)%>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH15_infant_vaccination %>% drop_na(studyID) %>% filter(vist == "PNC-26") %>%  
                  mutate(form_type = "MNH15")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH15: Infant Vaccination Form") %>%  select(`Form name`,status)) %>% 
  # Anthropometrics
  bind_rows(Locked_study1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "PNC-26") %>%  select(studyID) %>%  left_join(
                MNH_forms$MNH05_MaternalAnthro_Data %>% drop_na(studyID) %>% filter(vist == "PNC-26") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH05")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH05: Maternal Anthropometrics") %>%  select(`Form name`,status))%>% 
  ...
  bind_rows(Locked_study1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "PNC-26") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH07_Maternal_Spec_Coll_Data %>% drop_na(studyID) %>% filter(MAT_SPEC_COLLECT_VISIT == "PNC-26") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH07")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH07: Maternal specimen collection") %>%  select(`Form name`,status)) %>% 
  # Maternal Lab Results
  # bind_rows(Locked_study1  %>% distinct(studyID,vist, .keep_all = T) %>%
  #             filter(vist == "PNC-26") %>%  select(studyID) %>% left_join(
  #               MNH_forms$MNH08_Maternal_Lab_Results %>% drop_na(studyID) %>% filter(VISIT_LBSTDAT == "PNC-26") %>%  distinct(studyID) %>%
  #                 mutate(form_type = "MNH08")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
  #                                                        `Form name` = "MNH08: Maternal Laboratory Results") %>%  select(`Form name`,status)) %>%
...
# Maternal POC diagnostics
PNC_6_table <- Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist, .keep_all = T) %>% 
  select(SCRN_ID,studyID,vist) %>% filter(vist == "PNC-6") %>% left_join(
    MNH_forms$MNH06_Maternal_POC %>% drop_na(studyID) %>% filter(vist == "PNC-6") %>%  
      mutate(form_type = "MNH06")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                              `Form name` = "MNH06: Maternal POC Diagnostics") %>%  select(`Form name`,status) %>% 
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist) %>% filter(vist == "PNC-6") %>% left_join(
                MNH_forms$MNH12_Maternal_PNC_clinical %>% drop_na(studyID) %>% filter(vist == "PNC-6") %>%  
                  mutate(form_type = "MNH12")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH12: Maternal PNC Clinical Status") %>%  select(`Form name`,status)) %>% 
  ...
  # Infant PNC Clinicl status -- remove fetal losses
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist) %>% filter(vist == "PNC-6") %>% anti_join(fetal_losses) %>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH13_Clinical_status %>% drop_na(studyID) %>% filter(vist == "PNC-6") %>%  
                  mutate(form_type = "MNH13")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH13: Infant PNC Clinical Status") %>%  select(`Form name`,status)) %>% 
  
  # Mnh14 Infant POc Diagnostics
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-6") %>% anti_join(fetal_losses) %>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH14_POC_Diagnostics %>% drop_na(studyID) %>% filter(POC_VISIT == "PNC-6") %>%  
                  mutate(form_type = "MNH14")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH14: Infant POc Diagnostics") %>%  select(`Form name`,status)) %>% 
  ...
  bind_rows(Locked_study1 %>% drop_na(studyID) %>% distinct(studyID,vist,INFANTID, .keep_all = T) %>% 
              select(SCRN_ID,studyID,vist,INFANTID) %>% filter(vist == "PNC-6") %>% anti_join(fetal_losses) %>% anti_join(as.data.frame(neondeaths)) %>% left_join(
                MNH_forms$MNH15_infant_vaccination %>% drop_na(studyID) %>% filter(vist == "PNC-6") %>%  
                  mutate(form_type = "MNH15")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH15: Infant Vaccination Form") %>%  select(`Form name`,status)) %>% 
  ...
  # Anthropometrics
  bind_rows(Locked_study1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "PNC-6") %>%  select(studyID) %>%  left_join(
                MNH_forms$MNH05_MaternalAnthro_Data %>% drop_na(studyID) %>% filter(vist == "PNC-6") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH05")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH05: Maternal Anthropometrics") %>%  select(`Form name`,status))%>% 
  ...
  bind_rows(Locked_study1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "PNC-6") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH07_Maternal_Spec_Coll_Data %>% drop_na(studyID) %>% filter(MAT_SPEC_COLLECT_VISIT == "PNC-6") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH07")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH07: Maternal specimen collection") %>%  select(`Form name`,status)) %>% 
  # Maternal Lab Results
  bind_rows(Locked_study1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "PNC-6") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH08_Maternal_Lab_Results %>% drop_na(studyID) %>% filter(VISIT_LBSTDAT == "PNC-6") %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH08")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH08: Maternal Laboratory Results") %>%  select(`Form name`,status)) %>% 
  # Maternal depression form
  bind_rows(Locked_study1  %>% distinct(studyID,vist, .keep_all = T) %>% 
              filter(vist == "PNC-6") %>%  select(studyID) %>% left_join(
                MNH_forms$MNH25_depression %>% drop_na(studyID) %>% mutate(vist = case_when(ANC_VISIT_N == 1~"Enrollment",
                                                                                                ...
                                                                                                ANC_VISIT_N == 5~"ANC-36",
                                                                                                ANC_VISIT_N == 10~"PNC-6")) %>% 
                  filter(vist%in%c("PNC-6")) %>%  distinct(studyID) %>% 
                  mutate(form_type = "MNH25")) %>% mutate(status = if_else(!is.na(form_type),"filled","Not filled"),
                                                          `Form name` = "MNH25: Maternal Depression Form") %>%  select(`Form name`,status)) %>%
  mutate(status = fct_relevel(status,"filled","Not filled")) %>% 
  tbl_summary(status,percent = "row", digits = list(everything()~c(0,1))) %>% 
...
tbl_merge(tbls = list(PNC_0_table,PNC_1_table,PNC_4_table,PNC_6_table,PNC_26_table),
          tab_spanner = c(
            paste0("PNC-0 N: ",Locked_study1 %>% drop_na(studyID) %>% filter(vist == "PNC-0") %>%
                     distinct(studyID, .keep_all = T) %>% nrow()),
            paste0("PNC-1 N: ",Locked_study1 %>% drop_na(studyID) %>% filter(vist == "PNC-1") %>%
                     distinct(studyID, .keep_all = T) %>% nrow()),
            paste0("PNC-4 N: ",Locked_study1 %>% drop_na(studyID) %>% filter(vist == "PNC-4") %>%
                     distinct(studyID, .keep_all = T) %>% nrow()),
            paste0("PNC-6 N: ",Locked_study1 %>% drop_na(studyID) %>% filter(vist == "PNC-6") %>%
                     distinct(studyID, .keep_all = T) %>% nrow()),
            paste0("PNC-26 N: ",Locked_study1 %>% drop_na(studyID) %>% filter(vist == "PNC-26") %>%
                     distinct(studyID, .keep_all = T) %>% nrow()))) %>% bold_levels() %>%
  as_gt() %>% opt_table_lines(extent = "all") %>% opt_table_outline() %>%
  tab_options(table.background.color = "white") %>%
  #tab_options(data_row.padding = px(10)) %>%
...
Sample_Data <-  Data1 %>% filter(studyID!="KEARC00300") %>%  filter(complete.cases(studyID), vist == "Enrollment") %>%
  select(studyID,vist, CON_DSSTDAT, SCRN_FAC_SPFY_OBSLOC, MAT_BLOOD_SPEC_1, MAT_BLOOD_SPEC_COUNT_1,
...
Sample_Data %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist, MAT_BLOOD_SPEC_COUNT_1) %>% distinct(studyID,.keep_all = T) %>%
  mutate(Result = if_else(!is.na(MAT_BLOOD_SPEC_COUNT_1),"Result Available", "Missing result")) %>% #view()
  tabyl(Result) %>% mutate(Sample = paste0("EDTA tubes"),
                           Percent = round(percent*100,1)) %>%
  filter(Result == "Result Available") %>% select(Result,Sample, n , Percent) %>%
  # 
  #       rbind(Sample_Data %>% filter(complete.cases(studyID)) %>% select(studyID, MAT_BLOOD_SPEC_COUNT_2) %>% distinct(studyID,.keep_all = T) %>%
  ...
  rbind(Sample_Data %>% filter(complete.cases(studyID)) %>% select(studyID, SCRN_FAC_SPFY_OBSLOC,vist, MAT_BLOOD_SPEC_COUNT_3) %>% distinct(studyID,.keep_all = T) %>%
          mutate(Result = if_else(!is.na(MAT_BLOOD_SPEC_COUNT_3),"Result Available", "Missing result"))  %>% #view()
          tabyl(Result) %>% mutate(Sample = paste0("Serum separator tubes"),
                                   Percent = round(percent*100,1)) %>%
          filter(Result == "Result Available") %>% select(Result,Sample, n , Percent)) %>%
  
  rbind(Sample_Data %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist, MAT_BLOOD_SPEC_COUNT_4) %>% distinct(studyID,.keep_all = T) %>%
          mutate(Result = if_else(!is.na(MAT_BLOOD_SPEC_COUNT_4),"Result Available", "Missing result")) %>% #View()
          tabyl(Result) %>% mutate(Sample = paste0("Trace-element-free polyethylene evacuated tubes"),
                                   Percent = round(percent*100,1)) %>%
          filter(Result == "Result Available") %>% select(Result,Sample, n , Percent)) %>%
  rbind(Sample_Data %>% filter(complete.cases(studyID)) %>% select(studyID, SCRN_FAC_SPFY_OBSLOC,vist, MAT_BLOOD_SPEC_COUNT_5) %>% distinct(studyID,.keep_all = T) %>%
          mutate(Result = if_else(!is.na(MAT_BLOOD_SPEC_COUNT_5),"Result Available", "Missing result")) %>% #View()
          tabyl(Result) %>% mutate(Sample = paste0("Lithium Heparin tubes"),
                                   Percent = round(percent*100,1)) %>%
          filter(Result == "Result Available") %>% select(Result,Sample, n , Percent)) %>%
  
  rbind(Sample_Data %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist, MAT_BLOOD_SPEC_COUNT_6) %>% distinct(studyID,.keep_all = T) %>%
          mutate(Result = if_else(!is.na(MAT_BLOOD_SPEC_COUNT_6),"Result Available", "Missing result")) %>% 
          tabyl(Result) %>% mutate(Sample = paste0("Fluoride tube"),
                                   Percent = round(percent*100,1)) %>%
          filter(Result == "Result Available") %>% select(Result,Sample, n , Percent)) %>%
  
  rbind(Sample_Data %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist, MAT_URINE_COLLECT_COUNT) %>% distinct(studyID,.keep_all = T) %>%
          mutate(Result = if_else(!is.na(MAT_URINE_COLLECT_COUNT),"Result Available", "Missing result")) %>% #view()
          tabyl(Result) %>% mutate(Sample = paste0("urine containers"),
                                   Percent = round(percent*100,1)) %>%
          filter(Result == "Result Available") %>% select(Result,Sample, n , Percent)) %>%
  
  rbind(Sample_Data %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist, MAT_TB_SYM_TEST, MAT_TB_SPEC_COLLECT_COUNT) %>% distinct(studyID,.keep_all = T) %>%
          filter(MAT_TB_SYM_TEST == "Yes") %>%
          mutate(Result = if_else(!is.na(MAT_TB_SPEC_COLLECT_COUNT),"Result Available", "Missing result")) %>% #view()
          tabyl(Result) %>% mutate(Sample = paste0("sputum specimen - TB"),
                                   Percent = round(percent*100,1)) %>%
  ...
  rbind(Sample_Data %>% filter(complete.cases(studyID)) %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,vist,HELM_LBORRES, MAT_HELM_SPEC_COLLECT_COUNT) %>% distinct(studyID,.keep_all = T) %>%
...
# cbc
cbc_na <- Data1 %>% filter(complete.cases(studyID)) %>% filter(vist=="Enrollment") %>% #filter(studyID!="KEARC00300") %>% 
  select(studyID, CON_DSSTDAT,ANC_OBSSTDAT,visit_date, SCRN_FAC_SPFY_OBSLOC, vist, contains("CBC")) %>%
...
# select(studyID, CON_DSSTDAT,ANC_OBSSTDAT,visit_date, SCRN_FAC_SPFY_OBSLOC, vist, CBC_HB_LBORRES, CBC_HCT_LBORRES, CBC_WBC_LBORRES, CBC_NEU_PCT_LBORRES,
...
# CBC_HB_LBORRES
CBC_HB <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_HB_LBORRES) %>% filter(is.na(CBC_HB_LBORRES)) %>% nrow()
# CBC_HCT_LBORRES
CBC_HCT <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_HCT_LBORRES) %>% filter(is.na(CBC_HCT_LBORRES)) %>% nrow()
# CBC_WBC_LBORRES
CBC_WBC <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_WBC_LBORRES) %>% filter(is.na(CBC_WBC_LBORRES)) %>% nrow()
# CBC_NEU_PCT_LBORRES
CBC_NEU_PCT <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_NEU_PCT_LBORRES) %>% filter(is.na(CBC_NEU_PCT_LBORRES)) %>% nrow()
# CBC_NEU_FCC_LBORRES
CBC_NEU_FCC <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_NEU_FCC_LBORRES) %>% filter(is.na(CBC_NEU_FCC_LBORRES)) %>% nrow()
# CBC_LYMPH_PCT_LBORRES
CBC_LYMPH_PCT <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_LYMPH_PCT_LBORRES) %>% filter(is.na(CBC_LYMPH_PCT_LBORRES)) %>% nrow()
# CBC_LYMPH_FCC_LBORRES
CBC_LYMPH_FCC <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_LYMPH_FCC_LBORRES) %>% filter(is.na(CBC_LYMPH_FCC_LBORRES)) %>% nrow()
# CBC_ERYTH_MM_LBORRES
CBC_ERYTH_MM <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_ERYTH_MM_LBORRES) %>% filter(is.na(CBC_ERYTH_MM_LBORRES)) %>% nrow()
# CBC_MCV_LBORRES
CBC_MCV <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_MCV_LBORRES) %>% filter(is.na(CBC_MCV_LBORRES)) %>% nrow()
# CBC_MCH_LBORRES
CBC_MCH <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_MCH_LBORRES) %>% filter(is.na(CBC_MCH_LBORRES)) %>% nrow()
# CBC_MCHC_GDL_LBORRES
CBC_MCHC_GDL <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_MCHC_GDL_LBORRES) %>% filter(is.na(CBC_MCHC_GDL_LBORRES)) %>% nrow()
# CBC_PLATE_LBORRES
CBC_PLATE <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_PLATE_LBORRES) %>% filter(is.na(CBC_PLATE_LBORRES)) %>% nrow()
# CBC_MONO_PCT_LBORRES
CBC_MONO_PCT <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_MONO_PCT_LBORRES) %>% filter(is.na(CBC_MONO_PCT_LBORRES)) %>% nrow()
# CBC_MONO_FCC_LBORRES
CBC_MONO_FCC <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_MONO_FCC_LBORRES) %>% filter(is.na(CBC_MONO_FCC_LBORRES)) %>% nrow()
# CBC_EOS_PCT_LBORRES
CBC_EOS_PCT <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_EOS_PCT_LBORRES) %>% filter(is.na(CBC_EOS_PCT_LBORRES)) %>% nrow()
# CBC_EOS_FCC_LBORRES
CBC_EOS_FCC <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_EOS_FCC_LBORRES) %>% filter(is.na(CBC_EOS_FCC_LBORRES)) %>% nrow()
# CBC_RDW_PCT_LBORRES
CBC_RDW_PCT <- cbc_na %>% select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_RDW_PCT_LBORRES) %>% filter(is.na(CBC_RDW_PCT_LBORRES)) %>% nrow()
# CBC_PDW_CT_LBORRES
CBC_PDW_CT <- cbc_na %>% #filter(CBC_LBPERF_18%in%c("Yes",NA)) %>%  
  select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_LBPERF_18,CBC_PDW_CT_LBORRES) %>% filter(is.na(CBC_PDW_CT_LBORRES)) %>% nrow()
# CBC_PCT_PCT_LBORRES
CBC_PCT_PCT <- cbc_na %>% #filter(CBC_LBPERF_19%in%c("Yes",NA)) %>%  
  select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_LBPERF_19,CBC_PCT_PCT_LBORRES) %>% filter(is.na(CBC_PCT_PCT_LBORRES)) %>% nrow()
...
cbc20_rmve <- Data1 %>% drop_na(studyID) %>% 
  select(studyID,vist,US_GA_WKS_AGE_FTS1,US_GA_DAYS_AGE_FTS1) %>%
  mutate(Ga_weeks = US_GA_WKS_AGE_FTS1 +
           (US_GA_DAYS_AGE_FTS1/7)) %>% filter(Ga_weeks>=18.0 & Ga_weeks<20.0) %>% filter(vist=="ANC-20") %>% 
  select(studyID,vist) 

cbc_na_vsts <- Data1 %>% filter(complete.cases(studyID)) %>% anti_join(cbc20_rmve) %>% 
 filter(CON_DSSTDAT>=ymd("2023-04-03")) |> 
    filter(complete.cases(studyID)) %>% filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-36","PNC-6")) %>% #filter(studyID!="KEARC00300") |> 
    #filter(MAT_VISIT_MNH04%in%c("Yes, visit conducted in person",NA)) %>% filter(MAT_VISIT_MNH12%in%c("Yes, visit conducted in person",NA)) %>% 
    select(studyID, CON_DSSTDAT,MAT_VISIT_MNH04,MAT_VISIT_MNH12,ANC_OBSSTDAT,visit_date, SCRN_FAC_SPFY_OBSLOC, vist, contains("CBC")) %>%
    mutate(CBC_HB_LBORRES = if_else(CBC_LBPERF_1 == "No" & CBC_HB_LBORRES%in%c(0,-7),as.double(NA), CBC_HB_LBORRES),
           CBC_HCT_LBORRES = if_else(CBC_LBPERF_2 == "No" & CBC_HCT_LBORRES%in%c(0,-7),as.double(NA), CBC_HCT_LBORRES),
           CBC_WBC_LBORRES = if_else(CBC_LBPERF_3 == "No" & CBC_WBC_LBORRES%in%c(0,-7),as.double(NA), CBC_WBC_LBORRES),
           CBC_NEU_PCT_LBORRES = if_else(CBC_LBPERF_4 == "No" & CBC_NEU_PCT_LBORRES%in%c(0,-7),as.double(NA), CBC_NEU_PCT_LBORRES),
 ...
 # cbc_na %>% filter(vist!="Enrollment") %>% 
 #    select(studyID,ANC_OBSSTDAT,SCRN_FAC_SPFY_OBSLOC,vist,MAT_VISIT_MNH04,MAT_VISIT_MNH12,contains("LBORRES")) %>% filter(is.na(CBC_HB_LBORRES)) %>% View()
 #   group_by()


# CBC_HB_LBORRES
CBC_HB_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_HB_LBORRES) %>% filter(is.na(CBC_HB_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_HB")
# CBC_HCT_LBORRES
CBC_HCT_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_HCT_LBORRES) %>% filter(is.na(CBC_HCT_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_HCT")
# CBC_WBC_LBORRES
CBC_WBC_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_WBC_LBORRES) %>% filter(is.na(CBC_WBC_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_WBC")
# CBC_NEU_PCT_LBORRES
CBC_NEU_PCT_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_NEU_PCT_LBORRES) %>% filter(is.na(CBC_NEU_PCT_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_NEU_PCT")
# CBC_NEU_FCC_LBORRES
CBC_NEU_FCC_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_NEU_FCC_LBORRES) %>% filter(is.na(CBC_NEU_FCC_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_NEU_FCC")
# CBC_LYMPH_PCT_LBORRES
CBC_LYMPH_PCT_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_LYMPH_PCT_LBORRES) %>% filter(is.na(CBC_LYMPH_PCT_LBORRES)) %>%
  group_by(vist) %>% count() %>% mutate(type = "CBC_LYMPH_PCT")
# CBC_LYMPH_FCC_LBORRES
CBC_LYMPH_FCC_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_LYMPH_FCC_LBORRES) %>% filter(is.na(CBC_LYMPH_FCC_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_HCT")
# CBC_ERYTH_MM_LBORRES
CBC_ERYTH_MM_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_ERYTH_MM_LBORRES) %>% filter(is.na(CBC_ERYTH_MM_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_ERYTH_MM")
# CBC_MCV_LBORRES
CBC_MCV_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_MCV_LBORRES) %>% filter(is.na(CBC_MCV_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_MCV")
# CBC_MCH_LBORRES
CBC_MCH_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_MCH_LBORRES) %>% filter(is.na(CBC_MCH_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_MCH")
# CBC_MCHC_GDL_LBORRES
CBC_MCHC_GDL_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_MCHC_GDL_LBORRES) %>% filter(is.na(CBC_MCHC_GDL_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_MCHC_GDL")
# CBC_PLATE_LBORRES
CBC_PLATE_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_PLATE_LBORRES) %>% filter(is.na(CBC_PLATE_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_PLATE")
# CBC_MONO_PCT_LBORRES
CBC_MONO_PCT_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_MONO_PCT_LBORRES) %>% filter(is.na(CBC_MONO_PCT_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_MONO_PCT")
# CBC_MONO_FCC_LBORRES
CBC_MONO_FCC_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_MONO_FCC_LBORRES) %>% filter(is.na(CBC_MONO_FCC_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_MONO_FCC")
# CBC_EOS_PCT_LBORRES
CBC_EOS_PCT_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_EOS_PCT_LBORRES) %>% filter(is.na(CBC_EOS_PCT_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_EOS_PCT")
# CBC_EOS_FCC_LBORRES
CBC_EOS_FCC_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_EOS_FCC_LBORRES) %>% filter(is.na(CBC_EOS_FCC_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_EOS_FCC")
# CBC_RDW_PCT_LBORRES
CBC_RDW_PCT_vsts <- cbc_na_vsts %>% select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_RDW_PCT_LBORRES) %>% filter(is.na(CBC_RDW_PCT_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_RDW_PCT")
# CBC_PDW_CT_LBORRES
CBC_PDW_CT_vsts <- cbc_na_vsts %>% filter(CBC_LBPERF_18%in%c("Yes",NA)) %>%  
  select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_LBPERF_18,CBC_PDW_CT_LBORRES) %>% filter(is.na(CBC_PDW_CT_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_PDW_CT")
# CBC_PCT_PCT_LBORRES
CBC_PCT_PCT_vsts <- cbc_na_vsts %>% filter(CBC_LBPERF_19%in%c("Yes",NA)) %>%  
  select(studyID,vist,SCRN_FAC_SPFY_OBSLOC,CBC_LBPERF_19,CBC_PCT_PCT_LBORRES) %>% filter(is.na(CBC_PCT_PCT_LBORRES)) %>% 
  group_by(vist) %>% count() %>% mutate(type = "CBC_PCT_PCT")
...
#Tests not done
  ...
  select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_HB_LBORRES) %>% filter(is.na(CBC_HB_LBORRES)) %>% nrow()
  # CBC_HCT_LBORRES
  CBC_HCT_NOT_done <- cbc_na %>% filter(CBC_LBPERF_2 == "No"&is.na(CBC_HCT_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_HCT_LBORRES) %>% filter(is.na(CBC_HCT_LBORRES)) %>% nrow()
  # CBC_WBC_LBORRES
  CBC_WBC_NOT_done <- cbc_na %>% filter(CBC_LBPERF_3 == "No"&is.na(CBC_WBC_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_WBC_LBORRES) %>% filter(is.na(CBC_WBC_LBORRES)) %>% nrow()
  # CBC_NEU_PCT_LBORRES
  CBC_NEU_PCT_NOT_done <- cbc_na %>% filter(CBC_LBPERF_4 == "No"&is.na(CBC_NEU_PCT_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_NEU_PCT_LBORRES) %>% filter(is.na(CBC_NEU_PCT_LBORRES)) %>% nrow()
  # CBC_NEU_FCC_LBORRES
  CBC_NEU_FCC_NOT_done <- cbc_na %>% filter(CBC_LBPERF_5 == "No"&is.na(CBC_NEU_FCC_LBORRES)) %>% 
    filter(CBC_LBPERF_2 == "No"&is.na(CBC_HCT_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_NEU_FCC_LBORRES) %>% filter(is.na(CBC_NEU_FCC_LBORRES)) %>% nrow()
  # CBC_LYMPH_PCT_LBORRES
  CBC_LYMPH_PCT_NOT_done <- cbc_na %>% filter(CBC_LBPERF_6 == "No"&is.na(CBC_LYMPH_PCT_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_LYMPH_PCT_LBORRES) %>% filter(is.na(CBC_LYMPH_PCT_LBORRES)) %>% nrow()
  # CBC_LYMPH_FCC_LBORRES
  CBC_LYMPH_FCC_NOT_done <- cbc_na %>% filter(CBC_LBPERF_7 == "No"&is.na(CBC_LYMPH_FCC_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_LYMPH_FCC_LBORRES) %>% filter(is.na(CBC_LYMPH_FCC_LBORRES)) %>% nrow()
  # CBC_ERYTH_MM_LBORRES
  CBC_ERYTH_MM_NOT_done <- cbc_na %>% filter(CBC_LBPERF_8 == "No"&is.na(CBC_ERYTH_MM_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_ERYTH_MM_LBORRES) %>% filter(is.na(CBC_ERYTH_MM_LBORRES)) %>% nrow()
  # CBC_MCV_LBORRES
  CBC_MCV_NOT_done <- cbc_na %>% filter(CBC_LBPERF_9 == "No"&is.na(CBC_MCV_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_MCV_LBORRES) %>% filter(is.na(CBC_MCV_LBORRES)) %>% nrow()
  # CBC_MCH_LBORRES
  CBC_MCH_NOT_done <- cbc_na %>% filter(CBC_LBPERF_10 == "No"&is.na(CBC_MCH_LBORRES)) %>%
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_MCH_LBORRES) %>% filter(is.na(CBC_MCH_LBORRES)) %>% nrow()
  # CBC_MCHC_GDL_LBORRES
  CBC_MCHC_GDL_NOT_done <- cbc_na %>% filter(CBC_LBPERF_11 == "No"&is.na(CBC_MCHC_GDL_LBORRES)) %>%
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_MCHC_GDL_LBORRES) %>% filter(is.na(CBC_MCHC_GDL_LBORRES)) %>% nrow()
  # CBC_PLATE_LBORRES
  CBC_PLATE_NOT_done <- cbc_na %>% filter(CBC_LBPERF_12 == "No"&is.na(CBC_PLATE_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_PLATE_LBORRES) %>% filter(is.na(CBC_PLATE_LBORRES)) %>% nrow()
  # CBC_MONO_PCT_LBORRES
  CBC_MONO_PCT_NOT_done <- cbc_na %>% filter(CBC_LBPERF_13 == "No"&is.na(CBC_MONO_PCT_LBORRES)) %>% 
    filter(CBC_LBPERF_2 == "No"&is.na(CBC_HCT_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_MONO_PCT_LBORRES) %>% filter(is.na(CBC_MONO_PCT_LBORRES)) %>% nrow()
  # CBC_MONO_FCC_LBORRES
  CBC_MONO_FCC_NOT_done <- cbc_na %>% filter(CBC_LBPERF_14 == "No"&is.na(CBC_MONO_FCC_LBORRES)) %>% 
    filter(CBC_LBPERF_2 == "No"&is.na(CBC_HCT_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_MONO_FCC_LBORRES) %>% filter(is.na(CBC_MONO_FCC_LBORRES)) %>% nrow()
  # CBC_EOS_PCT_LBORRES
  CBC_EOS_PCT_NOT_done <- cbc_na %>% filter(CBC_LBPERF_15 == "No"&is.na(CBC_EOS_PCT_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_EOS_PCT_LBORRES) %>% filter(is.na(CBC_EOS_PCT_LBORRES)) %>% nrow()
  # CBC_EOS_FCC_LBORRES
  CBC_EOS_FCC_NOT_done <- cbc_na %>% filter(CBC_LBPERF_16 == "No"&is.na(CBC_EOS_FCC_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_EOS_FCC_LBORRES) %>% filter(is.na(CBC_EOS_FCC_LBORRES)) %>% nrow()
  # CBC_RDW_PCT_LBORRES
  CBC_RDW_PCT_NOT_done <- cbc_na %>% filter(CBC_LBPERF_17 == "No"&is.na(CBC_RDW_PCT_LBORRES)) %>% 
    select(studyID,SCRN_FAC_SPFY_OBSLOC,CBC_RDW_PCT_LBORRES) %>% filter(is.na(CBC_RDW_PCT_LBORRES)) %>% nrow()
  # CBC_PDW_CT_LBORRES
...
ReTention_Data <- Data1 %>% drop_na(studyID) %>%   select(studyID,vist,ANC_OBSSTDAT,DELIV_DSSTDAT_INF1,CLOSE_DSSTDAT) %>%
  group_by(studyID,vist) %>% fill(DELIV_DSSTDAT_INF1) %>% ungroup() %>%
  mutate(DELIV_DSSTDAT_INF1 = as.Date(DELIV_DSSTDAT_INF1),
         composite_obsdate = ANC_OBSSTDAT,
         composite_obsdate = if_else(is.na(composite_obsdate),DELIV_DSSTDAT_INF1,ANC_OBSSTDAT)) %>% 
...
kft_na <- Data1 %>% filter(complete.cases(studyID)) %>% filter(vist%in%c("Enrollment","ANC-32")) %>%  #filter(studyID!="KEARC00300") %>% 
  filter(MAT_VISIT_MNH04%in%c("Yes, visit conducted in person",NA)) %>% 
 #select(studyID,vist,MAT_VISIT_MNH04) %>% View()
  ...
  select(studyID,ANC_OBSSTDAT,visit_date,vist,SCRN_FAC_SPFY_OBSLOC,RENAL_LBPERF_1,BUN_mmoll_LBORRES, RENAL_LBPERF_2,CREAT_umoll_LBORRES,	
  ...
  # anti_join(ReTention_Data %>% filter(vist=="IPC") %>% filter(composite_obsdate<ANC_32_Earliest_Date) %>% distinct(studyID) %>%
  #             full_join(ReTention_Data %>% drop_na(CLOSE_DSSTDAT) %>% filter(CLOSE_DSSTDAT<ANC_32_Earliest_Date) %>% distinct(studyID)) %>% 
  #             mutate(vist = 4, vist = factor(vist,levels = c(4),labels = c("ANC-32"))))


# BUN_mmoll_LBORRES
BUN_mmoll_LBORRES_NA <- kft_na %>% #filter(RENAL_LBPERF_1%in%c("Yes",NA)) %>%
  filter(is.na(BUN_mmoll_LBORRES)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,BUN_mmoll_LBORRES) #%>% view()
# CREAT_umoll_LBORRES
CREAT_umoll_LBORRES_NA <- kft_na %>% #filter(RENAL_LBPERF_2%in%c("Yes",NA)) %>%
  filter(is.na(CREAT_umoll_LBORRES)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,CREAT_umoll_LBORRES) #%>% view()
# SODIUM_LBORRES
SODIUM_LBORRES_NA <- kft_na %>% #filter(RENAL_LBPERF_3%in%c("Yes",NA)) %>%
  filter(is.na(SODIUM_LBORRES)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,SODIUM_LBORRES) #%>% view()
# POTASSIUM_LBORRES
POTASSIUM_LBORRES_NA <- kft_na %>% #filter(RENAL_LBPERF_4%in%c("Yes",NA)) %>%
  filter(is.na(POTASSIUM_LBORRES)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,POTASSIUM_LBORRES) #%>% view()
# CHLORIDE_LBORRES
CHLORIDE_LBORRES_NA <- kft_na %>% #filter(RENAL_LBPERF_5%in%c("Yes",NA)) %>%
  filter(is.na(CHLORIDE_LBORRES)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,CHLORIDE_LBORRES) #%>% view()
# PHOSPHORUS_LBORRES
PHOSPHORUS_LBORRES_NA <- kft_na %>% #filter(RENAL_LBPERF_6%in%c("Yes",NA)) %>%
  filter(is.na(PHOSPHORUS_LBORRES)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,PHOSPHORUS_LBORRES) #%>% view()
# CALCIUM_LBORRES
CALCIUM_LBORRES_NA <- kft_na %>% #filter(RENAL_LBPERF_7%in%c("Yes",NA)) %>%
  filter(is.na(CALCIUM_LBORRES)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,CALCIUM_LBORRES) #%>% view()
# CARB_DIOX_LBORRES
CARB_DIOX_LBORRES_NA <- kft_na %>% #filter(RENAL_LBPERF_8%in%c("Yes",NA)) %>%
  filter(is.na(CARB_DIOX_LBORRES)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,CARB_DIOX_LBORRES) #%>% view()

#------------------------------------------------------------------------------------------------
#LIVER FUNCTION TEST.

lft_na <- Data1 %>% filter(complete.cases(studyID)) %>% filter(vist%in%c("Enrollment","ANC-32")) %>% #filter(studyID!="KEARC00300") %>% 
   filter(MAT_VISIT_MNH04%in%c("Yes, visit conducted in person",NA)) %>% 
  select(studyID,ANC_OBSSTDAT,visit_date,vist,SCRN_FAC_SPFY_OBSLOC, LFT_LBTSTDAT,	LFT_LBPERF_1,	AST_ul_LBORRES,	LFT_LBPERF_2,	ALT_ul_LBORRES,
...
AST_SGOT_NA <- lft_na %>% #filter(LFT_LBPERF_1%in%c("Yes",NA)) %>% 
  filter(is.na(AST_SGOT)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,AST_SGOT) #%>% view()
# ALT_SGPT
ALT_SGPT_NA <- lft_na %>% #filter(LFT_LBPERF_2%in%c("Yes",NA)) %>% 
  filter(is.na(ALT_SGPT)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,ALT_SGPT) #%>% view()
# ALP
ALP_NA <-  lft_na %>% #filter(LFT_LBPERF_3%in%c("Yes",NA)) %>% 
  filter(is.na(ALP)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,ALP) #%>% view()
# TBILIRUBIN
TBILIRUBIN_NA <- lft_na %>% #filter(LFT_LBPERF_4%in%c("Yes",NA)) %>% 
  filter(is.na(TBILIRUBIN)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,TBILIRUBIN) #%>% view()
# DBILIRUBIN
DBILIRUBIN_NA <- lft_na %>% #filter(LFT_LBPERF_5%in%c("Yes",NA)) %>% 
  filter(is.na(DBILIRUBIN)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,DBILIRUBIN) #%>% view()
# TPROTEIN
TPROTEIN_NA <- lft_na %>% #filter(LFT_LBPERF_6%in%c("Yes",NA)) %>% 
  filter(is.na(TPROTEIN)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,TPROTEIN) #%>% view()
# ALBUMIN
ALBUMIN_NA <- lft_na %>% #filter(LFT_LBPERF_7%in%c("Yes",NA)) %>% 
  filter(is.na(ALBUMIN)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,ALBUMIN) #%>% view()
# GAMMAGT
GAMMAGT_NA <- lft_na %>% #filter(LFT_LBPERF_8%in%c("Yes",NA)) %>% 
  filter(is.na(GAMMAGT)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,GAMMAGT) #%>% view()
# IBILIRUBIN
IBILIRUBIN_NA <- lft_na %>% #filter(LFT_LBPERF_9%in%c("Yes",NA)) %>% 
  filter(is.na(IBILIRUBIN)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,IBILIRUBIN) #%>% view()

#--------------------------------------------------------------------------------------
#             THYROID FUNCTION TEST

tsh_na <- Data1 %>% filter(complete.cases(studyID)) %>% filter(vist%in%c("Enrollment","ANC-32")) %>% #filter(studyID!="KEARC00300") %>% 
   filter(MAT_VISIT_MNH04%in%c("Yes, visit conducted in person",NA)) %>% 
  select(studyID,ANC_OBSSTDAT,visit_date,vist,SCRN_FAC_SPFY_OBSLOC,THYROID_LBTSTDAT, THYROID_LBPERF_1, THYROID_TSH_LBORRES, THYROID_LBPERF_2,
...
THYROID_TSH_NA <-  tsh_na %>% #filter(THYROID_LBPERF_1%in%c("Yes",NA)) %>% 
  filter(is.na(THYROID_TSH)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,THYROID_TSH) %>% 
  distinct(studyID,vist,.keep_all = T)#%>% view()
# THYROID_FREET4
THYROID_FREET4_NA <- tsh_na %>% #filter(THYROID_LBPERF_2%in%c("Yes",NA)) %>% 
  filter(is.na(THYROID_FREET4)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,THYROID_FREET4) %>% 
  distinct(studyID,vist,.keep_all = T)#%>% view()
# THYROID_FREET3
THYROID_FREET3_NA <- tsh_na %>% #filter(THYROID_LBPERF_3%in%c("Yes",NA)) %>% 
  filter(is.na(THYROID_FREET3)) %>% select(studyID,ANC_OBSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,THYROID_FREET3) %>% 
  distinct(studyID,vist,.keep_all = T)#%>% view()
...
# Removals
# anc 20
urin_rmve <- bind_rows(ReTention_Data %>% filter(vist=="IPC") %>% filter(composite_obsdate<ANC_20_Earliest_Date) %>% distinct(studyID) %>%
                         full_join(ReTention_Data %>% drop_na(CLOSE_DSSTDAT) %>% filter(CLOSE_DSSTDAT<ANC_20_Earliest_Date) %>% distinct(studyID)) %>% 
                         mutate(vist = 2, vist = factor(vist,levels = c(2),labels = c("ANC-20"))),
                       
                       # anc 28
                       ReTention_Data %>% filter(vist=="IPC") %>% filter(composite_obsdate<ANC_28_Earliest_Date) %>% distinct(studyID) %>%
                         full_join(ReTention_Data %>% drop_na(CLOSE_DSSTDAT) %>% filter(CLOSE_DSSTDAT<ANC_28_Earliest_Date) %>% distinct(studyID)) %>% 
                         mutate(vist = 3, vist = factor(vist,levels = c(3),labels = c("ANC-28"))), 
                       
                       # anc 32
                       ReTention_Data %>% filter(vist=="IPC") %>% filter(composite_obsdate<ANC_32_Earliest_Date) %>% distinct(studyID) %>%
                         full_join(ReTention_Data %>% drop_na(CLOSE_DSSTDAT) %>% filter(CLOSE_DSSTDAT<ANC_32_Earliest_Date) %>% distinct(studyID)) %>% 
                         mutate(vist = 4, vist = factor(vist,levels = c(4),labels = c("ANC-32"))),
                       
                       # anc 36
                       ReTention_Data %>% filter(vist=="IPC") %>% filter(composite_obsdate<ANC_36_Earliest_Date) %>% distinct(studyID) %>%
                         full_join(ReTention_Data %>% drop_na(CLOSE_DSSTDAT) %>% filter(CLOSE_DSSTDAT<ANC_36_Earliest_Date) %>% distinct(studyID)) %>% 
                         mutate(vist = 5, vist = factor(vist,levels = c(5),labels = c("ANC-36"))))



urin_na <- Data1 %>% filter(complete.cases(studyID)) %>% filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","PNC-6")) %>%
 # filter(studyID!="KEARC00300") %>%  
  filter(MAT_VISIT_MNH04%in%c("Yes, visit conducted in person",NA)) %>%
  filter(MAT_VISIT_MNH12%in%c("Yes, visit conducted in person",NA)) %>%  #select(studyID,vist,MAT_VISIT_MNH04,MAT_VISIT_MNH12) %>% View()
  select(studyID,VISIT_OBSSTDAT,visit_date,vist,SCRN_FAC_SPFY_OBSLOC,CON_DSSTDAT, UA_LBPERF_1,UA_PROT_LBORRES,UA_LBPERF_2,
         UA_LEUK_LBORRES,UA_LBPERF_3,UA_NITRITE_LBORRES) %>% group_by(studyID) %>% fill(SCRN_FAC_SPFY_OBSLOC,.direction = "updown") %>% 
  ungroup() %>% replace(.,.==77,NA) #%>% anti_join(urin_rmve)
#select(studyID,VISIT_OBSSTDAT,visit_date,vist,SCRN_FAC_SPFY_OBSLOC,CON_DSSTDAT,UA_PROT_LBORRES,UA_LEUK_LBORRES,UA_NITRITE_LBORRES)# %>% View()

# UA_PROT_LBORRES
UA_PROT_LBORRES_NA <-  urin_na %>% #filter(UA_LBPERF_1%in%c("Yes",NA)) %>% 
  filter(is.na(UA_PROT_LBORRES)) %>% select(studyID,visit_date,vist,SCRN_FAC_SPFY_OBSLOC,UA_PROT_LBORRES) %>% 
  distinct(studyID,vist,.keep_all = T) #%>% view()

# UA_LEUK_LBORRES
UA_LEUK_LBORRES_NA <-  urin_na %>% #filter(UA_LBPERF_2%in%c("Yes",NA)) %>% 
  filter(is.na(UA_LEUK_LBORRES)) %>% select(studyID,visit_date,vist,SCRN_FAC_SPFY_OBSLOC,UA_LEUK_LBORRES) %>% 
  distinct(studyID,vist,.keep_all = T)#%>% view()

# UA_NITRITE_LBORRES
UA_NITRITE_LBORRES_NA <-  urin_na %>% #filter(UA_LBPERF_3%in%c("Yes",NA)) %>% 
  filter(is.na(UA_NITRITE_LBORRES)) %>% select(studyID,visit_date,vist,SCRN_FAC_SPFY_OBSLOC,UA_NITRITE_LBORRES) %>% 
  distinct(studyID,vist,.keep_all = T)#%>% view()
...
# BUN
Lab_data_na <- bind_rows(BUN_mmoll_LBORRES_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(BUN_mmoll_LBORRES))) %>% 
                           mutate(lab_variable = paste0("BUN"),
                                  type =paste0("KFT")),
                         # CREATINE
                         CREAT_umoll_LBORRES_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(CREAT_umoll_LBORRES))) %>% 
                           mutate(lab_variable = paste0("CREATINE"),
                                  type =paste0("KFT")),
                         # SODIUM
                         SODIUM_LBORRES_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(SODIUM_LBORRES))) %>% 
                           mutate(lab_variable = paste0("SODIUM"),
                                  type =paste0("KFT")),
                         # POTASSIUM
                         POTASSIUM_LBORRES_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(POTASSIUM_LBORRES))) %>% 
                           mutate(lab_variable = paste0("POTASSIUM"),
                                  type =paste0("KFT")),
                         # CHLORIDE
                         CHLORIDE_LBORRES_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(CHLORIDE_LBORRES))) %>% 
                           mutate(lab_variable = paste0("CHLORIDE"),
                                  type =paste0("KFT")),
                         # PHOSPHORUS
                         PHOSPHORUS_LBORRES_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(PHOSPHORUS_LBORRES))) %>% 
                           mutate(lab_variable = paste0("PHOSPHORUS"),
                                  type =paste0("KFT")),
                         # CALCIUM
                         CALCIUM_LBORRES_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(CALCIUM_LBORRES))) %>% 
                           mutate(lab_variable = paste0("CALCIUM"),
                                  type =paste0("KFT")),
                         # CARB_DIOX
                         CARB_DIOX_LBORRES_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(CARB_DIOX_LBORRES))) %>% 
                         ...
                         # AST_SGOT
                         AST_SGOT_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(AST_SGOT))) %>% 
                           mutate(lab_variable = paste0("AST/SGOT"),
                                  type =paste0("LFT")),
                         # ALT_SGPT
                         ALT_SGPT_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(ALT_SGPT))) %>% 
                           mutate(lab_variable = paste0("ALT/SGPT"),
                                  type =paste0("LFT")),
                         # ALP
                         ALP_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(ALP))) %>% 
                           mutate(lab_variable = paste0("ALP"),
                                  type =paste0("LFT")),
                         # TBILIRUBIN
                         TBILIRUBIN_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(TBILIRUBIN))) %>% 
                           mutate(lab_variable = paste0("TBILIRUBIN"),
                                  type =paste0("LFT")),
                         # DBILIRUBIN
                         DBILIRUBIN_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(DBILIRUBIN))) %>% 
                           mutate(lab_variable = paste0("DBILIRUBIN"),
                                  type =paste0("LFT")),
                         # TPROTEIN
                         TPROTEIN_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(TPROTEIN))) %>% 
                           mutate(lab_variable = paste0("TPROTEIN"),
                                  type =paste0("LFT")),
                         # ALBUMIN
                         ALBUMIN_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(ALBUMIN))) %>% 
                           mutate(lab_variable = paste0("ALBUMIN"),
                                  type =paste0("LFT")),
                         # GAMMAGT
                         GAMMAGT_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(GAMMAGT))) %>% 
                           mutate(lab_variable = paste0("GAMMAGT"),
                                  type =paste0("LFT")),
                         # IBILIRUBIN
                         IBILIRUBIN_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(IBILIRUBIN))) %>% 
                         ...
                         # TSH
                         THYROID_TSH_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(THYROID_TSH))) %>% 
                           mutate(lab_variable = paste0("TSH"),
                                  type =paste0("TSH")),
                         # FREET4
                         THYROID_FREET4_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(THYROID_FREET4))) %>% 
                           mutate(lab_variable = paste0("FREET4"),
                                  type =paste0("TSH")),
                         # FREET3
                         THYROID_FREET3_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(THYROID_FREET3))) %>% 
                         ...
                         # Nitrites
                         UA_NITRITE_LBORRES_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(UA_NITRITE_LBORRES))) %>% 
                           mutate(lab_variable = paste0("Nitrites"),
                                  type =paste0("URINALYSIS")),
                         # Leukocytes
                         UA_LEUK_LBORRES_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(UA_LEUK_LBORRES))) %>% 
                           mutate(lab_variable = paste0("Leukocytes"),
                                  type =paste0("URINALYSIS")),
                         #Protein
                         UA_PROT_LBORRES_NA %>% group_by(vist) %>% summarize(count_na = sum(is.na(UA_PROT_LBORRES))) %>% 
                           mutate(lab_variable = paste0("Protein"),
                                  type =paste0("URINALYSIS")))  

# The graph
Lab_data_na %>% ggplot(aes(area = count_na,fill=vist,
...
#| echo: false
 ...
 Data1 %>% filter(vist%in%c("Enrollment","ANC-32","PNC-6")) %>% replace(.,.==-7,NA) %>%
   filter(MAT_VISIT_MNH04%in%c("Yes, visit conducted in person",NA)) %>% filter(MAT_VISIT_MNH12%in%c("Yes, visit conducted in person",NA)) %>% 
    select(studyID, CON_DSSTDAT, SCRN_FAC_SPFY_OBSLOC, vist,
  ...
  select(studyID, CON_DSSTDAT, SCRN_FAC_SPFY_OBSLOC, vist, Cobalamin = VITB12_COB_LBORRES,
    ...
    mutate(CON_DSSTDAT = as.Date(CON_DSSTDAT)) %>% select(studyID,vist,Cobalamin:Histidine_rich_protein_2) %>%
  ...
  group_by(Micronutrient_var,vist) %>% count(Results) %>% filter(Results=="Missing results") %>% 
...
#| echo: false
 ...
 Data1 %>% filter(vist%in%c("Enrollment","ANC-32","PNC-6")) %>% replace(.,.==-7,NA) %>%
  ...
  select(studyID, CON_DSSTDAT, SCRN_FAC_SPFY_OBSLOC, vist, Cobalamin = VITB12_COB_LBORRES,
         ...
         IODINE = IODINE_LBORRES, TRANSFERRIN = TRANSFERRIN_LBORRES, Retinol_protein = RBP4_LBORRES,  C_reactive_protein = CRP_LBORRES,
         Alpha_1_acid_glycoprotein = AGP_LBORRES,  Histidine_rich_protein_2 = HRP_LBORRES) %>%
    mutate(CON_DSSTDAT = as.Date(CON_DSSTDAT)) %>% select(studyID,Cobalamin:Histidine_rich_protein_2) %>%
    mutate_at(vars(Cobalamin:Histidine_rich_protein_2),~replace(.x,.x==-5,NA)) %>% 
  pivot_longer(cols = Cobalamin:Histidine_rich_protein_2 ,names_to = "Micronutrient_var",values_to = "Results") %>% 
  mutate_if(is.numeric,as.character) %>% 
  mutate_at(vars(Results),~replace(.x,is.na(.x),"Missing results")) %>% 
...
#| fig-height: 5.2
#| echo: false
 Data1 %>% drop_na(studyID) %>% filter(CON_DSSTDAT >= "2023-04-03",vist=="Enrollment") %>% 
  select(studyID,vist,RBC_LBTSTDAT:RBC_GLUC6_LBORRES,MAT_VISIT_MNH04,MAT_VISIT_MNH12,COVAL_MNH08,COVAL_MNH04,COYN_MNH12) %>% select(-contains("LBPERF")) %>%
  select(everything(),-contains("DAT"),vist) %>% select(studyID,vist,everything()) %>% 
...
select(studyID,vist,`Sickle cell disease`:G6PD) %>%
  ...
  mutate_at(vars(Results),~replace(.x,is.na(.x),"Missing results")) %>% 
  mutate_at(vars(Results),~replace(.x,.x!="Missing results","Available results")) %>% 
  group_by(RBC_vars,vist) %>% count(Results) %>% filter(Results=="Missing results") %>% 
...
#           =================================================
PS.approach <- Data1 %>% drop_na(SCRN_ID) %>% distinct(SCRN_ID,.keep_all = T) %>% 
  #filter(between(SCRN_OBSSTDAT,ymd("2023-06-01"),ymd("2023-06-30"))) %>% 
  select(SCRN_ID,SCRN_OBSSTDAT) %>% nrow()

#   -----------  Total Screened participants by Ultrasound  ---------
#           =================================================
PS.eligible <- Data1 %>% drop_na(SCRN_ID) %>% distinct(SCRN_ID,.keep_all = T) %>% 
  #filter(between(SCRN_OBSSTDAT,ymd("2023-06-01"),ymd("2023-06-30"))) %>% 
  filter(complete.cases(US_PELOC)) %>% select(SCRN_ID,SCRN_OBSSTDAT) %>% distinct(SCRN_ID,.keep_all = T) %>% nrow()

#  ------------  Total consented participants after Ultrasound  ---------
#           ===================================================
PS.consented <- Data1 %>% drop_na(studyID) %>%
  #filter(between(CON_DSSTDAT,ymd("2023-06-01"),ymd("2023-06-30")) ) %>% 
  distinct(studyID) %>% nrow()
...
No.ultrasound <- Data1 %>% drop_na(US_OHOSTDAT) %>% filter(!vist %in%c("ANC-20")) %>% 
...
#""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
ultrsnd_ga.20 <- Data1 %>%drop_na(SCRN_ID,US_OHOSTDAT) %>% 
  #filter(between(US_OHOSTDAT,ymd("2023-06-01"),ymd("2023-06-30")) ) %>% 
  filter(US_GA_WKS_AGE_FTS1 < 20) %>% distinct(SCRN_ID,.keep_all = T) %>% 
  nrow()

#   -----------------     Total Fetal abnormalities  ----------------
#                ===================================================
fetl.abnom <- Data1 %>% drop_na(studyID) %>% 
  #filter(between(US_OHOSTDAT,ymd("2023-06-01"),ymd("2023-06-30")) ) %>% 
  filter(ANOMALY_PEDESC_FTS1 == "Yes") %>% distinct(studyID,.keep_all = T) %>% nrow()
 
 # Locked_study1 %>% drop_na(studyID) %>% group_by(studyID) %>% fill(pregnancy_outcomes,.direction = "updown") %>% ungroup() %>% 
 #  filter(ANOMALY_PEDESC_FTS1 == "Yes") %>% select(studyID,ANOMALY_PEDESC_FTS1,ANOMALY_SPFY_PEDESC_FTS1,pregnancy_outcomes,COVAL_MNH01) %>% View()
 
 

#   --------------- Total women deferred from Pregnancy Complications ----------------
#                ========================================================
complication <- Data1 %>% drop_na(studyID) %>% 
...
#         ========================================================
# No.MITS = Data1 %>% drop_na(studyID) %>% 
#   filter(between(outcome_dt,ymd("2023-06-01"),ymd("2023-06-30"))) %>% 
#   filter(dth_coenrolled == 1) %>%  distinct(studyID) %>% nrow()

#     --------------- Total Day 01(PNC-0) forms completed ----------------
#        ==================================================
Day.1 = Data1 %>% drop_na(studyID) %>% 
  # filter(between(visit_date,ymd("2023-06-01"),ymd("2023-06-30"))) %>% 
  filter(vist == "IPC") %>% distinct(studyID,.keep_all = T) %>% nrow()

#     --------------- Total Day 42 forms completed ----------------
#        ==================================================
Day.2 = Data1 %>% drop_na(studyID) %>% 
  #filter(between(visit_date,ymd("2023-06-01"),ymd("2023-06-30"))) %>% 
  filter(vist == "PNC-6") %>% distinct(studyID,.keep_all = T) %>% nrow()
...
#   ---------------        Total Fetal loss         ----------------
#         ========================================================
fetal_loss <- Data1 %>% drop_na(studyID) %>% 
  #filter(between(visit_date,ymd("2023-04-01"),ymd("2023-04-30"))) %>% 
  drop_na(FETAL_LOSS_DSDECOD) %>% 
  select(studyID,FETAL_LOSS_DSDECOD,FETAL_LOSS_DSSTDAT,visit_date,DELIV_DSSTDAT_INF1,CON_DSSTDAT,
...
#____________________________________________________________________________________________________________________________________________
 ...
 mits_data <- outcomes %>% rename(studyID = subjid) %>% filter(complete.cases(studyID)) %>% 
...
#                           Fetal loss
#--------------------------===========--------------------------
# Fetalloss <- Data1 %>% drop_na(studyID) %>%
#   filter(FETAL_LOSS_DSDECOD%in%c("Miscarriage/spontaneous abortion","Induced abortion","Stillbirth")) %>% nrow()

indcedAbrt <- Data1 %>% drop_na(studyID) %>% filter(FETAL_LOSS_DSDECOD%in%c("Induced abortion")) %>% nrow()


# stillbirths

Stillbirth_data <-  Data1 %>% drop_na(studyID) %>% #filter(between(visit_date,ymd("2024-02-01"),ymd("2024-03-31"))) %>% 
  select(studyID,Ga_Enrollment,CON_DSSTDAT,INFANTID,vist,visit_date,Ga_visit_weeks,FETAL_LOSS_DSDECOD,BIRTH_DSTERM_INF1,BIRTH_DSTERM_INF2,DELIV_DSSTDAT_INF1) %>% 
  mutate(FETAL_LOSS_DSDECOD = replace(FETAL_LOSS_DSDECOD, FETAL_LOSS_DSDECOD==77,NA)) %>% 
  mutate(is_ga_wk_20wks = if_else(BIRTH_DSTERM_INF1 == "Fetal death",(as.numeric(DELIV_DSSTDAT_INF1 - CON_DSSTDAT) + Ga_Enrollment)/7,NA)) %>% 
  mutate(stillbirth = if_else(is_ga_wk_20wks>=20,"Yes",as.character(NA)),
...
# stillbirths counts
stills_1 <- Stillbirth_data %>%  filter(vist == "IPC",stillbirth1 == "Yes") %>%  distinct(studyID,.keep_all = T) %>% nrow()
stills_2 <-  Stillbirth_data %>%  filter(vist == "IPC",stillbirth2 == "Yes") %>%  distinct(studyID,.keep_all = T) %>% nrow()
...
stillbirth_type <- Stillbirth_data %>% mutate(stillbirth1=coalesce(stillbirth1,stillbirth2)) %>% filter(stillbirth1 == "Yes") %>% 
  distinct(studyID,INFANTID,.keep_all = T) %>% drop_na(INFANTID) %>% group_by(studyID) %>% fill(is_ga_wk_20wks) %>% ungroup() %>% 
...
#miscarriages
Miscarriages <- Stillbirth_data %>%  filter(miscarriage == "Yes") %>%  distinct(studyID,.keep_all = T) %>%
  anti_join( Stillbirth_data %>%  filter(vist == "IPC",stillbirth1 == "Yes") %>%  distinct(studyID)) %>% nrow()
...
#                   ==================================================
IPC = Data1 %>% drop_na(studyID) %>% 
  # filter(between(visit_date,ymd("2023-06-01"),ymd("2023-06-30"))) %>% 
  filter(vist == "IPC") %>% distinct(studyID) %>% nrow()

# total number of delivery outcomes
IPC2 = Data1 %>% drop_na(studyID) %>% 
  # filter(between(visit_date,ymd("2023-06-01"),ymd("2023-06-30"))) %>% 
  filter(vist == "IPC") %>% nrow()

# cases not captured in IPC


missing_ipc <- Stillbirth_data %>%  filter(miscarriage == "Yes") %>%  distinct(studyID,.keep_all = T) %>%
  full_join( Stillbirth_data %>%  filter(vist == "IPC",stillbirth1 == "Yes") %>%  distinct(studyID)) %>% 
  anti_join(Data1 %>% drop_na(studyID) %>% filter(vist == "IPC") %>% distinct(studyID)) %>% nrow()
# total preg. outcomes
Total_preg_outcomes = IPC + missing_ipc


IPC_twins_included = Data1 %>% drop_na(studyID) %>% 
  # filter(between(visit_date,ymd("2023-06-01"),ymd("2023-06-30"))) %>% 
  filter(vist == "IPC") %>%  nrow()
...
#   ---------------        Total Live births         ----------------
#         ========================================================
live_birth <- Data1 %>% drop_na(studyID) %>% select(studyID,INFANTID,BIRTH_DSTERM_INF1,BIRTH_DSTERM_INF2) %>% 
  #filter(between(visit_date,ymd("2023-01-01"),ymd("2023-06-30"))) %>% 
  filter(BIRTH_DSTERM_INF1 == "Live birth") %>% distinct(studyID,INFANTID) %>% nrow()


preg_outcomes <- live_birth + Fetal_loss
#--------------------------------------------------------------------------------------
# Preterm births
# pretermBrth <- Data1 %>% drop_na(studyID) %>% select(studyID,PTL_MHOCCUR) %>%  filter(PTL_MHOCCUR == "Yes") %>% select(studyID) %>% full_join(
#   Data1 %>% drop_na(studyID) %>% select(studyID,PTB_MHOCCUR) %>%  filter(PTB_MHOCCUR == "Yes") %>% select(studyID)) %>% nrow()

# preterm delivery
 pretermBrth <- Data1 %>% drop_na(studyID) %>% select(studyID,INFANTID,vist,INF_VITAL_MNH11,INF_DSTERM,BIRTH_DSTERM_INF1,Ga_outcome_weeks,INFANTID,Ga_visit_weeks) %>% 
...
# Preterm births
 preterm_births <- Data1 %>% drop_na(studyID) %>% select(studyID,INFANTID,vist,INF_VITAL_MNH11,INF_DSTERM,BIRTH_DSTERM_INF1,Ga_outcome_weeks,INFANTID,Ga_visit_weeks) %>% 
...
# Neonatal deaths
infant_closeout <- read_rds(here("Data","MNH24_Infant_close_out.rds"))
neonDeaths <- Data1 %>% drop_na(studyID) %>% group_by(studyID) %>% 
  fill(INFANTID,BIRTH_DSTERM_INF1,INF_VITAL_MNH11,INF_VITAL_MNH13,INF_VITAL_MNH14,INF_VITAL_MNH15,.direction = "updown") %>% 
  ungroup() %>% filter(BIRTH_DSTERM_INF1 == "Live birth") %>% 
  select(studyID,INFANTID,vist,DELIV_DSSTDAT_INF1,BIRTH_DSTERM_INF1,INF_VITAL_MNH11,INF_VITAL_MNH13,INF_VITAL_MNH14,INF_VITAL_MNH15) %>% 
  mutate(Neonatal_death = if_else(INF_VITAL_MNH11 == "Died"|INF_VITAL_MNH13 == "Died"|INF_VITAL_MNH14== "Died"|INF_VITAL_MNH15== "Died","Yes",NA)) %>% 
  filter(Neonatal_death == "Yes") %>% distinct(studyID,.keep_all = T) %>% select(studyID,INFANTID,DELIV_DSSTDAT_INF1) %>% full_join(infant_closeout %>% filter(studyID%in%c("KEARC01006","KEARC00102")) %>% select(studyID,INFANTID)) %>% nrow()

#neonDeaths %>% left_join(infant_closeout) %>% View()


# Anemia prevalence
anemia_prev <- Data1 %>% drop_na(studyID) %>%
  #filter(complete.cases(studyID)) %>%
  select(studyID,vist,HB_POC_LBORRES) %>%
  ...
  tabyl(vist,HB_POC_1) %>% adorn_totals(where = "col") %>%  #select(vist,Normal, Anemic) %>% 
  mutate(vist = as.character(vist)) %>%
  mutate(vist=replace(vist,vist==" N/A",as.character(NA))) %>%
  drop_na(vist) %>% filter(!vist%in%c("N/A","Non-scheduled ANC visit for routine care","Non-scheduled visit",
                                                  "Non-scheduled visit for routine care")) %>%
  mutate(Prevalence = paste0(round((Anemic/Total)*100,1),"%"),
         HB_tool = paste0("POC Hemacue")) %>% 
  mutate(anemprev = paste0(Anemic,"(",Prevalence,")")) %>% select(vist,anemprev) 

# BIRTHWEIGHT
# BW_FAORRES -- study staff, BW_FAORRES_REPORT--facility weight
BW <- Data1 %>% select(studyID,INFANTID,BW_FAORRES_REPORT,BW_FAORRES,BW_EST_FAORRES,WEIGHT_PERES,HEIGHT_PERES,vist,BIRTH_DSTERM_INF1) %>% 
...
APH <- Data1 %>% select(studyID,vist,APH_CEOCCUR) %>% 
  ...
  distinct(studyID,.keep_all = T) %>% tabyl(`antepartum hemorrhage (APH)`) %>% 
  mutate(percent = round(percent*100,1))

# PPH
PPH <- Data1 %>% select(studyID,vist,PPH_CEOCCUR) %>% 
  mutate(PPH_CEOCCUR = replace(PPH_CEOCCUR, PPH_CEOCCUR == "Don’t know",NA),
    `postpartum hemorrhage (PPH)`= if_else(PPH_CEOCCUR=="Yes","Yes",NA)) %>%
  group_by(studyID) %>% fill(`postpartum hemorrhage (PPH)`,.direction = "updown") %>% ungroup() %>% 
  ...
  distinct(studyID,.keep_all = T) %>% tabyl(`postpartum hemorrhage (PPH)`) %>% 
  mutate(percent = round(percent*100,1))


# Obstructed labor
Obs_labour <- Data1 %>% select(studyID,vist,OBSTRUCTED_LABOR_CEOCCUR) %>% 
  ...
  group_by(studyID) %>% fill(`Obstructed labor`,.direction = "updown") %>% ungroup() %>% 
  mutate(`Obstructed labor`= if_else(is.na(`Obstructed labor`),OBSTRUCTED_LABOR_CEOCCUR,`Obstructed labor`)) %>% 
    drop_na(`Obstructed labor`) %>% 
  distinct(studyID,.keep_all = T) %>% tabyl(`Obstructed labor`) %>% 
  mutate(percent = round(percent*100,1))

# Fetal distress
Fetal_distress <- Data1 %>% select(studyID,vist,FETAL_DISTRESS_CEOCCUR) %>% 
...
Ga_less14_prop <- round(Ga_less14/enrollmennts_study1*100,1)

Ga_above14 <- Data1 %>% drop_na(studyID) %>% filter(vist=="Enrollment") %>%
  select(studyID,US_GA_WKS_AGE_FTS1,US_GA_DAYS_AGE_FTS1) %>%
  mutate(Ga_weeks = US_GA_WKS_AGE_FTS1 +
           (US_GA_DAYS_AGE_FTS1/7)) %>% filter(Ga_weeks>=14) %>% nrow()
Ga_above14_prop <- round(Ga_above14/enrollmennts_study1*100,1)
...
table1 <- tribble( ~"Indicator",                               ~"Count",
         "Pre-screened",                            as.character(pre_scrn_study1),
         "Screened",                                as.character(paste0(scrnd_study1,"(",(round(scrnd_study1/pre_scrn_study1*100,1)),"%)")),
         "Enrollments",                             as.character(paste0(enrollmennts_study1,"(",(round(enrollmennts_study1/scrnd_study1*100,1)),"%)")),
...
table1 %>% tail(n=14) %>% mutate(Count = as.numeric(Count),
  ...
  tab_header(title = md("**study1 Pregnancy Outcome Table 2023/2024**")) %>% 
  opt_vertical_padding(scale = 0.5) %>% 
  tab_style(style = cell_fill(color = c("#FFE4E1")),
            locations = cells_column_labels()) %>% 
  tab_style(style = cell_fill(color = c("white")),
...
### Completeness / Distribution of study1 and Facility based birthweight measurement

```{r}
#| label: tab-facbwmeasure
#| tab-cap: "Distribution of study1 and Facility based birthweight measurement"
#| warning: false
#| column: body-outset-right
#| fig-width: 12
#| fig-height: 6
...
infant_dta <- Locked_study1 %>% drop_na(studyID) %>% filter(visit_date<=ymd("2024-04-05")) %>% 
  select(studyID,INFANTID,vist,SCRN_FAC_SPFY_OBSLOC,Ga_outcome_weeks, INF_DSTERM,BIRTH_DSTERM_INF1,US_Ga_days_enrol,LMP_Ga_days_enrol,
         ...
         JAUND_CEOCCUR,BW_EST_FAORRES,BIRTH_DSTERM_INF1,BW_FAORRES_REPORT,BW_FAORRES,inf_weight,inf_weight_cat,`study1 measured birthweight`,
...
Locked_study1 %>% drop_na(pregnancy_outcomes) %>% 
  mutate_at(vars(SCRN_FAC_SPFY_OBSLOC),~replace(.x,.x=="facility_5", "KCRH")) %>% 
    mutate_at(vars(SCRN_FAC_SPFY_OBSLOC),~replace(.x,.x=="facility_2", "KUOYO")) %>% 
    mutate_at(vars(SCRN_FAC_SPFY_OBSLOC),~replace(.x,.x=="Lumumba Sub County Hospital", "LUMUMBA")) %>% 
    mutate_at(vars(SCRN_FAC_SPFY_OBSLOC),~replace(.x,.x=="facility_4", "SIAYA")) %>% 
    mutate_at(vars(SCRN_FAC_SPFY_OBSLOC),~replace(.x,.x=="facility_7", "Tingwangi")) %>% 
  
  mutate(bw_completeness = case_when(BIRTH_DSTERM_INF1 == "Live birth" & is.na(inf_weight)~"Missing study1 & facility birthweight",
                                     BIRTH_DSTERM_INF1 == "Live birth" & is.na(BW_FAORRES)~"Missing study1 birthweight",
  ...
  tbl_summary(by = SCRN_FAC_SPFY_OBSLOC,include = c(bw_completeness,Missing_hrs_birthwght,inf_weight_cat,`study1 measured birthweight`,
...
# study1 measured birth weight
 bw_median = median((Locked_study1 %>% drop_na(BW_FAORRES) %>% filter(BIRTH_DSTERM_INF1 == "Live birth"))$BW_FAORRES,na.rm = T)
LBW_hist = Locked_study1 %>% drop_na(`Low BirthWeight (<2.5Kg)`) %>% tabyl(`Low BirthWeight (<2.5Kg)`) %>% adorn_pct_formatting()
vlbw_hist = Locked_study1 %>% drop_na(`Low BirthWeight (<2.5Kg)`) %>% tabyl(`VLBW (<1.5Kg)`) %>% adorn_pct_formatting()

Locked_study1 %>% drop_na(BW_FAORRES) %>% filter(BIRTH_DSTERM_INF1 == "Live birth") %>% 
...
timeHist = median((Locked_study1 %>% drop_na(BW_EST_FAORRES) %>% filter(BIRTH_DSTERM_INF1 == "Live birth"))$BW_EST_FAORRES,na.rm = T)
  
Locked_study1 %>% drop_na(BW_EST_FAORRES) %>% filter(BIRTH_DSTERM_INF1 == "Live birth") %>% 
...
infant_vitals <- Locked_study1 %>% drop_na(studyID) %>% 
  select(studyID,Scrn_facility,INFANTID,pregnancy_outcomes,contains("apgar"),BREATH_VSORRES_1,BREATH_VSORRES_2,TEMP_VSORRES,TEMP_VSORRES_2,) 
...
#| fig-height: 6
#| echo: false
Locked_study1 %>% drop_na(studyID) %>% filter(vist%in%c("IPC","PNC-0","PNC-1","PNC-4","PNC-6","PNC-26","PNC-52")) %>% 
 # select(studyID,INFANTID,vist,Scrn_facility,INFANTID,pregnancy_outcomes,MUAC_PERES_1_infant,MUAC_PERES_2_infant,MUAC_PERES_3_infant) %>% View()
  group_by(studyID) %>% fill(pregnancy_outcomes,neonatal_dth,.direction = "updown") %>% ungroup() %>% 
  filter(pregnancy_outcomes =="Live birth",neonatal_dth=="No") %>% 
  select(studyID,INFANTID,vist,Scrn_facility,INFANTID,pregnancy_outcomes,neonatal_dth,`Delivery method`,weight_infant_ipc_pnc52,
  ...
  mutate(`Missing delivery method` = if_else(is.na(`Delivery method`),"Yes","No"),
         ...
         `missing 1st Head circumference` = if_else(is.na(HC_1_inf_combined),"Yes","No") %>% replace(.,vist=="PNC-0",NA),
         `missing 2nd Head circumference` = if_else(is.na(HC_2_inf_combined),"Yes","No")%>% replace(.,vist=="PNC-0",NA),
         `missing 3rd Head circumference` = if_else(is.na(HC_3_inf_combined),"Yes","No")%>% replace(.,vist=="PNC-0",NA),
         `Missing Length` = if_else(is.na(length_1_inf_combined),"Yes","No")%>% replace(.,vist%in%c("PNC-0","PNC-1"),NA),
         `Missing MUAC` = if_else(is.na(MUAC_PERES_1_infant),"Yes","No")%>% replace(.,vist%in%c("PNC-0","IPC"),NA)) %>% 
  droplevels() %>% 
...
infant_dta %>% select(studyID,Scrn_facility,SGA_categories,birth_asphyxia,breath_failure,pregnancy_outcomes,Bag_mask,cont_airwave,
...
#| fig-width: 11
#| fig-height: 5
infant_dta %>% drop_na(SGA_Percentile) %>% #select(studyID,INFANTID,SGA_Percentile) %>% View()
...
#| fig-width: 11
#| fig-height: 5
infant_dta %>% select(studyID,Scrn_facility,SGA_Percentile,SGA_categories,birth_asphyxia,breath_failure,pregnancy_outcomes,Bag_mask,cont_airwave,inf_weight,
...
#| fig-width: 11
#| fig-height: 5
us_difMed <- median((Locked_study1 %>% mutate(ga_diff = abs(US_Ga_days_enrol - LMP_Ga_days_enrol)) %>% filter(vist=="Enrollment"))$ga_diff,na.rm = T)

Locked_study1 %>% mutate(ga_diff = abs(US_Ga_days_enrol - LMP_Ga_days_enrol)) %>% filter(vist=="Enrollment") %>% 
...
#| fig-width: 11
#| fig-height: 5
ga_out_Med <- median((Locked_study1 %>% filter(vist=="IPC") %>% distinct(studyID,.keep_all = T))$Ga_outcome_weeks,na.rm = T) 
preterm_hist <- Locked_study1 %>% tabyl(preterm_delivery_levels) %>% adorn_pct_formatting()

Locked_study1 %>% filter(vist=="IPC") %>% distinct(studyID,.keep_all = T) %>% 
...
EDD_dates <- Data1 %>% drop_na(studyID) %>%  select(studyID,US_EDD_enrolment,DELIV_DSSTDAT_INF1,BIRTH_DSTERM_INF1) %>%
  group_by(studyID) %>% fill(BIRTH_DSTERM_INF1,.direction = "updown") %>% drop_na(DELIV_DSSTDAT_INF1) %>% 
  distinct(studyID,.keep_all = T) %>% filter(BIRTH_DSTERM_INF1=="Live birth") %>% mutate(date_diff = as.numeric(DELIV_DSSTDAT_INF1-US_EDD_enrolment)) %>% 
...
#| fig-width: 12
#| fig-height: 5.8
Data1 %>% drop_na(studyID) %>%  
  select(studyID,INFANTID,vist,US_EDD_enrolment,US_Date_enrolment,DELIV_DSSTDAT_INF1,Ga_outcome_weeks) %>% 
 # group_by(studyID) %>% fill(pregnancy_outcomes,.direction = "updown") %>% 
  distinct(studyID,.keep_all = T) %>% 
...
# Locked_study1 %>% drop_na(studyID) %>%  
#   select(studyID,INFANTID,vist,US_EDD_enrolment,US_Date_enrolment,DELIV_DSSTDAT_INF1,Ga_outcome_weeks,pregnancy_outcomes) %>% 
#   group_by(studyID) %>% fill(pregnancy_outcomes,.direction = "updown") %>% 
#   distinct(studyID,INFANTID,.keep_all = T) %>% 
#   filter(Ga_outcome_weeks>=42) %>% View()
```

## study1 Birth Facilities
...
#| fig-width: 14
#| fig-height: 8
birth_center <- Locked_study1 %>% drop_na(studyID) %>% filter(vist == "IPC") %>% 
...
birth_center %>% select(Year,Birth_facility) %>% #mutate(Year = forcats::fct_na_value_to_level(Year,)) %>% 
  ...
  add_overall() %>%  modify_spanning_header(c("stat_1", "stat_2") ~ "**YEAR**") %>% as_gt() %>% 
    tab_header(title = "Distribution of study1 IPCs against Birth locations") %>% opt_table_outline(style = "solid",width = px(2.4),color = "black") %>% 
...
#| fig-width: 10
#| fig-height: 8
obstetric <- Data1 %>% drop_na(studyID) %>% #filter(PH_PREV_RPORRES== "Yes") %>%  
  select(studyID,`Antepartum hemorrhage` = APH_RPORRES,`Postpartum hemorrhage` = PPH_RPORRES, #Oligohydramnios = OLIGOHYDRAMNIOS_RPORRES,
  ...
  group_by(studyID) %>% fill(everything(.)) %>% ungroup() %>% distinct(studyID,.keep_all = T)


 obstetric %>% select(-studyID) %>%  mutate_all(~replace(.x,.x=="Don’t know",NA)) %>% droplevels() %>% 
...
Locked_study1 %>% mutate(MALARIA_POC_LBORRES = case_when(MALARIA_POC_LBORRES=="Positive"~"Yes",MALARIA_POC_LBORRES=="Negative"~"No"),
                         POC_hemocue_YN = case_when(POC_hemocue_YN=="Anemic"~"Yes",POC_hemocue_YN=="Normal"~"No")) %>% 
  select(studyID,vist,`BMI Levels: (Kg/m2)` = BMI_Levels,`Maternal Age` = maternal_age,`MUAC Levels` = MUAC_levels,
  ...
  filter(vist == "Enrollment") %>% select(-1,-2) %>% droplevels() %>% 
...
Locked_study1 %>% mutate(MALARIA_POC_LBORRES = case_when(MALARIA_POC_LBORRES=="Positive"~"Yes",MALARIA_POC_LBORRES=="Negative"~"No"),
  ...
  group_by(studyID) %>% fill(Birth_outcome,.direction = "updown") %>% ungroup() %>% 
  select(studyID,vist,Birth_outcome,`BMI Levels: (Kg/m2)` = BMI_Levels,`Maternal Age` = maternal_age,`MUAC Levels` = MUAC_levels,
         `Anemia: POC hemocue` = POC_hemocue,`Anemic: POC hemocue` = POC_hemocue_YN, `Maternal height: cm` = HEIGHT_PERES,
         `Weight: Kg` = WEIGHT_PERES,`Malaria positivity` = MALARIA_POC_LBORRES,`Temperature Celsius` = TEMP_VSORRES,
         `Maternal Heart rate` = MHR_VSORRES,`Respiratory rate: breaths per minute`= RR_VSORRES,`oxygen saturation (SpO2): %` = PULSEOX_VSORRES, 
         `Systolic Bp: mm Hg` = `Systolic Bp`,`Diastolic Bp: mm Hg` = `Diastolic Bp`, `Pallor signs` = PALLOR_VSORRES) %>% labelled::remove_var_label() %>% 
  filter(vist == "Enrollment") %>% select(-1,-2,-`oxygen saturation (SpO2): %`) %>% droplevels() %>% 
...
#| fig-height: 5.2
#| echo: false
Locked_study1 %>% drop_na(studyID) %>% filter(MAT_VISIT_MNH04 == "Yes, visit conducted in person") %>% 
  select(studyID,vist,`Hypertensive disorder (HPD)`= HPD_MHOCCUR,`Type of HPD` = HPD_MHTERM) %>% 
  filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","Non-scheduled visit")) %>% 
  distinct(studyID,vist,.keep_all = T) %>% mutate_all(~replace(.x,.x%in%c("Don’t know","77"),NA)) %>% select(-studyID) %>% 
 droplevels() %>% tbl_summary(by=vist,missing = "no") %>% bold_labels() %>% italicize_levels() %>% as_gt() %>% 
...
#   select(vist,`Bedrest, at home` = HPD_OECTRT_1,`Bedrest, at facility` = HPD_OECTRT_2,`Magnesium sulfate` = HPD_OECTRT_3,
...
#   droplevels() %>% 
#   tbl_summary(by = vist,missing = "no",statistic = all_categorical()~"{n}") %>% as_gt() %>% 
...
desc_data <- Data1 %>% drop_na(studyID) %>% select(studyID,vist,HB_POC_LBORRES,CBC_HB_LBORRES,SPHB_LBORRES) %>% 
  filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","IPC","PNC-0","PNC-1","PNC-4","PNC-6","PNC-26"))
...
desc_data %>% rename(`POC Hemocue` = HB_POC_LBORRES,`HB CBC` = CBC_HB_LBORRES,`POC Masimo` = SPHB_LBORRES) %>%
  ...
  mutate(Anemia = if_else(`Hb value`<11,"Anemic","Normal")) %>% group_by(vist,`HB measurement`) %>% count(Anemia) %>% 
  mutate(Prev = paste0(n," (",round(n/sum(n)*100,1),"%)")) %>% filter(Anemia=="Anemic") %>% select(-c(Anemia,n)) %>% 
  pivot_wider(id_cols = `HB measurement`,names_from = vist,values_from = Prev) %>% 
  mutate(`PNC-0` = replace(`PNC-0`,`HB measurement`=="HB CBC",NA)) %>% gt(row_group_as_column = T) %>% 
   # gt(rowname_col = "HB_Tool",groupname_col ="vist",row_group_as_column = TRUE) %>% 
...
HB_poc_hemacue_tile <- Data1 %>% drop_na(studyID) %>%
  #filter(complete.cases(studyID)) %>%
  select(studyID,vist,HB_POC_LBORRES) %>%
  mutate(HB_POC = factor(case_when(HB_POC_LBORRES >=11~ 1,
                                   HB_POC_LBORRES >=10 & HB_POC_LBORRES <11~ 2,
...
#---------------------------------------------------------------------------
Hb_masimo_tile <- Data1 %>% drop_na(studyID) %>%
  select(studyID,vist,SPHB_LBORRES) %>% mutate(SPHB_LBORRES = replace(SPHB_LBORRES,SPHB_LBORRES<0,NA)) %>%
  ...
  tabyl(vist,SPHB_1)%>% adorn_totals(where = "col") %>%  #select(vist,Normal, Anemic) %>%  
  mutate(vist = as.character(vist)) %>%
  mutate(vist=replace(vist,vist==" N/A",as.character(NA))) %>%
  drop_na(vist) %>% filter(!vist%in%c("N/A","Non-scheduled ANC visit for routine care","Non-scheduled visit",
                                                  "Non-scheduled visit for routine care","14")) %>%
...
#---------------------------------------------------------------------------
HB_CBC_tile <- Data1 %>% filter(complete.cases(studyID),vist == "Enrollment") %>%
  select(vist,CBC_HB_LBORRES) %>%
  ...
  tabyl(vist,CBC_HB_1) %>% adorn_totals(where = "col") %>%  #select(vist,Normal, Anemic) %>% 
  filter(vist == "Enrollment") %>%
  mutate(`Anemia Prevalence (%)` = round((Anemic/Total)*100,1),
         HB_tool = paste0("CBC")) #%>% view()

HB_CBC_tile %>% bind_rows(HB_poc_hemacue_tile) %>% bind_rows(Hb_masimo_tile) %>% filter(!vist%in%c("Non-scheduled visit","14")) %>%
  mutate(vist = fct_relevel(vist,c("Enrollment","ANC-20","ANC-28","ANC-32",
                                               "ANC-36","IPC","PNC-0","PNC-1",
                                               "PNC-4","PNC-6","PNC-26","PNC-52"))) %>%
  ggplot(aes(y = HB_tool,x=vist,fill=`Anemia Prevalence (%)`,color=HB_tool)) +
...
#| fig-height: 7
#| echo: false
Data1 %>% drop_na(studyID) %>% select(studyID,vist,HB_POC_LBORRES,CBC_HB_LBORRES) %>% #,SPHB_LBORRES
  filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","PNC-6")) %>% 
...
rename(`POC Hemocue` = HB_POC_LBORRES,`HB CBC` = CBC_HB_LBORRES) %>% # `POC Masimo` = SPHB_LBORRES
  ...
  ggplot(aes(y = `HB measurement`,x = vist)) + labs(x="Study Visits",y="",fill ="Prev %") +
  theme_light() + 
...
desc_data %>% rename(`POC Hemocue` = HB_POC_LBORRES,`HB CBC` = CBC_HB_LBORRES,`POC Masimo` = SPHB_LBORRES) %>%
  ...
  mutate(Anemia = case_when(`Hb value`>=11.0~"Normal",
                            ...
                            `Hb value`<7.0~"Severe")) %>% select(vist ,`HB measurement`,Anemia) %>% 
  group_by(vist,`HB measurement`) %>% count(Anemia) %>% 
  mutate(Prev = paste0(n," (",round(n/sum(n)*100,1),"%)")) %>% select(-n) %>% 
  pivot_wider(id_cols = c(`HB measurement`,Anemia),names_from = vist,values_from = Prev) %>% 
...
alluvialdta <- Locked_study1 %>% drop_na(studyID) %>% filter(vist %in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36")) %>% 
  select(studyID,vist,POC_hemocue) %>% 
  distinct() %>%  
  pivot_wider(id_cols = studyID,names_from = vist,values_from = POC_hemocue) %>% select(-studyID) %>% 
...
#| echo: false
#| 
Ga_data <-   Data1 %>% drop_na(studyID) %>% filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36")) %>%
  select(studyID,CON_DSSTDAT,ANC_OBSSTDAT,ANT_PEDAT,vist,US_GA_WKS_AGE_FTS1,US_GA_DAYS_AGE_FTS1,HB_POC_LBORRES) %>% 
  mutate(ANC_OBSSTDAT1 = if_else(is.na(ANC_OBSSTDAT)& !is.na(ANT_PEDAT),ANT_PEDAT,ANC_OBSSTDAT),
         ANC_OBSSTDAT1 = if_else(is.na(ANC_OBSSTDAT1)& vist=="Enrollment",CON_DSSTDAT,ANC_OBSSTDAT1)) %>% 
  mutate(ga_wks = round(US_GA_WKS_AGE_FTS1 + (US_GA_DAYS_AGE_FTS1/7),1),
         ga_wks = if_else(!vist=="Enrollment",as.double(NA),ga_wks),
         ga_at_enrollment = ga_wks) %>%
  group_by(studyID) %>% fill(c(CON_DSSTDAT,ga_at_enrollment),.direction = "updown") %>% ungroup() %>%
...
#| echo: false
desc_data <- Data1 %>% drop_na(studyID) %>% select(studyID,vist,HB_POC_LBORRES,CBC_HB_LBORRES,SPHB_LBORRES) %>% 
  filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","IPC"))


bind_rows(desc_data %>% filter(vist == "Enrollment") %>% 
descr(transpose = T) %>% as.data.frame() %>% rownames_to_column() %>% 
  ...
  mutate(vist = "Enrollment"),

desc_data %>% filter(vist == "ANC-20") %>% 
descr(transpose = T) %>% as.data.frame() %>% rownames_to_column() %>% 
  select(HB_Tool = rowname,N= "N.Valid","Mean","Std.Dev","Min","Q1","Median","Q3","Max","IQR","CV","Skewness","Kurtosis") %>% 
  mutate(vist = "ANC-20"),

desc_data %>% filter(vist == "ANC-28") %>% 
descr(transpose = T) %>% as.data.frame() %>% rownames_to_column() %>% 
  select(HB_Tool = rowname,N= "N.Valid","Mean","Std.Dev","Min","Q1","Median","Q3","Max","IQR","CV","Skewness","Kurtosis") %>% 
  mutate(vist = "ANC-28"),


desc_data %>% filter(vist == "ANC-32") %>% 
descr(transpose = T) %>% as.data.frame() %>% rownames_to_column() %>% 
  select(HB_Tool = rowname,N= "N.Valid","Mean","Std.Dev","Min","Q1","Median","Q3","Max","IQR","CV","Skewness","Kurtosis") %>% 
  mutate(vist = "ANC-32") %>% filter(HB_Tool!="CBC_HB_LBORRES"),


desc_data %>% filter(vist == "ANC-36") %>% 
descr(transpose = T) %>% as.data.frame() %>% rownames_to_column() %>% 
  select(HB_Tool = rowname,N= "N.Valid","Mean","Std.Dev","Min","Q1","Median","Q3","Max","IQR","CV","Skewness","Kurtosis") %>% 
  mutate(vist = "ANC-36"),

desc_data %>% filter(vist == "IPC") %>% 
  descr(transpose = T) %>% as.data.frame() %>% rownames_to_column() %>% 
  select(HB_Tool = rowname,N= "N.Valid","Mean","Std.Dev","Min","Q1","Median","Q3","Max","IQR","CV","Skewness","Kurtosis") %>% 
  mutate(vist = "IPC") %>% filter(HB_Tool!="CBC_HB_LBORRES")) %>%  mutate_if(is.double,~round(.x,2)) %>% 
  gt(rowname_col = "HB_Tool",groupname_col ="vist",row_group_as_column = TRUE) %>% 
...
# hiv data
hiv_data <- Data1 %>% drop_na(studyID) %>% filter(!vist%in%c("ANC Non-scheduled visit","PNC Non-scheduled visit")) %>% 
   anti_join(Data1 %>% drop_na(studyID) %>% filter(US_GA_WKS_AGE_FTS1_Enrol>=18,vist=="ANC-20")) %>% 
  distinct(studyID,vist,.keep_all = T) %>%  select(studyID,vist,HIV_POC_LBORRES1) %>% 
  mutate(.=replace(.,.==55,NA)) %>%
  pivot_wider(id_cols = studyID,names_from = vist,values_from = HIV_POC_LBORRES1) %>% 
  ...
  pivot_longer(cols = Enrollment:`PNC-26`,values_to = "HIV_POC_LBORRES1",names_to = "vist") %>% 
  drop_na(HIV_POC_LBORRES1) %>% select(-`PNC-52`)


hiv_incidence <- Data1 %>% drop_na(studyID) %>% filter(!vist%in%c("ANC Non-scheduled visit","PNC Non-scheduled visit")) %>% 
   anti_join(Data1 %>% drop_na(studyID) %>% filter(US_GA_WKS_AGE_FTS1_Enrol>=18,vist=="ANC-20")) %>% 
  distinct(studyID,vist,.keep_all = T) %>%  select(studyID,vist,HIV_POC_test) %>% 
  mutate(.=replace(.,.==55,NA)) #%>% tabyl(HIV_POC_test)
  # pivot_wider(id_cols = studyID,names_from = vist,values_from = HIV_POC_LBORRES) %>% 
  # mutate(`ANC-20`=if_else(Enrollment=="Positive"&`ANC-20`=="Positive",NA,`ANC-20`),
  #        `ANC-28`=if_else(Enrollment=="Positive"&`ANC-28`=="Positive",NA,`ANC-28`),
  #        `ANC-32`=if_else(Enrollment=="Positive"&`ANC-32`=="Positive",NA,`ANC-32`),
  #        `ANC-36`=if_else(Enrollment=="Positive"&`ANC-36`=="Positive",NA,`ANC-36`),
  #        IPC=if_else(Enrollment=="Positive"&IPC=="Positive",NA,IPC)) %>% 
  # pivot_longer(cols = Enrollment:`PNC-26`,values_to = "HIV_POC_LBORRES",names_to = "vist") %>% 
  # drop_na(HIV_POC_LBORRES)


# syphylis data
syph_data <- Data1 %>% drop_na(studyID) %>% filter(!vist%in%c("ANC Non-scheduled visit","PNC Non-scheduled visit")) %>% 
  anti_join(Data1 %>% drop_na(studyID) %>% filter(US_GA_WKS_AGE_FTS1_Enrol>=18,vist=="ANC-20")) %>% 
  distinct(studyID,vist,.keep_all = T) %>%  select(studyID,vist,SYPH_POC_LBORRES1) %>% 
  mutate(.=replace(.,.==55,NA)) %>%
  pivot_wider(id_cols = studyID,names_from = vist,values_from = SYPH_POC_LBORRES1) %>% 
  mutate(`ANC-20`=if_else(Enrollment=="Positive"&`ANC-20`=="Positive",NA,`ANC-20`),
         `ANC-28`=if_else(Enrollment=="Positive"&`ANC-28`=="Positive",NA,`ANC-28`),
         `ANC-32`=if_else(Enrollment=="Positive"&`ANC-32`=="Positive",NA,`ANC-32`),
         `ANC-36`=if_else(Enrollment=="Positive"&`ANC-36`=="Positive",NA,`ANC-36`),
  ...
  pivot_longer(cols = Enrollment:`PNC-26`,values_to = "SYPH_POC_LBORRES1",names_to = "vist") %>% 
  drop_na(SYPH_POC_LBORRES1) %>% select(-`PNC-52`)

# hbv data
hbv_data <- Data1 %>% drop_na(studyID) %>% filter(!vist%in%c("ANC Non-scheduled visit","PNC Non-scheduled visit")) %>% 
  anti_join(Data1 %>% drop_na(studyID) %>% filter(US_GA_WKS_AGE_FTS1_Enrol>=18,vist=="ANC-20")) %>% 
  distinct(studyID,vist,.keep_all = T) %>%  select(studyID,vist,HBV_POC_LBORRES) %>% 
  mutate(.=replace(.,.==55,NA)) %>%
  pivot_wider(id_cols = studyID,names_from = vist,values_from = HBV_POC_LBORRES) %>% 
  mutate(`ANC-20`=if_else(Enrollment=="Positive"&`ANC-20`=="Positive",NA,`ANC-20`),
         `ANC-28`=if_else(Enrollment=="Positive"&`ANC-28`=="Positive",NA,`ANC-28`),
         `ANC-32`=if_else(Enrollment=="Positive"&`ANC-32`=="Positive",NA,`ANC-32`),
         `ANC-36`=if_else(Enrollment=="Positive"&`ANC-36`=="Positive",NA,`ANC-36`),
         IPC=if_else(Enrollment=="Positive"&IPC=="Positive",NA,IPC),
         `PNC-26`=if_else(Enrollment=="Positive"&`PNC-26`=="Positive",NA,`PNC-26`)) %>% 
  pivot_longer(cols = Enrollment:`PNC-26`,values_to = "HBV_POC_LBORRES",names_to = "vist") %>% 
   drop_na(HBV_POC_LBORRES) %>% select(-`PNC-52`)

# hcv data
hcv_data <- Data1 %>% drop_na(studyID) %>% filter(!vist%in%c("ANC Non-scheduled visit","PNC Non-scheduled visit")) %>% 
  anti_join(Data1 %>% drop_na(studyID) %>% filter(US_GA_WKS_AGE_FTS1_Enrol>=18,vist=="ANC-20")) %>% 
  distinct(studyID,vist,.keep_all = T) %>%  select(studyID,vist,HCV_POC_LBORRES) %>% 
  mutate(.=replace(.,.==55,NA)) %>%
  pivot_wider(id_cols = studyID,names_from = vist,values_from = HCV_POC_LBORRES) %>% 
  mutate(`ANC-20`=if_else(Enrollment=="Positive"&`ANC-20`=="Positive",NA,`ANC-20`),
         `ANC-28`=if_else(Enrollment=="Positive"&`ANC-28`=="Positive",NA,`ANC-28`),
         `ANC-32`=if_else(Enrollment=="Positive"&`ANC-32`=="Positive",NA,`ANC-32`),
         `ANC-36`=if_else(Enrollment=="Positive"&`ANC-36`=="Positive",NA,`ANC-36`),
         IPC=if_else(Enrollment=="Positive"&IPC=="Positive",NA,IPC),
         `PNC-26`=if_else(Enrollment=="Positive"&`PNC-26`=="Positive",NA,`PNC-26`)) %>% 
  pivot_longer(cols = Enrollment:`PNC-26`,values_to = "HCV_POC_LBORRES",names_to = "vist") %>% 
  drop_na(HCV_POC_LBORRES)%>% select(-`PNC-52`)


# Helm data
helm_data <- Data1 %>% drop_na(studyID) %>% filter(!vist%in%c("ANC Non-scheduled visit","PNC Non-scheduled visit")) %>% 
  anti_join(Data1 %>% drop_na(studyID) %>% filter(US_GA_WKS_AGE_FTS1_Enrol>=18,vist=="ANC-20")) %>% 
  distinct(studyID,vist,.keep_all = T) %>%  select(studyID,vist,HELM_LBORRES) %>% 
  mutate(.=replace(.,.==55,NA)) #%>%
  # pivot_wider(id_cols = studyID,names_from = vist,values_from = HELM_LBORRES) %>% 
  # mutate(`ANC-20`=if_else(Enrollment=="Positive"&`ANC-20`=="Positive",NA,`ANC-20`),
  #        `ANC-28`=if_else(Enrollment=="Positive"&`ANC-28`=="Positive",NA,`ANC-28`),
  #        `ANC-32`=if_else(Enrollment=="Positive"&`ANC-32`=="Positive",NA,`ANC-32`),
  #        `ANC-36`=if_else(Enrollment=="Positive"&`ANC-36`=="Positive",NA,`ANC-36`),
  #        IPC=if_else(Enrollment=="Positive"&IPC=="Positive",NA,IPC),
  #        `PNC-26`=if_else(Enrollment=="Positive"&`PNC-26`=="Positive",NA,`PNC-26`)) %>% 
  # pivot_longer(cols = Enrollment:`PNC-26`,values_to = "HELM_LBORRES",names_to = "vist") %>% 
  # drop_na(HELM_LBORRES)

# schisto data
schisto_data <- Data1 %>% drop_na(studyID) %>% filter(!vist%in%c("ANC Non-scheduled visit","PNC Non-scheduled visit")) %>% 
  anti_join(Data1 %>% drop_na(studyID) %>% filter(US_GA_WKS_AGE_FTS1_Enrol>=18,vist=="ANC-20")) %>% 
  distinct(studyID,vist,.keep_all = T) %>%  select(studyID,vist,SCHISTO_LBORRES_STOOL) %>% 
  mutate(.=replace(.,.==55,NA)) #%>%
  # pivot_wider(id_cols = studyID,names_from = vist,values_from = SCHISTO_LBORRES_STOOL) %>% 
  # mutate(`ANC-20`=if_else(Enrollment=="Positive"&`ANC-20`=="Positive",NA,`ANC-20`),
  #        `ANC-28`=if_else(Enrollment=="Positive"&`ANC-28`=="Positive",NA,`ANC-28`),
  #        `ANC-32`=if_else(Enrollment=="Positive"&`ANC-32`=="Positive",NA,`ANC-32`),
  #        `ANC-36`=if_else(Enrollment=="Positive"&`ANC-36`=="Positive",NA,`ANC-36`),
  #        IPC=if_else(Enrollment=="Positive"&IPC=="Positive",NA,IPC),
  #        `PNC-26`=if_else(Enrollment=="Positive"&`PNC-26`=="Positive",NA,`PNC-26`)) %>% 
  # pivot_longer(cols = Enrollment:`PNC-26`,values_to = "SCHISTO_LBORRES_STOOL",names_to = "vist") %>% 
...
Diagnosis_data <- Data1 %>% drop_na(studyID) %>% 
  select(studyID,vist,MALARIA_POC_LBORRES,MALBL_LBORRES,HB_POC_LBORRES,CBC_HB_LBORRES) %>% 
  filter(!vist%in%c("Non-scheduled visit",14)) %>% 
  anti_join(Data1 %>% drop_na(studyID) %>% filter(US_GA_WKS_AGE_FTS1_Enrol>=18,vist=="ANC-20") %>% select(studyID,vist)) %>% 
  ...
  droplevels() %>% mutate(`Malaria Positives`=if_else(MALARIA_POC_LBORRES=="Positive","Yes","No"),
                         ...
                         `HIV Positives`=if_else(HIV_POC_LBORRES1=="Positive","Yes","No")) %>% 
  select(`Malaria Positives`,`Malaria Positives (Blood slide)`,vist,`HIV Positives`,
  ...
  mutate(vist =fct_relevel(vist,"Enrollment")) 

  Diagnosis_data %>% tbl_summary(by=vist,missing = "no",digits = list(everything()~c(0,2))) %>% 
...
**Note:** It seems we are not capturing data on `Malaria testing (Blood slide)`. So far we have **`r Diagnosis_data %>% clean_names() %>%  drop_na(malaria_positives_blood_slide) %>% nrow()`** results captured in the database. At Enrollment **`r Diagnosis_data %>% clean_names() %>% drop_na(malaria_positives_blood_slide) %>% filter(vist=="Enrollment") %>% nrow()`** records have been documented.

`HIV new positives` at Enrollment are **`r Diagnosis_data %>% clean_names() %>% drop_na(hiv_new_positives) %>% filter(vist=="Enrollment",hiv_new_positives=="Yes") %>%  nrow()`** out of **`r Diagnosis_data %>% clean_names() %>% drop_na(hiv_new_positives) %>% filter(vist=="Enrollment") %>% nrow()`** tests performed.
...
#| echo: false
Data1 %>% drop_na(studyID) %>% 
  select(vist,MALARIA_POC_LBORRES,MALARIA_EVER_CMTRT_1,MALARIA_EVER_CMTRT_2,MALARIA_EVER_CMTRT_3,
         MALARIA_EVER_CMTRT_88,MALARIA_EVER_CMTRT_77,MALARIA_EVER_CMTRT_99) %>% filter(vist!=14) %>% 
   filter(vist!="Non-scheduled visit") %>% 
  ...
  tbl_summary(by = vist,missing = "no") %>% 
  as_gt() %>% opt_table_outline() %>% #opt_table_lines() %>% 
  sub_missing(missing_text = "-") %>% sub_values(values = "0 (NA%)",replacement = "--") %>% 
...
ogtt_data <- Data1 %>% select(studyID,CON_DSSTDAT,vist,SCRN_FAC_SPFY_OBSLOC,HBA1C_TEST_YN,HBA1C_PRCNT,BGLUC_LBPERF_1,
...
paired_ogtt <- ogtt_data %>% select(studyID, vist, BGLUC_PRETEST_MMOLL_LBORRES) %>% 
  filter(vist == "ANC-28") %>% select(studyID, BGLUC_PRETEST_MMOLL_LBORRES) %>% 
  filter(complete.cases(BGLUC_PRETEST_MMOLL_LBORRES)) %>% filter(BGLUC_PRETEST_MMOLL_LBORRES<5.1) %>% 
  left_join(ogtt_data %>% select(studyID, vist, BGLUC_ORAL_1HR_MMOLL_LBORRES,BGLUC_ORAL_2HR_MMOLL_LBORRES) %>% 
              filter(vist == "ANC-28") %>% select(studyID, BGLUC_ORAL_1HR_MMOLL_LBORRES,BGLUC_ORAL_2HR_MMOLL_LBORRES) %>% 
              filter(complete.cases(BGLUC_ORAL_1HR_MMOLL_LBORRES))) 

ogtt_long_dat <- paired_ogtt %>% 
  select(studyID,BGLUC_PRETEST = BGLUC_PRETEST_MMOLL_LBORRES, BGLUC_ORAL_1HR = BGLUC_ORAL_1HR_MMOLL_LBORRES,BGLUC_ORAL_2HR = BGLUC_ORAL_2HR_MMOLL_LBORRES) %>% 
...
```{r}
Diabetes_data <- Data1 %>% drop_na(studyID) %>% select(studyID,vist,HBA1C_PRCNT,BGLUC_POC_LBORRES,BGLUC_PRETEST_MMOLL_LBORRES,
...
# #| echo: false
# 
# Diabetes_data %>% mutate(HBA1C_PRCNT = replace(HBA1C_PRCNT,HBA1C_PRCNT==-5,NA)) %>% filter(vist=="Enrollment") %>% 
...
# #| echo: false
# 
# Diabetes_data %>% filter(vist=="ANC-28") %>% 
...
# Prevalence of Malaria, HBV & HCV
 # Data1 %>% drop_na(studyID) %>% filter(!vist%in%c("Non-scheduled visit",14)) %>% 
 #  filter(MAT_VISIT_MNH04%in%c("Yes, visit conducted in person",NA)) %>% 
 #  filter(MAT_VISIT_MNH12%in%c("Yes, visit conducted in person",NA)) %>% 
 #  select(studyID,US_GA_WKS_AGE_FTS1_Enrol,vist,MALARIA_POC_LBORRES,
 #         HIV_POC_LBORRES,SYPH_POC_LBORRES1,HBV_POC_LBORRES,HCV_POC_LBORRES) %>% 
 #  anti_join(Data1 %>% drop_na(studyID) %>% filter(US_GA_WKS_AGE_FTS1_Enrol>=18,vist=="ANC-20")) %>% 
 #  group_by(studyID) %>% mutate(ID = row_number(),hbv_2 = if_else(ID>1,HBV_POC_LBORRES,NA)) %>%
 #  fill(hbv_2,.direction = "up") %>% 
 # # mutate(HBV_POC_LBORRES = if_else(vist=="Enrollment"&HBV_POC_LBORRES=="Positive"&hbv_2=="Positive",NA,HBV_POC_LBORRES)) %>% 
 #  #View()
 #  #group_by(studyID,vist) %>% fill(MALARIA_POC_LBORRES:HCV_POC_LBORRES) %>%
 #  distinct(studyID,vist,.keep_all = T) %>% ungroup() %>% 
 #  mutate_at(vars(MALARIA_POC_LBORRES:HCV_POC_LBORRES),~replace(.,.=="N/A",NA)) %>% 
 #  select(vist,MALARIA_POC_LBORRES,HBV_POC_LBORRES,HCV_POC_LBORRES) %>% 
 #   mutate(HBV_POC_LBORRES = replace(HBV_POC_LBORRES,vist=="ANC-20",NA),
 #          HCV_POC_LBORRES = replace(HCV_POC_LBORRES,vist=="ANC-20",NA)) %>% 
 #  
 #  droplevels() %>% 
 #  tbl_summary(by = vist,missing = "no",
...
#| echo: false
#| 
Data1 %>% drop_na(studyID) %>% select(`POC Hemacue` = HB_POC_LBORRES,`HB CBC` = CBC_HB_LBORRES,`POC Masimo` = SPHB_LBORRES,studyID,vist,Ga_visit_days) %>% 
...
Data1 %>% drop_na(studyID) %>% select(`HB POC Hemocue` = HB_POC_LBORRES,`HB CBC` = CBC_HB_LBORRES,`SPHB - POC Masimo` = SPHB_LBORRES,studyID,vist,Ga_visit_days) %>% 
   filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","IPC")) %>%
...
#| fig-height: 6.4
#| echo: false
Data1 %>% drop_na(studyID) %>% select(HB_POC = HB_POC_LBORRES,CBC_HB = CBC_HB_LBORRES,SPHB = SPHB_LBORRES,studyID,vist,Ga_visit_days) %>% filter(complete.cases(HB_POC)) %>% 
  filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","IPC")) %>% 
  ggplot(aes(y = HB_POC,color =vist,fill=vist)) +
  ...
  #geom_text(aes(label = studyID),data = extreme_kft_enrol,angle = 40, vjust = 0) +
  theme_light() +
...
Muac_decimals <- Locked_study1 %>% drop_na(MUAC_PERES) %>% mutate(muac_point_preference = as.factor(round(MUAC_PERES%%1*10))) #%>% 

Muac_decimals %>%  filter(Scrn_facility == "facility_1") %>% 
select(studyID,vist,MUAC_PERES,muac_point_preference,Scrn_facility) %>% filter(!vist%in%c("ANC Non-scheduled visit","PNC Non-scheduled visit")) %>% 
group_by(vist) %>% count(muac_point_preference) %>%  
ggplot(aes(x=muac_point_preference,y = n,color=vist)) +
geom_col(width = .4,fill = "#FCCB80EC",color="gray52",alpha =.5)+
labs(x="MUAC Point preferences",y = "Frequencies",title = "facility_1 -- Distribution of MUAC Point preferences")+
theme_pubclean() +
theme(axis.text = element_text(size = 10,colour = "darkblue"),
plot.title = element_text(hjust = 0.3),
strip.text = element_text(size = 10,color="black"))+
facet_wrap(vars(vist),scales = "free")
```
...
weight_loss <- Locked_study1 %>% drop_na(studyID) %>% select(studyID,vist,WEIGHT_PERES,BMI_Levels,Ga_visit_weeks,Ga_visit_days) %>% 
  filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","IPC")) %>% 
  drop_na(WEIGHT_PERES) %>% arrange(studyID,vist) %>% group_by(studyID) %>% mutate(weight_gain = WEIGHT_PERES - first(WEIGHT_PERES)) #%>% View()

# Query
# weight_loss %>% filter(weight_gain<0) %>%  left_join(Locked_study1 %>% select(studyID,vist,Weight_Enrolment=WEIGHT_PERES) %>% filter(vist == "Enrollment") %>% select(-vist)) %>% 
#   select(studyID,vist,Weight_Enrolment,WEIGHT_PERES,weight_gain,everything()) %>% View()
...
#| fig-height: 5.8
#| echo: false
muac_loss <- Locked_study1 %>% drop_na(studyID) %>% select(studyID,vist,MUAC_levels,
                                            MUAC_PERES,Ga_visit_weeks,Ga_visit_days) %>%
  filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","IPC")) %>%
  drop_na(MUAC_PERES) %>% arrange(studyID,vist) %>%  group_by(studyID) %>% mutate(muac_gain = MUAC_PERES - first(MUAC_PERES)) 
...
#| fig-height: 5.2
#| echo: false
Locked_study1 %>% select(studyID,vist,Age_categories) %>% drop_na(Age_categories) %>% distinct(studyID,.keep_all = T) %>% 
...
#| fig-height: 5.2
#| echo: false
Data1 %>% filter(complete.cases(studyID)) %>% select(BRTHDAT,CON_DSSTDAT,ESTIMATED_AGE) %>%
...
ggMarginal(Data1 %>% select(studyID,vist,INFANTID_INF1,BW_FAORRES_REPORT,BW_FAORRES,BW_EST_FAORRES,WEIGHT_PERES,HEIGHT_PERES) %>% 
  mutate_if(is.numeric,~replace(.x, .x == -5, NA)) %>% 
  mutate(inf_weight = if_else(is.na(BW_FAORRES),BW_FAORRES_REPORT,BW_FAORRES),
         inf_weight = round(inf_weight/1000,2)) %>% group_by(studyID) %>% fill(inf_weight,.direction = "updown") %>% ungroup() %>% 
  filter(vist=="Enrollment") %>%  mutate(BMI = round(WEIGHT_PERES/(HEIGHT_PERES/100)^2,1),Birthweight = inf_weight) %>% filter(complete.cases(BMI,Birthweight))  %>% 
...
#| fig-height: 6
#| echo: false
ggMarginal(Data1 %>% select(INFANTID_INF1,BW_FAORRES_REPORT,BW_FAORRES,BW_EST_FAORRES,WEIGHT_PERES,HEIGHT_PERES,vist) %>% 
...
#| fig-height: 6
#| echo: false
ggMarginal(Data1 %>% select(studyID,vist,INFANTID_INF1,BW_FAORRES_REPORT,BW_FAORRES,BW_EST_FAORRES,WEIGHT_PERES,HEIGHT_PERES,MUAC_PERES) %>% 
  mutate_if(is.numeric,~replace(.x, .x == -5, NA)) %>% 
  mutate(inf_weight = if_else(is.na(BW_FAORRES),BW_FAORRES_REPORT,BW_FAORRES),
         inf_weight = round(inf_weight/1000,2)) %>% group_by(studyID) %>% fill(inf_weight,.direction = "updown") %>% ungroup() %>% 
    mutate(Birthweight = inf_weight) %>% 
  filter(vist=="Enrollment") %>% filter(complete.cases(MUAC_PERES,Birthweight)) %>% 
...
ggMarginal(Data1 %>% select(studyID,vist,INFANTID_INF1,BW_FAORRES_REPORT,BW_FAORRES,BW_EST_FAORRES,WEIGHT_PERES,HEIGHT_PERES,MUAC_PERES) %>% 
  ...
  mutate(inf_weight = if_else(is.na(BW_FAORRES),BW_FAORRES_REPORT,BW_FAORRES),
         inf_weight = round(inf_weight/1000,2)) %>% group_by(studyID) %>% fill(inf_weight,.direction = "updown") %>% ungroup() %>% 
    mutate(Birthweight = inf_weight) %>% 
  filter(vist=="ANC-36") %>% filter(complete.cases(MUAC_PERES,Birthweight)) %>%
             ggplot(aes(y = Birthweight, x = MUAC_PERES)) +
             geom_point()+
...
#| fig-height: 5.4
#| echo: false
Data1 %>% select(studyID,vist,INFANTID_INF1,BW_FAORRES_REPORT,BW_FAORRES,BW_EST_FAORRES,WEIGHT_PERES,HEIGHT_PERES,vist,MUAC_PERES) %>% 
  filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36","IPC")) %>% 
  mutate_if(is.numeric,~replace(.x, .x == -5, NA)) %>% 
  mutate(Birthweight = round(BW_FAORRES_REPORT/1000,2),
         BMI = round(WEIGHT_PERES/(HEIGHT_PERES/100)^2,1))  %>% 
  ggplot(aes(x=MUAC_PERES,y=BMI,color=vist))+
  geom_jitter() +
...
#| fig-height: 6
#| echo: false
Data1 %>% drop_na(studyID) %>% select(studyID,vist,MUAC_PERES,HEIGHT_PERES,WEIGHT_PERES,MAT_VISIT_MNH04) %>%  
  filter(MAT_VISIT_MNH04=="Yes, visit conducted in person") %>% select(-MAT_VISIT_MNH04) %>% 
  filter(vist %in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36")) %>% 
 ...
 mutate(MUAC=factor(case_when(muac_peres<23~2,
  ...
  select(vist,MUAC,BMI) %>% droplevels() %>% 
  tbl_summary(by =vist,missing = "no",digits = everything()~c(0,1)) %>% bold_labels() %>% italicize_levels() %>% 
...
alluvialmuac<- Locked_study1 %>% drop_na(studyID) %>% filter(vist %in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36")) %>% 
  select(studyID,vist,MUAC_levels) %>% 
  distinct() %>%  
  pivot_wider(id_cols = studyID,names_from = vist,values_from = MUAC_levels) %>% select(-studyID) %>% 
...
#| fig-height: 5.4
#| echo: false
Data1 %>% drop_na(studyID) %>%  select(studyID,vist,BP_DIA_VSORRES_1,BP_DIA_VSORRES_2,BP_DIA_VSORRES_3,
  ...
  filter(vist%in%c("Enrollment","ANC-20","ANC-28","ANC-32","ANC-36")) %>% 
  mutate_if(is.numeric,~replace(.x, .x == -5, NA)) %>% 
  ggplot(aes(x=`DIASTOLIC Bp`,y=`SYSTOLIC Bp`,color=vist))+
  geom_jitter() +
...
gt(Data1 %>% drop_na(studyID) %>% 
     filter(!vist%in%c("ANC Non-scheduled visit","PNC Non-scheduled visit")) %>% labelled::remove_var_label() %>% 
     select(studyID,vist,POC_Hemocue = HB_POC_LBORRES,CBC = CBC_HB_LBORRES) %>% 
...
cbc_vars <- Data1 %>% select(studyID,CON_DSSTDAT,vist,contains("cbc")) %>% select_if(is.numeric) %>% select(-1)
cbc_LBORRES <- Data1 %>% select(studyID, vist,colnames(cbc_vars)) %>%
  pivot_longer(cols = 3:colnames(cbc_vars)[19],names_to = "CBC",values_to = "CBC_measurement") %>% 
  filter(complete.cases(CBC_measurement)) 

#extreme values
extreme1 <- cbc_LBORRES %>% filter(CBC%in%c(colnames(cbc_vars)[1:10])) %>% group_by(CBC,vist) %>%  rstatix::identify_outliers(CBC_measurement) %>% ungroup() %>% filter(is.extreme == TRUE) #%>% View()

# 1. CBC distributions

cbc_LBORRES %>% filter(CBC%in%c(colnames(cbc_vars)[1:10])) %>% #select(1,2,11,12) %>%  View()
  ggplot(aes(x = CBC, y = CBC_measurement,fill = vist)) +
  ...
  #geom_text(aes(label = studyID),data = extreme1,angle = 40, vjust = 0,size = 4) +
  labs(x = "", y = "",
...
extreme_lft_Enrol <- Data1 %>% filter(complete.cases(studyID)) %>% 
  filter(vist%in%c("Enrollment","ANC-32")) %>% filter(vist == "Enrollment") %>% 
  mutate_if(is.double,~replace(.,.==-7,NA)) %>%   mutate_if(is.double,~replace(.,.==-77,NA)) %>%
  mutate_if(is.double,~replace(.,.==-5,NA)) %>% 
  select(studyID,vist,AST_ul_LBORRES,	ALT_ul_LBORRES,
         ALP_LBORRES,	TBILIRUBIN_LBORRES,	DBILIRUBIN_LBORRES,	TPROTEIN_LBORRES,
...
Data1 %>% filter(complete.cases(studyID)) %>% filter(vist%in%c("Enrollment","ANC-32")) %>% filter(vist == "Enrollment") %>% 
  ...
  #geom_text(aes(label = studyID),data = extreme_lft_Enrol,angle = 40, vjust = 0) +
  theme_light() +
...
#extreme values
extreme_lft_anc32 <- Data1 %>% filter(complete.cases(studyID)) %>% 
  filter(vist%in%c("Enrollment","ANC-32")) %>% filter(vist == "ANC-32") %>% 
...
Data1 %>% filter(complete.cases(studyID)) %>% filter(vist%in%c("Enrollment","ANC-32")) %>% filter(vist == "ANC-32") %>% 
  mutate_if(is.double,~replace(.,.==-7,NA)) %>%   mutate_if(is.double,~replace(.,.==-77,NA)) %>% mutate_if(is.numeric,~replace(.,.==-5,NA)) %>% 
  select(studyID,vist,AST_ul_LBORRES,	ALT_ul_LBORRES,
         ALP_LBORRES,	TBILIRUBIN_LBORRES,	DBILIRUBIN_LBORRES,	TPROTEIN_LBORRES,
         ALBUMIN_LBORRES, GAMMAGT_LBORRES, IBILIRUBIN_LBORRES) %>% 
  ...
  # geom_text(aes(label = studyID),data = extreme_lft_anc32,angle = 40, vjust = 0) +
  theme_light() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold", color = "black",size = 22),
        panel.border = element_rect(colour = "black",linewidth = .8),
        axis.title =  element_text( face = "italic", color = "black", size = 16),
...
#extreme values
extreme_kft_enrol <- Data1 %>% filter(complete.cases(studyID)) %>%
  filter(vist%in%c("Enrollment","ANC-32")) %>% filter(vist == "Enrollment") %>% 
  mutate_if(is.double,~replace(.,.==-7,NA)) %>% mutate_if(is.numeric,~replace(.,.==-5,NA)) %>% 
  select(studyID, SCRN_FAC_SPFY_OBSLOC, BUN_mmoll_LBORRES,CREAT_umoll_LBORRES,
...
#extreme values
extreme_kft_anc32 <- Data1 %>% filter(complete.cases(studyID)) %>% 
  filter(vist%in%c("Enrollment","ANC-32")) %>% filter(vist == "ANC-32") %>%
  mutate_if(is.double,~replace(.,.==-7,NA)) %>% mutate_if(is.numeric,~replace(.,.==-5,NA)) %>% 
  select(studyID, SCRN_FAC_SPFY_OBSLOC, BUN_mmoll_LBORRES,CREAT_umoll_LBORRES,
         SODIUM_LBORRES,	POTASSIUM_LBORRES,	CHLORIDE_LBORRES, PHOSPHORUS_LBORRES,	CALCIUM_LBORRES,CARB_DIOX_LBORRES) %>% 
...
Data1 %>% filter(complete.cases(studyID)) %>% filter(complete.cases(HB_POC_LBORRES)) %>% 
  filter(!vist%in%c("ANC Non-scheduled visit","PNC Non-scheduled visit")) %>% 
  select(studyID,vist,HB_POC_LBORRES) %>% 
  ggplot(aes(x=vist,y=HB_POC_LBORRES)) +
  geom_boxplot(width = .6, outlier.colour = "darkred", fill = c("#C1FFC1"), notch = TRUE,
...
#| fig-height: 8
#| echo: false
Us_var <- Data1 %>% drop_na(studyID) %>% select(contains("fts")) %>%  select_if(is.numeric) %>%
  mutate_if(is.numeric,~replace(.,.==77,NA)) %>% mutate_if(is.numeric,~replace(.,.==-7,NA)) %>% remove_empty() %>% colnames()

us_transfmd <- Data1 %>% select(studyID,vist,all_of(Us_var)) %>% filter(vist%in%c("Enrollment","ANC-32")) %>%
  select(studyID,vist,contains("res")) %>% select(studyID,vist,contains("fts1")) %>% 
...
us_transfmd %>% filter(vist == "Enrollment") %>% filter(!US_vars%in%c("Estimated fetal weight","fetal heart rate")) %>% 
...
us_transfmd %>% filter(vist == "ANC-32")%>% filter(!US_vars%in%c("Estimated fetal weight","fetal heart rate")) %>% #filter(US_vars =="Femur Length:mean") %>%  View()
...
[**This marks the END of study1 report.**]{style="color: blue;"}
